...
int readfromC(int first, int nsize, double *posr, double *post,
	...
	rad.set_domain(d) = space.get_domain(d)->get_radius() ;
      ...
      Scalar gff (space) ;
      gff = bigR2+pow(aa/rad,2)*delta*(1.0-pow(cos,2));
      gff.set_domain(0) = 1 ;
      gff.coef();

      Scalar grf (space) ;
      grf = -aa/rad*delta*sin;
      grf.set_domain(0) = 1 ;
      grf.coef();
      ...
      // Check if one can compute the coef
             dgff.coef() ;
      Scalar Nf (space) ;
      Nf =  2.0*M*aa/(bigR2*rad*delta);//*sin;
      Nf.std_base() ;
      Nf.set_domain(0) = 0 ;
      Nf.coef() ;

      Scalar lapse (space) ;
      lapse = sqrt(1.0/delta);
      lapse.std_base() ;
      ...
      Scalar dbeta (space) ;
       dbeta =Nf/(rad*sigma)*(2.0-pow(aa/rad,2)*2.0*M/rad+pow(aa/rad,2)*(2.0*M/rad-2.0*pow(aa/rad,2))*cos);
        ...
        Scalar grf_der_r(grf.der_r());
        Scalar grf_der_t(grf.der_var(2));
	...
	// Reconstruction mapping :
        ...
        dg13dr[i]   = grf_der_r.val_point(MM);
        dg13dt[i]   = grf_der_t.val_point(MM);
