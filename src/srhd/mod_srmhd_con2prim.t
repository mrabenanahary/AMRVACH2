      ...
      if(f*df==zero) then
         if(f==zero) then
            ...
            if(tests)then
              ...
              !         srmhd_gamma*small_pressure/(srmhd_gamma-one))*lfaci
...
if(ssqr .ne. zero)then
  ...
  v=dsqrt(one-one/(lfac**2))*(s + b*sdotb/xi)/temp
  if((sum(v)**2.0d0)>one) then
...
LoopNR:  do ni=1,srmhd_maxiterationNR
     ...
     lfac2inv=one - sqrs/xicurrent**2.0d0
...
!                                  tau,sqrs,xicurrent,h,ierror)
       ...
       !=========================================!
        ...
        if( Nff*ff>zero)then
           ...
           d   = 2.0d0*(one + 10.0d0 * small_density) * small_density
           tau = 2.0d0*(one + 10.0d0 * small_e) * small_e
           s   =zero
           pcurrent     = (srmhd_gamma-one)*tau
           xi=tau+d+pcurrent
           lfac = one
...
if (vsqr<one) then
   ...
   dF = one + sb2/xi**3.0d0  -dpdxi+dlfacdxi*bsqr/lfac**3.0d0
else 
