module mod_mhd_alfven_hllc
  use mod_mhd_alfven_phys

  implicit none
  private

  public :: mhd_alfven_hllc_init

contains

  subroutine mhd_alfven_hllc_init()
    use mod_physics_hllc

    phys_diffuse_hllcd => mhd_alfven_diffuse_hllcd
    phys_get_lCD => mhd_alfven_get_lCD
    phys_get_wCD => mhd_alfven_get_wCD

  end subroutine mhd_alfven_hllc_init

  subroutine mhd_alfven_diffuse_hllcd(ixI^L,ixO^L,idim,wLC,wRC,fLC,fRC,patchf)
  ...
  end subroutine mhd_alfven_diffuse_hllcd

  subroutine mhd_alfven_get_lCD(wLC,wRC,fLC,fRC,cmin,cmax,idim,ixI^L,ixO^L, &
  ...
  end subroutine mhd_alfven_get_lCD

  subroutine mhd_alfven_get_wCD(x,wLC,wRC,whll,fRC,fLC,Fhll,patchf,lambdaCD,&
                    cmin,cmax,&
                    ixI^L,ixO^L,idim,f)
  ...
  ! reference T. Miyoski, Kusano JCP, 2008, 2005.
    ...
    double precision, dimension(ixI^S)                       :: vSub,cspeed,&
                                                                pCD,VdotBCD
    integer         , dimension(ixI^S), intent(inout)        :: patchf
    ...
    do iw=1,nwflux
     if(iw == mag(idim)) then
       f(ixO^S,iw)=zero
     else if(mhd_alfven_glm .and. iw == psi_) then
       f(ixO^S,iw)=zero
     else
  ...
  end subroutine mhd_alfven_get_wCD

end module mod_mhd_alfven_hllc
