...
module mod_mhd_alfven_roe
  ...
  public :: mhd_alfven_roe_init

contains

  subroutine mhd_alfven_roe_init()
    use mod_global_parameters, only: entropycoef, nw
    integer :: il

    phys_average => mhd_alfven_average
    phys_get_eigenjump => mhd_alfven_get_eigenjump
    phys_rtimes => mhd_alfven_rtimes
  ...
  end subroutine mhd_alfven_roe_init
  ...
  ! Calculate also alpha_f,alpha_s,c_f,c_s,csound2,dpres,rhodv
  !
  ! wL,wR,wroe are all interface centered quantities
  subroutine mhd_alfven_average(wL,wR,x,ix^L,idim,wroe,workroe)
  ...
  end subroutine mhd_alfven_average
  ...
  !
  ! wL,wR,wroe are all interface centered quantities
  subroutine average2(wL,wR,x,ix^L,idim,wroe,cfast,cslow,afast,aslow,csound2,dpres, &
       rhodv,tmp)
    use mod_global_parameters
    ...
    double precision, dimension(ixG^T)    :: cfast,cslow,afast,aslow,csound2,dpres, &
         rhodv,tmp
    ...
    call mhd_alfven_get_pthermal(wL,x,ixG^LL,ix^L,afast)
    call mhd_alfven_get_pthermal(wR,x,ixG^LL,ix^L,aslow)

    if(mhd_alfven_energy) then
      wroe(ix^S,e_)=half*(afast(ix^S)+aslow(ix^S))
      ! dpres=pR-pL
      dpres(ix^S)=aslow(ix^S)-afast(ix^S)
    else
      dpres(ix^S)=aslow(ix^S)-afast(ix^S)
    end if
    ...
    ! get csound**2
    if(mhd_alfven_energy) then
      csound2(ix^S)=mhd_alfven_gamma*wroe(ix^S,p_)/wroe(ix^S,rho_)
    else
      csound2(ix^S)=mhd_alfven_gamma*mhd_alfven_adiab*wroe(ix^S,rho_)**(mhd_alfven_gamma-one)
    end if
  ...
  ! bdv,bdb,dpres and dv variables. "smalla" contains a lower limit for "a" to be
  ...
  subroutine mhd_alfven_get_eigenjump(wL,wR,wroe,x,ix^L,il,idim,smalla,a,jump,workroe)
    use mod_global_parameters
  ...
  end subroutine mhd_alfven_get_eigenjump
  ...
  subroutine geteigenjump2(wL,wR,wroe,x,ix^L,il,idim,smalla,a,jump, &
       cfast,cslow,afast,aslow,csound2,dpres,rhodv,bdv,bdb,cs2L,cs2R,cs2ca2L,cs2ca2R)
    use mod_global_parameters
    use mod_tvd
    ...
    double precision, intent(in)          :: x(ixG^T,1:ndim)
    double precision, dimension(ixG^T)    :: smalla,a,jump
    double precision, dimension(ixG^T)    :: cfast,cslow,afast,aslow,csound2,dpres,rhodv
    ...
    case(fastRW_)
       ...
       jump(ix^S)=half/csound2(ix^S)*(&
            afast(ix^S)*(+cfast(ix^S)*rhodv(ix^S)+dpres(ix^S))&
    ...
    case(slowRW_)
       ...
       jump(ix^S)=half/csound2(ix^S)*(&
            aslow(ix^S)*(+cslow(ix^S)*rhodv(ix^S)+dpres(ix^S))&
    ...
    case(entroW_)
       ...
       jump(ix^S)=wR(ix^S,rho_)-wL(ix^S,rho_)-dpres(ix^S)/csound2(ix^S)
    case(diverW_)
       if(divbwave)then
          a(ix^S)=wroe(ix^S,mom(idim))
    ...
    case('harten','powell', 'ratio')
       ...
       case(fastRW_)
          ...
          call mhd_alfven_get_csound2(wL,x,ixG^LL,ix^L,cs2L)
          call mhd_alfven_get_csound2(wR,x,ixG^LL,ix^L,cs2R)
  ...
  subroutine mhd_alfven_rtimes(q,w,ix^L,iw,il,idim,rq,workroe)
    use mod_global_parameters
  ...
  end subroutine mhd_alfven_rtimes

  ! Multiply q by R(il,iw), where R is the right eigenvalue matrix at wroe
  subroutine rtimes2(q,wroe,ix^L,iw,il,idim,rq, &
       cfast,cslow,afast,aslow,csound2,dpres,rhodv,bv,v2a2)
    use mod_global_parameters
    ...
    if(iw == rho_) then
      ...
      if(il==fastRW_)then
        ...
        v2a2(ix^S)=half*sum(wroe(ix^S,mom(:))**2,dim=ndim+1)+ &
             (two-mhd_alfven_gamma)/(mhd_alfven_gamma-one)*csound2(ix^S)
