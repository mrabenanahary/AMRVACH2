...
double precision :: epsilon, threshold, wtol(1:nw), xtol(1:ndim)
double precision, dimension(ixM^T) :: numerator, denominator, error
double precision, dimension(ixG^T) :: dpres, dm, dref, tmp1
logical, dimension(ixG^T) :: refineflag, coarsenflag
!-----------------------------------------------------------------------------
epsilon=1.0d-6
level=node(plevel_,igrid)
...
do iflag=1,nw+1
   ...
   do idims=1,ndim
      ...
      if (iflag<=nw) then
        if (logflag(iflag)) then
          dpres(ix^S)=dlog10(pw(igrid)%w(jx^S,iflag))-dlog10(pw(igrid)%w(ix^S,iflag))
          ...
          dpres(ix^S)=pw(igrid)%w(jx^S,iflag)-pw(igrid)%w(ix^S,iflag)
      ...
      numerator(ixM^T)=numerator+(dpres(ixM^T)-dm(ixM^T))**2.0d0

      denominator(ixM^T)=denominator &
           + (dabs(dpres(ixM^T)) + dabs(dm(ixM^T)) + amr_wavefilter(level)*dref(ixM^T))**2.0d0
