...
module mod_chemical
  ...
  type chemical_coef_reaction
    ...
    real(kind=dp),allocatable :: rate_toward(:)
    real(kind=dp),allocatable :: rate_inward(:)
...
contains
  ...
  subroutine chemical_elements_init(thechemical_inuse)
    ...
    !-------------------------------------------------------------------
  cond_network :  if(thechemical_inuse%myconfig%network ==1)then
    Loop_i_chemical0 : do i_element=1,thechemical_inuse%myconfig%n_species
      ...
      thechemical_inuse%element(i_element)%ionmax=&
          merge(thechemical_inuse%element(i_element)%ionmax  , &
                thechemical_inuse%element(i_element)%ionmax-1, &
                i_element>1)
  ...
  elseif(thechemical_inuse%myconfig%network ==2) then  cond_network

      Loop_i_chemical_1 : do i_element=1,thechemical_inuse%myconfig%n_species
      if(i_element==thechemical_inuse%myconfig%H_)then
        thechemical_inuse%element(i_element)%ionmin=0
        thechemical_inuse%element(i_element)%ionmax=1
        thechemical_inuse%element(i_element)%elem_name='H'
      elseif(i_element==thechemical_inuse%myconfig%H2_)then
        ...
        thechemical_inuse%element(i_element)%ionmax=4
        thechemical_inuse%element(i_element)%elem_name='H2'
      elseif(i_element==thechemical_inuse%myconfig%frelec_)then
        thechemical_inuse%element(i_element)%ionmin=0
        thechemical_inuse%element(i_element)%ionmax=0
        thechemical_inuse%element(i_element)%elem_name='frelec'
      else
        write(*,*)' This element : ',i_element,' is not implimented'
        call mpistop('Stop at chemical_elements_init in mod_chemical.t')
      end if
    ...
    Loop_ionisation_1 :     do i_ion =thechemical_inuse%element(i_element)%ionmin,I_ionmax-1
       if(i_element==thechemical_inuse%myconfig%H_)then
        thechemical_inuse%element(i_element)%ion(0)%myreccoef%rate_ion(1)= &
                      1.00978d-9  * T300**(0.5d0)  * exp(-1.578d5/T)

        thechemical_inuse%element(i_element)%ion(1)%myreccoef%rate_capture(1)= &
                      4.074746d-12* T300**(-0.79d0)* exp(-0.d0   /T)




        thechemical_inuse%element(H_)%ion(0)%asso_elem(H2_)%myreccoef%rate_gain(h2_1toh_) = &
                  5.98d-18
        thechemical_inuse%element(H_)%ion(0)%asso_elem(H2_)%myreccoef%rate_gain(h2_2toh_) = &
                  1.3e-18
        thechemical_inuse%element(H_)%ion(0)%asso_elem(H2_)%myreccoef%rate_gain(h2h2toh_) = &
                  1.d-8 * dexp(-8.41d4 /T)
        thechemical_inuse%element(H_)%ion(0)%asso_elem(H2_)%myreccoef%rate_gain(h2htoh_) = &
                  4.67d-7* T300**(-1.0_dp)  * exp(-5.5d4  /T)
        thechemical_inuse%element(H_)%ion(0)%asso_elem(H2_)%myreccoef%rate_gain(h2ietoh_) = &
                  3.22d-9     * T300**(0.35d0) * exp(-1.02d5 /T)

        if (T < 1d3) then
          thechemical_inuse%element(H_)%ion(0)%asso_elem(Htotal_)%myreccoef%rate_lose(hhtotal_) = &
             3.22d-9     * T300**(0.35d0) * exp(-1.02d5 /T)
        else
          thechemical_inuse%element(H_)%ion(0)%asso_elem(Htotal_)%myreccoef%rate_lose(hhtotal_) = &
             0.0_dp
     ...
     end do Loop_ionisation_1
    end do Loop_i_chemical_1
