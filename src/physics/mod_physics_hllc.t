module mod_physics_hllc
  ...
  abstract interface
     ...
     subroutine sub_get_wCD(x,wLC,wRC,whll,fRC,fLC,Fhll,patchf,lambdaCD,cmin,cmax,&
          ixI^L,ixO^L,idims,f)
       use mod_global_parameters
       integer, intent(in)                                     :: ixI^L,ixO^L,idims
       double precision, dimension(ixI^S,1:nw), intent(in)     :: wRC,wLC
...
contains
  ...
  subroutine dummy_get_wCD(x,wLC,wRC,whll,fRC,fLC,Fhll,patchf,lambdaCD,cmin,cmax,&
       ixI^L,ixO^L,idims,f)
    use mod_global_parameters
    integer, intent(in)                                     :: ixI^L,ixO^L,idims
    double precision, dimension(ixI^S,1:nw), intent(in)     :: wRC,wLC
