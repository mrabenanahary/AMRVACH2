...
contains
    ...
    do n=1,num_particles
      ...
      if(ipe_particle == mype) then
        ...
        if (.not. associated(usr_update_payload)) then
          call advect_update_payload(igrid,pw(igrid)%w,pw(igrid)%wold,&
             pw(igrid)%x,x(:,n),payload,npayload,0.d0)
        else
          call usr_update_payload(igrid,pw(igrid)%w,pw(igrid)%wold,pw(igrid)%x,&
             x(:,n),payload,npayload,0.d0)
        end if
  ...
  subroutine advect_fill_gridvars
    ...
    do iigrid=1,igridstail; igrid=igrids(iigrid);
      ...
      w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nw) = pw(igrid)%w(ixGlo1:ixGhi1,&
         ixGlo2:ixGhi2,1:nw)
      ...
      if(time_advance) then
        ...
        w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nw) = pw(igrid)%wold(ixGlo1:ixGhi1,&
           ixGlo2:ixGhi2,1:nw)
        call phys_to_primitive(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixGlo1,ixGlo2,&
           ixGhi1,ixGhi2,w,pw(igrid)%x)
  ...
  subroutine advect_integrate_particles(end_time)
    ...
    do iipart=1,nparticles_active_on_mype
      ...
      if (.not. associated(usr_update_payload)) then
        call advect_update_payload(igrid,pw(igrid)%w,pw(igrid)%wold,&
           pw(igrid)%x,x,payload,npayload,tlocnew)
      else
        call usr_update_payload(igrid,pw(igrid)%w,pw(igrid)%wold,pw(igrid)%x,x,&
           payload,npayload,tlocnew)
      end if
  ...
  subroutine advect_update_payload(igrid,w,wold,xgrid,xpart,payload,npayload,&
     particle_time)
    use mod_global_parameters
    ...
    if (.not.time_advance) then
      call interpolate_var(igrid,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
         ixMhi1,ixMhi2,w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,iw_rho),xgrid,xpart,rho)
    else
      call interpolate_var(igrid,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
         ixMhi1,ixMhi2,wold(ixGlo1:ixGhi1,ixGlo2:ixGhi2,iw_rho),xgrid,xpart,&
         rho1)
      call interpolate_var(igrid,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
         ixMhi1,ixMhi2,w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,iw_rho),xgrid,xpart,rho2)
  ...
  pure function advect_get_particle_dt(partp, end_time) result(dt_p)
    ...
    if(typeaxial =='cylindrical'.and.phi_>0) v(phi_) = &
    ...
    if(.not.time_advance) then
      do ivar=ibeg,iend
        ...
        call interpolate_var(igrid,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
           ...
           pw(igrid)%x(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:ndim),x,var(iloc))
      end do
    else
      ...
      do ivar=ibeg,iend
        ...
        call interpolate_var(igrid,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
           ...
           ivar),pw(igrid)%x(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:ndim),x,e1(iloc))
        call interpolate_var(igrid,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
           ixMhi1,ixMhi2,gridvars(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ivar),&
           pw(igrid)%x(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:ndim),x,e2(iloc))
