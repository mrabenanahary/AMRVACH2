...
111   close(unitpar)
  ...
  subroutine dust_check_params()
    ...
    if (any(dust_size < 0.0d0)) call mpistop(&
       "Dust error: any(dust_size < 0) is not set")
  ...
  subroutine dust_get_flux(w, x, ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,&
    ...
    real(kind=dp), intent(inout) :: f(ixImin1:ixImax1,ixImin2:ixImax2, nwflux)
    integer                         :: n, idir
  ...
  function get_vdust(w, ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,ixOmin2,&
     ixOmax1,ixOmax2, idim, idust) result(vdust)
    use mod_global_parameters, only: nw
    integer, intent(in)           :: ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,&
       ixOmin2,ixOmax1,ixOmax2, idim, idust
    real(kind=dp), intent(in)  :: w(ixImin1:ixImax1,ixImin2:ixImax2, nw)
    real(kind=dp)              :: vdust(ixOmin1:ixOmax1,ixOmin2:ixOmax2)

    where (w(ixOmin1:ixOmax1,ixOmin2:ixOmax2, dust_rho(idust)) > dust_min_rho)
      vdust = w(ixOmin1:ixOmax1,ixOmin2:ixOmax2, dust_mom(idim,&
          idust)) / w(ixOmin1:ixOmax1,ixOmin2:ixOmax2, dust_rho(idust))
    elsewhere
      vdust                       = 0.0d0
    end where
  ...
  subroutine set_dusttozero(qdt, ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,&
    ...
    call dust_check_w(.false., ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,&
       ixOmin2,ixOmax1,ixOmax2, flag, w)
    call dust_set_floor(.false.,ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,&
       ixOmin2,ixOmax1,ixOmax2,flag,x,w)
    ! do n = 1, dust_n_species
  ...
  subroutine get_3d_dragforce(ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,ixOmin2,&
     ixOmax1,ixOmax2, w, x, fdrag, ptherm, vgas,fd_flag)
    use mod_global_parameters
    ...
    real(kind=dp), intent(in)       :: w(ixImin1:ixImax1,ixImin2:ixImax2,&
        1:nw)
    real(kind=dp), intent(out)      :: fdrag(ixImin1:ixImax1,ixImin2:ixImax2,&
        1:ndir, 1:dust_n_species)
    real(kind=dp), intent(in)       :: ptherm(ixImin1:ixImax1,ixImin2:ixImax2),&
        vgas(ixImin1:ixImax1,ixImin2:ixImax2, ndir)
    logical, intent(out)            :: fd_flag(ixImin1:ixImax1,&
       ixImin2:ixImax2)
    real(kind=dp), dimension(ixImin1:ixImax1,ixImin2:ixImax2) :: vt2, deltav,&
        fd, vdust
    real(kind=dp)                   :: alpha_T(ixImin1:ixImax1,ixImin2:ixImax2,&
        1:dust_n_species)
          ...
          where(w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,&
            ...
            deltav(ixOmin1:ixOmax1,ixOmin2:ixOmax2) = (vgas(ixOmin1:ixOmax1,&
             ...
             fd(ixOmin1:ixOmax1,ixOmin2:ixOmax2)     = &
                0.75d0*w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,&
    ...
    !                                                     fd,fd_flag,w)
         ...
         !  end if   test_todelete
          ...
          ! testttt
          if(it<dust_it_diff)fd(ixOmin1:ixOmax1,&
             ixOmin2:ixOmax2) = coef*fd(ixOmin1:ixOmax1,ixOmin2:ixOmax2)
  ...
  subroutine get_sticking(w, x, ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,&
    ...
    real(kind=dp), intent(in)  :: x(ixImin1:ixImax1,ixImin2:ixImax2, 1:ndim)
    real(kind=dp), intent(in)  :: w(ixImin1:ixImax1,ixImin2:ixImax2, 1:nw)
    real(kind=dp), intent(out) :: alpha_T(ixImin1:ixImax1,ixImin2:ixImax2,&
        1:dust_n_species)
    real(kind=dp), intent(in)  :: ptherm(ixImin1:ixImax1,ixImin2:ixImax2)
    real(kind=dp)              :: Tgas(ixImin1:ixImax1,ixImin2:ixImax2)
    integer                       :: n
  ...
  subroutine get_tdust(w, x, ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,ixOmin2,&
    ...
    real(kind=dp), intent(in)  :: x(ixImin1:ixImax1,ixImin2:ixImax2, 1:ndim)
    real(kind=dp), intent(in)  :: w(ixImin1:ixImax1,ixImin2:ixImax2, 1:nw)
    real(kind=dp), intent(out) :: Td(ixImin1:ixImax1,ixImin2:ixImax2,&
        1:dust_n_species)
    real(kind=dp)              :: G0(ixOmin1:ixOmax1,ixOmin2:ixOmax2)
    integer                       :: n
  ...
  subroutine dust_add_source(qdt, ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,&
    ...
    ! .. local ..
    logical          :: fd_flag(ixImin1:ixImax1,ixImin2:ixImax2)
    real(kind=dp)    :: ptherm(ixImin1:ixImax1,ixImin2:ixImax2),&
        vgas(ixImin1:ixImax1,ixImin2:ixImax2, ndir),vdust(ixImin1:ixImax1,&
       ixImin2:ixImax2),dv(ixImin1:ixImax1,ixImin2:ixImax2)
    real(kind=dp)    :: fdrag(ixImin1:ixImax1,ixImin2:ixImax2, ndir,&
        dust_n_species)
    ...
    case default !all regular dust methods here
      if (qsourcesplit .eqv. dust_source_split) then
        ...
        call get_3d_dragforce(ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,ixOmin2,&
           ixOmax1,ixOmax2, wCT, x, fdrag, ptherm, vgas,fd_flag)

        fdrag(ixOmin1:ixOmax1,ixOmin2:ixOmax2,:, :) = fdrag(ixOmin1:ixOmax1,&
           ixOmin2:ixOmax2,:, :) * qdt
          ...
          do idust = 1, dust_n_species
            ...
            if (gas_e_ > 0) then
              w(ixOmin1:ixOmax1,ixOmin2:ixOmax2, gas_e_) = w(ixOmin1:ixOmax1,&
                 ...
                 ixOmin2:ixOmax2, idir, idust)
            end if

            w(ixOmin1:ixOmax1,ixOmin2:ixOmax2, dust_mom(idir,&
                idust)) = w(ixOmin1:ixOmax1,ixOmin2:ixOmax2, dust_mom(idir,&
                idust)) - fdrag(ixOmin1:ixOmax1,ixOmin2:ixOmax2, idir, idust)
          end do
        end do
        ! it_diff=2000000
        ! test_todelete : if(it<=it_diff)then
  ...
  subroutine dust_get_dt(w, ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,ixOmin2,&
    ...
    real(kind=dp)                             :: vgas(ixImin1:ixImax1,&
       ixImin2:ixImax2, ndir)
    real(kind=dp), dimension(1:dust_n_species):: dtdust
    real(kind=dp), dimension(ixImin1:ixImax1,ixImin2:ixImax2)           :: vt2,&
        deltav, tstop, vdust,ptherm
    real(kind=dp), dimension(ixImin1:ixImax1,ixImin2:ixImax2,&
  ...
  subroutine dust_check_w(primitive, ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,&
     ixOmin2,ixOmax1,ixOmax2, flag, w)
    use mod_global_parameters
    implicit none
    ...
    Loop_dust :  do idust=1,dust_n_species
      where(w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,dust_rho(idust))<=dust_min_rho)
        flag(ixOmin1:ixOmax1,ixOmin2:ixOmax2) = - dust_rho(idust)
      end where
    end do Loop_dust
  end subroutine dust_check_w
  !--------------------------------------------------------------------
  !> subroutine to handel small values
  subroutine dust_set_floor(primitive,ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,&
     ixOmin2,ixOmax1,ixOmax2,flag,x,w)
    use mod_global_parameters
    implicit none
    logical, intent(in)       :: primitive
    ...
    ! .. local ..
    integer                   :: idust,idir
    logical, dimension(ixImin1:ixImax1,ixImin2:ixImax2) :: patch_correct
    !------------------------------------
    Loop_dust :  do idust=1,dust_n_species
      patch_correct(ixOmin1:ixOmax1,ixOmin2:ixOmax2) = (flag(ixOmin1:ixOmax1,&
         ixOmin2:ixOmax2)==-dust_rho(idust)).or.(flag(ixOmin1:ixOmax1,&
         ixOmin2:ixOmax2)>0)
      where(patch_correct(ixOmin1:ixOmax1,ixOmin2:ixOmax2))
        w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,dust_rho(idust)) = 0.0_dp
      end where
      Loop_idir :  do idir=1,ndir
        where(patch_correct(ixOmin1:ixOmax1,ixOmin2:ixOmax2))
          w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,dust_mom(idir,idust)) = 0.0_dp
        end where
      end do Loop_idir
    end do Loop_dust
  ...
  subroutine dust_mean_dragforce(ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,&
     ixOmin2,ixOmax1,ixOmax2,forcedrag,mean_force,mean_flag)
    use mod_global_parameters
    ...
    logical, intent(out)            :: mean_flag(ixImin1:ixImax1,&
       ixImin2:ixImax2)
    ! .. local ..
    integer                         :: ix1,ix2,kxOmin1,kxOmin2,kxOmax1,kxOmax2
    ...
    do ix1= ixOmin1,ixOmax1
      ...
      kxOmin2= max(ix2-small_values_daverage, ixOmin2);
      kxOmax2= min(ix2+small_values_daverage, ixOmax2);
      SUM_force=sum(forcedrag(kxOmin1:kxOmax1,kxOmin2:kxOmax2))
      mean_force(ix1,ix2)=(sum_force-forcedrag(ix1,&
         ix2))/((kxOmax1-kxOmin1+1)*(kxOmax2-kxOmin2+1)-1)
      if(dabs(forcedrag(ix1,ix2))>smalldouble.and.dabs(mean_force(ix1,&
         ix2))>smalldouble)then
          mean_flag(ix1,ix2)=dabs((forcedrag(ix1,ix2)-mean_force(ix1,&
             ix2))/mean_force(ix1,ix2))<dust_max_s_dragforce
      else
          mean_flag(ix1,ix2)=.true.
      end if
  ...
  subroutine dust_handle_largevariation_dragforce(primitive,ixImin1,ixImin2,&
     ixImax1,ixImax2,ixOmin1,ixOmin2,ixOmax1,ixOmax2,forcedrag,mean_flag,w)
   use mod_global_parameters
   implicit none
   integer, intent(in)             :: ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,&
      ixOmin2,ixOmax1,ixOmax2
   ...
   !----------------------
   call dust_mean_dragforce(ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,ixOmin2,&
      ixOmax1,ixOmax2,forcedrag,mean_force,mean_flag)
   !call dust_average_drag_force(ixI^L,ixO^L,mean_flag,w)

   where(.not.mean_flag(ixOmin1:ixOmax1,ixOmin2:ixOmax2))
    forcedrag(ixOmin1:ixOmax1,ixOmin2:ixOmax2) = mean_force(ixOmin1:ixOmax1,&
       ixOmin2:ixOmax2)
   end where
  ...
  !------------------------------------------------------------------------------------------
  !> subroutine to handel large variation in drag force
  subroutine dust_average_drag_force(ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,&
     ixOmin2,ixOmax1,ixOmax2,idust,w_flag,w)

      use mod_global_parameters
      use mod_small_values
    integer, intent(in)             :: ixImin1,ixImin2,ixImax1,ixImax2,&
        ixOmin1,ixOmin2,ixOmax1,ixOmax2,idust
    ...
    if (.not.w_flag(ix1,ix2)) then
      ...
      do i = 1, max(small_values_daverage, 1)
        ...
        if (any(w_flag(kxOmin1:kxOmax1,kxOmin2:kxOmax2))) exit
      end do

      if (any(w_flag(kxOmin1:kxOmax1,kxOmin2:kxOmax2))) then
        ...
        do ivar = 1,ndir+1
           ...
           print*,' before here  ',w(ix1,ix2, iw),w(ix1,ix2,ivar)-w(ix1,ix2,iw)
          w(ix1,ix2, iw) = sum(w(kxOmin1:kxOmax1,kxOmin2:kxOmax2, iw),&
              w_flag(kxOmin1:kxOmax1,kxOmin2:kxOmax2))/ &
             count(w_flag(kxOmin1:kxOmax1,kxOmin2:kxOmax2))
         print*,' this values ',w(kxOmin1:kxOmax1,kxOmin2:kxOmax2,iw),iw,  w(ix1,ix2, iw)&
                                ,w(ix1,ix2,ivar)-w(ix1,ix2,iw)
        end do
  ...
  !------------------------------------------------------------------------------------------
  !> subroutine to handel large variation in drag force
  subroutine dust_average_dspeed(ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,&
     ixOmin2,ixOmax1,ixOmax2,idust,w_flag,w)
        ...
        do ivar = 1,ndim+1
           if(ivar==1) then
             ...
             w(ix1,ix2, iw) = sum(w(kxOmin1:kxOmax1,kxOmin2:kxOmax2, iw),&
                ...
                kxOmin2:kxOmax2))/ count(w_flag(kxOmin1:kxOmax1,&
                kxOmin2:kxOmax2))
           else

             iw =  dust_mom(ivar-1,idust)
             if(dabs(w(ix1,ix2, iw))>smalldouble)then
              w(ix1,ix2, iw) = sum(w(kxOmin1:kxOmax1,kxOmin2:kxOmax2,&
                  iw)-w(kxOmin1:kxOmax1,kxOmin2:kxOmax2,gas_mom(ivar-1)),&
                 mask=w_flag(kxOmin1:kxOmax1,&
                 kxOmin2:kxOmax2))/ count(w_flag(kxOmin1:kxOmax1,&
                 kxOmin2:kxOmax2))+w(ix1,ix2, gas_mom(ivar-1))
             end if
           end if
    ...
    !------------------------------------------------------------------------------------------
    !> subroutine to handel large variation in drag force
    subroutine dust_average_dustdensity(ixImin1,ixImin2,ixImax1,ixImax2,&
       ixOmin1,ixOmin2,ixOmax1,ixOmax2,idust,w_flag,w)
      ...
      real(kind=dp), intent(inout)    :: w(ixImin1:ixImax1,ixImin2:ixImax2,&
          1:nw)
      integer                         :: kxOmin1,kxOmin2,kxOmax1,kxOmax2, ix1,&
         ix2
      ...
      if (.not.w_flag(ix1,ix2)) then
        ...
        if (any(w_flag(kxOmin1:kxOmax1,kxOmin2:kxOmax2))) then
          ...
          ! only average those which were ok and replace faulty cells
               ...
               if(dabs(w(ix1,ix2, dust_rho(idust)))>smalldouble)then
                w(ix1,ix2, dust_rho(idust)) = sum(w(kxOmin1:kxOmax1,&
                   kxOmin2:kxOmax2, dust_rho(idust))*w(kxOmin1:kxOmax1,&
                   kxOmin2:kxOmax2,gas_rho_),mask=w_flag(kxOmin1:kxOmax1,&
                   kxOmin2:kxOmax2))/ count(w_flag(kxOmin1:kxOmax1,&
                   kxOmin2:kxOmax2))/w(ix1,ix2, gas_rho_)
               end if
