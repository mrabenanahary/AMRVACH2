...
contains
  ...
  subroutine HC_integrate_particles(end_time)
    ...
    do iipart=1,nparticles_active_on_mype
      ...
      case('slab')
        ! HC mover
        uhalf(1:ndir) = particle(ipart)%self%u(1:ndir) + q * dt_p /(2.0d0 * m &
           * const_c) * e(1:ndir)
        ...
        ustar = sum(uhalf(:)*tau(:))
        lfacprime = sqrt((sigma + sqrt(sigma**2 + 4.d0 * (sum(tau(:)*tau(:)) + &
           sum(ustar(:)*ustar(:))))) / 2.d0)

        t(1:ndir) = tau(1:ndir) / lfacprime
        sscal = 1.d0 / (1.d0 + sum(t(:)*t(:)))
        call cross(uhalf,t,tmp)
        uprime(1:ndir) = sscal * (uhalf(1:ndir) + sum(uhalf(:)*t(:)) * &
           t(1:ndir) + tmp(1:ndir))
        call cross(uprime,t,tmp)
        particle(ipart)%self%u(1:ndir) = uprime(1:ndir) + q * dt_p /(2.0d0 * m &
           * const_c) * e(1:ndir) + tmp(1:ndir)
      case default
