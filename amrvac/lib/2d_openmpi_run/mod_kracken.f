...
contains
  ...
  ! @(#) given keyword, fetch single logical value from the language dictionary (zero on error)
     ...
     elseif(value(:2)==".F")then
        ...
        call send_message("*lget* bad value for logical for "//keyword(&
           :len_trim(keyword)))
        lget=.false.
     endif
  ...
  !     optionally needed if you are going to allow multiple commands on a line
        ...
        if((currnt=="-".and.prev==" ".and.delmt == &
           "off".and.index("0123456789.",forwrd) == 0).or.ipoint > islen)then
           ...
           if(ii > 0)then
              ...
              do i10=1,ic
                 if(name(:ii)  ==  ix2(i10)(:ii))then
                    call send_message(" -"//ix2(i10)(ii+&
                       1:len_trim(ix2(i10)))//" "//values(i10)(:ivalue(i10)))
                 endif
              enddo
           endif
           return
