...
111    close(unitpar)
  ...
  subroutine hd_angmomfix(fC,x,wnew,ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,&
    ...
    inv_volume = 1.0_dp/block%dvolume(ixOmin1:ixOmax1,ixOmin2:ixOmax2)
  ...
  subroutine hd_handle_small_values(primitive, w, x, ixImin1,ixImin2,ixImax1,&
    ...
    if (any(flag(ixOmin1:ixOmax1,ixOmin2:ixOmax2) /= 0)) then
      ...
      case ("replace")
        call hd_small_values_floor(primitive, w, x, ixImin1,ixImin2,ixImax1,&
      ...
      case ("average")
        ...
        if(small_values_force_floor.and.ierror/=0)then
         ...
         cond_dust : if(hd_dust)then
          cond_dust_error : if(any(flag(ixOmin1:ixOmax1,&
             ixOmin2:ixOmax2)<0)) then
           call dust_set_floor(primitive,ixImin1,ixImin2,ixImax1,ixImax2,&
              ixOmin1,ixOmin2,ixOmax1,ixOmax2,flag,x,w)
  ...
  subroutine hd_small_values_floor(primitive, w, x, ixImin1,ixImin2,ixImax1,&
     ixImax2, ixOmin1,ixOmin2,ixOmax1,ixOmax2, subname,flag)
    use mod_global_parameters
    use mod_small_values
    use mod_dust, only : dust_set_floor
    ...
    real(dp)                        :: smallone
    integer                         :: idir
     if(all(flag(ixOmin1:ixOmax1,ixOmin2:ixOmax2) <= 0)) return
