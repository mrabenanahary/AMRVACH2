...
'---------------------------------------------------------------------------'
  ...
  subroutine read_par_files()
    ...
    namelist /methodlist/ time_integrator, source_split_usr,typesourcesplit,&
       ...
       entropycoef,typeaverage, typegrad,typediv,typecurl,nxdiffusehllc,&
        flathllc, tvdlfeps,flatcd,flatsh,small_temperature,small_pressure,&
       small_density, small_values_method, small_values_daverage,&
        check_small_values, small_values_force_floor, solve_internal_e,&
        angmomfix
    ...
    namelist /meshlist/ refine_max_level,nbufferx1,nbufferx2,refine_threshold,&
       derefine_ratio, refine_criterion, stretched_grid, stretched_dim,&
        stretched_symm_dim, qstretch_baselevel, nstretchedblocks_baselevel,&
        amr_wavefilter,max_blocks,block_nx1,block_nx2,domain_nx1,domain_nx2,&
       iprob,xprobmin1,xprobmin2,xprobmax1,xprobmax2, w_refine_weight,&
        prolongprimitive,coarsenprimitive, typeprolonglimit, logflag,tfixgrid,&
       itfixgrid,ditregrid
    namelist /paramlist/  courantpar, dtpar, dtdiffpar, typecourant, slowsteps,&
       small_getdt_average,small_dt_coef
...
107    close(unitpar)
    ...
    select case (typeaxial)
      case("cartesian")
       xprobmin1=xprobmin1/unit_length;xprobmax1=xprobmax1/unit_length
       xprobmin2=xprobmin2/unit_length;xprobmax2=xprobmax2/unit_length;
      case ("spherical")
    ...
    box_limit(1,1)=xprobmin1;box_limit(1,2)=xprobmin2;
    box_limit(2,1)=xprobmax1;box_limit(2,2)=xprobmax2;
    ...
    if (any([ domain_nx1/block_nx1,domain_nx2/block_nx2 ] == 1) .and. mype == &
     ...
     if(mod(domain_nx1,block_nx1)/=0) then
       write(*,*) 'in the direction ', 1, 'the number of cells in domaine is' ,&
          domain_nx1
       write(*,*) 'in the direction ', 1, 'the number of cells in block is' ,&
          block_nx1
       call mpistop&
          ('Grid (domain_nx^D) and block (block_nx^D) must be consistent')
     end if
     
      if(mod(domain_nx2,block_nx2)/=0) then
       write(*,*) 'in the direction ', 2, 'the number of cells in domaine is' ,&
          domain_nx2
       write(*,*) 'in the direction ', 2, 'the number of cells in block is' ,&
          block_nx2
       call mpistop&
          ('Grid (domain_nx^D) and block (block_nx^D) must be consistent')
     end if
    ...
    if(stretched_grid) then
       ...
       if(.not.any(stretched_dim(:)) .and. .not.any(stretched_symm_dim(:))) &
          then
...
'will try default value for qstretch_baselevel in dimension', 2
       ...
       if(stretched_symm_dim(1)) then
           ...
           if(mype==0) then
              write(*,*) 'uniform part of size=',sizeuniformpart1
              write(*,*) 'setting of domain is then=',&
                 2*xstretch1+sizeuniformpart1
              write(*,*) 'versus=',xprobmax1-xprobmin1
           endif
       ...
       if(stretched_symm_dim(2)) then
           ...
           if(mype==0) then
              write(*,*) 'uniform part of size=',sizeuniformpart2
              write(*,*) 'setting of domain is then=',&
                 2*xstretch2+sizeuniformpart2
              write(*,*) 'versus=',xprobmax2-xprobmin2
           endif
    ...
    if (tfixgrid<bigdouble/2.0d0) then
       if(mype==0)write(*,*) 'Warning, at time=',tfixgrid,&
          'the grid will be fixed'
    end if
    if (itfixgrid<biginteger/2) then
       if(mype==0)write(*,*) 'Warning, at iteration=',itfixgrid,&
          'the grid will be fixed'
    end if
    if (ditregrid>1) then
       if(mype==0)write(*,*) 'Note, Grid is reconstructed once every',&
          ditregrid,'iterations'
    end if
