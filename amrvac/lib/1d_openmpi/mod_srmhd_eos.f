module mod_srmhd_eos
 ...
 contains
  ...
  subroutine srmhd_get_enthalpy(ixOmin1,ixOmax1,rho,p,rhoh)
  ! made by Z. MELIANI 14/02/2018
    implicit none
    integer, intent(in)                :: ixOmin1,ixOmax1
    real(kind=dp)   , intent(in)       :: rho(ixOmin1:ixOmax1),&
       p(ixOmin1:ixOmax1)
    real(kind=dp)   , intent(inout)    :: rhoh(ixOmin1:ixOmax1)
    ! .. local ..
    real(kind=dp)   , dimension(ixOmin1:ixOmax1) :: E_th,E
  ...
  subroutine srmhd_get_pressure_primitive_eos(ixImin1,ixImax1,ixOmin1,ixOmax1,&
     rho,rhoh,pth)
  ! made by Z. MELIANI 14/02/2018
    use mod_global_parameters
    implicit none
 ...
 !==========================================================================
  ...
  subroutine srmhd_get_pthermal_eos(ixImin1,ixImax1,ixOmin1,ixOmax1,x,rho,rhoh,&
     e_in,pth)
  ! made by Z. MELIANI 14/02/2018
    use mod_global_parameters
    implicit none
    ...
    integer, intent(in)          :: ixImin1,ixImax1, ixOmin1,ixOmax1
    real(kind=dp)   , intent(in) :: rho(ixOmin1:ixOmax1),rhoh(ixOmin1:ixOmax1),&
       e_in(ixOmin1:ixOmax1)
    ...
    is_notdiab: if(srmhd_energy) then
      ...
      is_einternal : if(block%e_is_internal) then
        is_eos_einter : if(srmhd_eos)then
         pth(ixOmin1:ixOmax1) = 0.5d0*srmhd_gamma_1*(e_in-rho*(rho/e_in))
        else is_eos_einter
         pth(ixOmin1:ixOmax1)=srmhd_gamma_1*e_in
       ...
       is_eos : if(srmhd_eos) then
         ...
         pth(ixOmin1:ixOmax1) = (rhoh - rho)/gamma_to_srmhd_gamma_1
       end if is_eos
      ...
      pth(ixOmin1:ixOmax1)=srmhd_adiab*rho**srmhd_gamma
  ...
  !> csound2=gamm*p/rho
  subroutine srmhd_get_csound2_eos(ixImin1,ixImax1,ixOmin1,ixOmax1,x,rho,rhoh,&
     csound2)
  ! made by Z. MELIANI 14/02/2018
    use mod_global_parameters
    implicit none
    ...
    real(kind=dp)                   :: h(ixOmin1:ixOmax1),E(ixOmin1:ixOmax1),&
       p(ixOmin1:ixOmax1)


    if(srmhd_energy) then
      if(srmhd_eos) then
       E(ixOmin1:ixOmax1)=(rhoh(ixOmin1:ixOmax1)+&
          dsqrt(rhoh(ixOmin1:ixOmax1)**2.0_dp+&
          (srmhd_gamma**2.0_dp-one)*rho**2.0_dp))/(srmhd_gamma+one)

       p=0.5_dp*(E-rho*(rho/E))

       csound2(ixOmin1:ixOmax1)=(p*((srmhd_gamma+one)+&
    ...
    !   p =   (rhoh - rho)/gamma_to_srmhd_gamma_1
       csound2(ixOmin1:ixOmax1)=(1.0_dp - rho/rhoh)*srmhd_gamma_1


      end if
    else
      csound2(ixOmin1:ixOmax1)=srmhd_gamma*srmhd_adiab*rho**srmhd_gamma_1
    end if
  ...
  !> Calculate the square of the thermal sound speed csound2 within ixO^L.
  !> csound2=gamm*p/rho
  subroutine srmhd_get_csound2_prim_eos(ixImin1,ixImax1,ixOmin1,ixOmax1,x,rho,&
     rhoh,p,csound2)
  ! made by Z. MELIANI 14/02/2018
    use mod_global_parameters
    implicit none
    integer, intent(in)             :: ixImin1,ixImax1, ixOmin1,ixOmax1
    real(kind=dp)   , intent(in)    :: rho(ixOmin1:ixOmax1),&
       rhoh(ixOmin1:ixOmax1),p(ixOmin1:ixOmax1)
    real(kind=dp)   , intent(in)    :: x(ixImin1:ixImax1,1:ndim)
    real(kind=dp)   , intent(out)   :: csound2(ixOmin1:ixOmax1)

    real(kind=dp)                   :: h(ixImin1:ixImax1),E(ixImin1:ixImax1)
    if(srmhd_energy) then
      if(srmhd_eos) then
       E(ixOmin1:ixOmax1)=(rhoh(ixOmin1:ixOmax1)+&
          dsqrt(rhoh(ixOmin1:ixOmax1)**2.0d0+&
          (srmhd_gamma**2.0d0-one)*rho**2.0d0))/(srmhd_gamma+one)
       csound2(ixOmin1:ixOmax1)=(p*((srmhd_gamma+one)+&
          srmhd_gamma_1*(rho/E)**2.0d0))/(2.0*rhoh)
      else
       csound2(ixOmin1:ixOmax1)=srmhd_gamma*p/rhoh
      end if
    else
      csound2(ixOmin1:ixOmax1)=srmhd_gamma*srmhd_adiab*rho**srmhd_gamma_1
    end if
