...
contains
 ...
 subroutine small_var_average(ixImin1,ixImax1, ixOmin1,ixOmax1, pth, x,&
     w_flag)
     use mod_global_parameters

    integer, intent(in)             :: ixImin1,ixImax1, ixOmin1,ixOmax1
    integer, intent(in)             :: w_flag(ixImin1:ixImax1)
    double precision, intent(inout) :: pth(ixImin1:ixImax1)
    ...
    if (w_flag(ix1) /= 0) then
      ...
      if (any(w_flag(kxOmin1:kxOmax1) == 0)) then
        ...
        ! only average those which were ok and replace faulty cells
          ...
          pth(ix1) = sum(pth(kxOmin1:kxOmax1),&
  ...
  subroutine small_values_average(ixImin1,ixImax1, ixOmin1,ixOmax1, subname, w,&
      x, w_flag,ierror)
    use mod_global_parameters
    use mod_variables
    integer, intent(in)             :: ixImin1,ixImax1, ixOmin1,ixOmax1
    ...
    ! point with local failure identified by w_flag
    cond_need : if (w_flag(ix1) > 0) then
      ! verify in cube with border width small_values_daverage the presence of
      ! cells where all went ok
      do i = 1, max(small_values_daverage, 1)
        kxOmin1= max(ix1-i, ixOmin1);
      ...
      cond_exist: if (any(w_flag(kxOmin1:kxOmax1) <= 0)) then
        ! within surrounding cube, cells without problem were found
        ...
        write(*,*) "w_flag(ix^D):", w_flag(ix1)
        Loop_iw : do iw = 1,nw
         write(*,*)  prim_wnames(iw)," =", w(ix1, iw)
        end do Loop_iw
        write(*,*)  "variable = ",prim_wnames(abs(w_flag(ix1)))
        write(*,*) 'iteration',it
        write(*,*) "Saving status at the previous time step"
