    ...
    do iipart=1,nparticles_active_on_mype
      ...
      case('slab')
        ...
        uhalf(1:ndir) = particle(ipart)%self%u(1:ndir) + q * dt_p /(2.0d0 * m &
           * const_c) * (e(1:ndir) + tmp(1:ndir)/lfac)

        tau(1:ndir) = q * dt_p / (2.d0 * m * const_c) * b(1:ndir)
        uprime(1:ndir) = uhalf(1:ndir) + q * dt_p / (2.d0 * m * const_c) * &
           e(1:ndir)
        ...
        particle(ipart)%self%u(1:ndir) = sscal * (uprime(1:ndir) + &
           sum(uprime(:)*t(:)) * t(1:ndir) + tmp(1:ndir))
      case default
