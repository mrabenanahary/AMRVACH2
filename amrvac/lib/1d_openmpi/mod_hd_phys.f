...
111    close(unitpar)
  ...
  subroutine hd_angmomfix(fC,x,wnew,ixImin1,ixImax1,ixOmin1,ixOmax1,idim)
    ...
    real(dp), intent(inout)    :: fC(ixImin1:ixImax1,1:nwflux,1:ndim),&
         wnew(ixImin1:ixImax1,1:nw)
    integer, intent(in)                :: ixImin1,ixImax1, ixOmin1,ixOmax1
    integer, intent(in)                :: idim
    integer                            :: hxOmin1,hxOmax1, kxCmin1,kxCmax1, iw
    real(dp)                   :: inv_volume(ixImin1:ixImax1)
    ...
    inv_volume = 1.0_dp/block%dvolume(ixOmin1:ixOmax1)
      ...
      do iw=1,nwflux
        if (idim==r_ .and. iw==iw_mom(phi_)) then
          ...
          wnew(ixOmin1:ixOmax1,iw)=wnew(ixOmin1:ixOmax1,&
             ...
             idim)) * (inv_volume/x(ixOmin1:ixOmax1,idim))
        else
          wnew(ixOmin1:ixOmax1,iw)=wnew(ixOmin1:ixOmax1,&
             iw) + (fC(ixOmin1:ixOmax1,iw,idim)-fC(hxOmin1:hxOmax1,iw,&
             idim)) * inv_volume
    ...
    case ("spherical")
      do iw=1,nwflux
        ...
        elseif (idim==2  .and. iw==iw_mom(phi_)) then
          ...
          wnew(ixOmin1:ixOmax1,iw)=wnew(ixOmin1:ixOmax1,&
             iw) + (fC(ixOmin1:ixOmax1,iw,idim)-fC(hxOmin1:hxOmax1,iw,&
             idim)) * (inv_volume/sin(x(ixOmin1:ixOmax1,idim)))
        else
          wnew(ixOmin1:ixOmax1,iw)=wnew(ixOmin1:ixOmax1,&
             iw) + (fC(ixOmin1:ixOmax1,iw,idim)-fC(hxOmin1:hxOmax1,iw,&
             idim)) * inv_volume
  ...
  subroutine hd_check_w(primitive, ixImin1,ixImax1, ixOmin1,ixOmax1, w, flag)
    ...
    integer, intent(in)          :: ixImin1,ixImax1, ixOmin1,ixOmax1
    real(dp), intent(in)         :: w(ixImin1:ixImax1, nw)
    integer, intent(inout)       :: flag(ixImin1:ixImax1)
    real(dp)                     :: tmp(ixImin1:ixImax1)
    !-----------------------
    flag(ixOmin1:ixOmax1) = 0
!    if(it>559.and.saveigrid==28)print*,' is testt',it,saveigrid,w(ixOmin1:ixOmax1, rho_)
    if(hd_dust)call dust_check_w(primitive, ixImin1,ixImax1, ixOmin1,ixOmax1,&
        flag, w)
    ...
    if (hd_energy) then
       if (primitive) then
         ...
         where(w(ixOmin1:ixOmax1, rho_) > small_density)
  ...
  !> Returns 0 in argument flag where values are ok
  subroutine hd_check_pth(ixImin1,ixImax1, ixOmin1,ixOmax1, pth, flag)
    use mod_global_parameters
    use mod_dust, only : dust_check_w

    integer, intent(in)          :: ixImin1,ixImax1, ixOmin1,ixOmax1
    real(dp), intent(in)         :: pth(ixImin1:ixImax1)
    integer, intent(inout)       :: flag(ixImin1:ixImax1)
    !-----------------------
    flag(ixOmin1:ixOmax1) = 0
    if (hd_energy) then
      where(pth(ixOmin1:ixOmax1) < small_pressure) flag(ixOmin1:ixOmax1) = e_
    end if
  ...
  subroutine hd_to_conserved(ixImin1,ixImax1, ixOmin1,ixOmax1, w, x)
    ...
    integer, intent(in)             :: ixImin1,ixImax1, ixOmin1,ixOmax1
    real(dp), intent(inout) :: w(ixImin1:ixImax1, nw)
    real(dp), intent(in)    :: x(ixImin1:ixImax1, 1:ndim)
    real(dp)                :: invgam
    integer                         :: idir, itr
    ...
    if (check_small_values) call hd_handle_small_values(.false., w, x, ixImin1,&
       ixImax1, ixOmin1,ixOmax1, 'hd_to_conserved')
  ...
  subroutine hd_to_primitive(ixImin1,ixImax1, ixOmin1,ixOmax1, w, x)
    ...
    if (check_small_values) call hd_handle_small_values(.true., w, x, ixImin1,&
       ixImax1, ixOmin1,ixOmax1, 'hd_to_primitive')
  ...
  subroutine hd_get_v(w, x, ixImin1,ixImax1, ixOmin1,ixOmax1, idim, v)
    use mod_global_parameters
    integer, intent(in)           :: ixImin1,ixImax1, ixOmin1,ixOmax1, idim
    real(dp), intent(in)  :: w(ixImin1:ixImax1, nw), x(ixImin1:ixImax1,&
        1:ndim)
    real(dp), intent(out) :: v(ixImin1:ixImax1)
  ...
  subroutine hd_get_cmax(w, x, ixImin1,ixImax1, ixOmin1,ixOmax1, idim, cmax)
    ...
    real(dp), intent(inout)           :: cmax(ixImin1:ixImax1)
    real(dp)                          :: csound(ixImin1:ixImax1)
    real(dp)                          :: v(ixImin1:ixImax1)

    call hd_get_v(w, x, ixImin1,ixImax1, ixOmin1,ixOmax1, idim, v)
    call hd_get_csound2(w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,csound)
    csound(ixOmin1:ixOmax1) = sqrt(csound(ixOmin1:ixOmax1))

    cmax(ixOmin1:ixOmax1) = abs(v(ixOmin1:ixOmax1))+csound(ixOmin1:ixOmax1)
  ...
  subroutine hd_get_cbounds(wLC, wRC, wLp, wRp, x, ixImin1,ixImax1, ixOmin1,&
    ...
    real(dp), intent(in)              :: wLC(ixImin1:ixImax1, nw),&
        wRC(ixImin1:ixImax1, nw)
    ! primitive left and right status
    real(dp), intent(in)              :: wLp(ixImin1:ixImax1, nw),&
        wRp(ixImin1:ixImax1, nw)
    real(dp), intent(in)              :: x(ixImin1:ixImax1, 1:ndim)
    real(dp), intent(inout)           :: cmax(ixImin1:ixImax1)
    real(dp), intent(inout), optional :: cmin(ixImin1:ixImax1)
    ! .. local ..
    real(dp)                          :: wmean(ixImin1:ixImax1,nw)
    real(dp), dimension(ixImin1:ixImax1)        :: umean, dmean, csoundL,&
    ...
    if (typeboundspeed/='cmaxmean') then
      ...
      if(hd_energy) then
        ...
        csoundL(ixOmin1:ixOmax1)=hd_gamma*hd_adiab*wLp(ixOmin1:ixOmax1,&
           rho_)**(hd_gamma-one)
        csoundR(ixOmin1:ixOmax1)=hd_gamma*hd_adiab*wRp(ixOmin1:ixOmax1,&
           rho_)**(hd_gamma-one)
      end if
      ...
      csoundR(ixOmin1:ixOmax1) = sqrt(csoundR(ixOmin1:ixOmax1))
  ...
  subroutine hd_get_csound2(w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,csound2)
    ...
    real(dp), intent(in)    :: w(ixImin1:ixImax1,nw)
    real(dp), intent(in)    :: x(ixImin1:ixImax1,1:ndim)
    real(dp), intent(out)   :: csound2(ixImin1:ixImax1)
    ...
    if(hd_energy) then
      ...
      csound2(ixOmin1:ixOmax1)=hd_gamma*hd_adiab*w(ixOmin1:ixOmax1,&
         rho_)**(hd_gamma-one)
    end if
  ...
  subroutine hd_get_pthermal(w, x, ixImin1,ixImax1, ixOmin1,ixOmax1, pth)
    ...
    real(dp), intent(in)         :: w(ixImin1:ixImax1, nw)
    real(dp), intent(in)         :: x(ixImin1:ixImax1, 1:ndim)
    real(dp), intent(out)        :: pth(ixImin1:ixImax1)
    !----------------------------------------------------
    if (hd_energy) then
  ...
  subroutine hd_get_flux_cons(w, x, ixImin1,ixImax1, ixOmin1,ixOmax1, idim, f)
    ...
    real(dp), intent(in)            :: w(ixImin1:ixImax1, 1:nw),&
        x(ixImin1:ixImax1, 1:ndim)
    real(dp), intent(out)           :: f(ixImin1:ixImax1, nwflux)
    real(dp)                        :: pth(ixImin1:ixImax1),&
        v(ixImin1:ixImax1)
    integer                         :: idir, itr
  ...
  subroutine hd_get_flux(wC, w, x, ixImin1,ixImax1, ixOmin1,ixOmax1, idim, f)
    ...
    integer, intent(in)             :: ixImin1,ixImax1, ixOmin1,ixOmax1, idim
    ! conservative w
    real(dp), intent(in)            :: wC(ixImin1:ixImax1, 1:nw)
    ! primitive w
    real(dp), intent(in)            :: w(ixImin1:ixImax1, 1:nw)
    real(dp), intent(in)            :: x(ixImin1:ixImax1, 1:ndim)
    real(dp), intent(out)           :: f(ixImin1:ixImax1, nwflux)
    real(dp)                        :: pth(ixOmin1:ixOmax1)
    integer                         :: idir, itr
  ...
  subroutine hd_add_source_geom(qdt, ixImin1,ixImax1, ixOmin1,ixOmax1, wCT, w,&
    ...
    integer, intent(in)             :: ixImin1,ixImax1, ixOmin1,ixOmax1
    real(dp), intent(in)    :: qdt, x(ixImin1:ixImax1, 1:ndim)
    real(dp), intent(inout) :: wCT(ixImin1:ixImax1, 1:nw), w(ixImin1:ixImax1,&
        1:nw)
    ...
    real(dp) :: tmp(ixImin1:ixImax1),tmp1(ixImin1:ixImax1)
    ...
    case ("cylindrical")
       ...
       call hd_get_pthermal(wCT,x,ixImin1,ixImax1,ixOmin1,ixOmax1,tmp)
       if(phi_>0) then
         tmp(ixOmin1:ixOmax1)=tmp(ixOmin1:ixOmax1)+wCT(ixOmin1:ixOmax1,&
            mphi_)**2/wCT(ixOmin1:ixOmax1,rho_)
         ...
         if(.not. angmomfix) then
           tmp(ixOmin1:ixOmax1)=-wCT(ixOmin1:ixOmax1,&
              mphi_)*wCT(ixOmin1:ixOmax1,mr_)/wCT(ixOmin1:ixOmax1,rho_)
           w(ixOmin1:ixOmax1,mphi_)=w(ixOmin1:ixOmax1,&
              mphi_)+qdt*tmp(ixOmin1:ixOmax1)/x(ixOmin1:ixOmax1,1)
         end if
       else
    ...
    case ("spherical")
       ...
       call hd_get_pthermal(wCT,x,ixImin1,ixImax1,ixOmin1,ixOmax1,tmp1)
       ...
       if(ndir>1) then
         do idir=2,ndir
           tmp(ixOmin1:ixOmax1)=tmp(ixOmin1:ixOmax1)+wCT(ixOmin1:ixOmax1,&
              mom(idir))**2/wCT(ixOmin1:ixOmax1,rho_)
         end do
       end if
       w(ixOmin1:ixOmax1,mr_)=w(ixOmin1:ixOmax1,&
          mr_)+qdt*tmp(ixOmin1:ixOmax1)/x(ixOmin1:ixOmax1,1)
  ...
  subroutine hd_add_source(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,wCT,w,x,&
    ...
    real(dp), intent(in)    :: wCT(ixImin1:ixImax1, 1:nw), x(ixImin1:ixImax1,&
        1:ndim)
    ...
    if (check_small_values) call hd_handle_small_values(.false., w, x, ixImin1,&
       ixImax1, ixOmin1,ixOmax1, 'hd_add_source')
  ...
  subroutine hd_get_dt(w, ixImin1,ixImax1, ixOmin1,ixOmax1, dtnew, dx1, x)
    ...
    integer, intent(in)             :: ixImin1,ixImax1, ixOmin1,ixOmax1
    real(dp), intent(in)    :: dx1, x(ixImin1:ixImax1, 1:1)
    real(dp), intent(in)    :: w(ixImin1:ixImax1, 1:nw)
  ...
  function hd_kin_en(w, ixImin1,ixImax1, ixOmin1,ixOmax1, inv_rho) result(ke)
    ...
    integer, intent(in)                    :: ixImin1,ixImax1, ixOmin1,ixOmax1
    real(dp), intent(in)           :: w(ixImin1:ixImax1, nw)
    real(dp)                       :: ke(ixOmin1:ixOmax1)
    real(dp), intent(in), optional :: inv_rho(ixOmin1:ixOmax1)
  ...
  subroutine hd_handle_small_values(primitive, w, x, ixImin1,ixImax1, ixOmin1,&
    ...
    if (any(flag(ixOmin1:ixOmax1) /= 0)) then
      ...
      case ("replace")
        call hd_small_values_floor(primitive, w, x, ixImin1,ixImax1, ixOmin1,&
           ixOmax1, subname,flag)
        if(hd_dust)call dust_set_floor(primitive,ixImin1,ixImax1,ixOmin1,&
           ixOmax1,flag,x,w)
      case ("average")
        call small_values_average(ixImin1,ixImax1, ixOmin1,ixOmax1,subname, w,&
            x, flag,ierror)
        ...
        if(small_values_force_floor.and.ierror/=0)then
         ...
         cond_dust : if(hd_dust)then
          cond_dust_error : if(any(flag(ixOmin1:ixOmax1)<0)) then
           call dust_set_floor(primitive,ixImin1,ixImax1,ixOmin1,ixOmax1,flag,&
              x,w)
  ...
  subroutine hd_small_values_floor(primitive, w, x, ixImin1,ixImax1, ixOmin1,&
     ixOmax1, subname,flag)
    use mod_global_parameters
    use mod_small_values
    use mod_dust, only : dust_set_floor
    ...
    real(dp)                        :: smallone
    integer                         :: idir
     if(all(flag(ixOmin1:ixOmax1) <= 0)) return
  ...
  subroutine hd_handle_small_values_pressure(ixImin1,ixImax1,ixOmin1,ixOmax1,x,&
     subname,pth,w)
    use mod_global_parameters
    use mod_small_values
    implicit none
    integer, intent(in)             :: ixImin1,ixImax1,ixOmin1,ixOmax1
    real(dp), intent(in)            :: x(ixImin1:ixImax1, 1:ndim)
    real(dp), intent(inout)         :: pth(ixImin1:ixImax1)
    real(dp), intent(in)            :: w(ixImin1:ixImax1,1:nw)
    character(len=*), intent(in)    :: subname
    !.. local ..
    real(dp)                        :: smallone
    ...
    call hd_check_pth( ixImin1,ixImax1, ixOmin1,ixOmax1, pth, flag)

    if (any(flag(ixOmin1:ixOmax1) /= 0)) then
      select case (small_values_method)
      case ("replace")
        where(flag(ixOmin1:ixOmax1) > 0) pth(ixOmin1:ixOmax1) = small_pressure
      case ("average")
        call small_var_average(ixImin1,ixImax1, ixOmin1,ixOmax1, pth, x, flag)
      case default
        call small_values_error(w, x, ixImin1,ixImax1, ixOmin1,ixOmax1, flag,&
            subname)
      end select
