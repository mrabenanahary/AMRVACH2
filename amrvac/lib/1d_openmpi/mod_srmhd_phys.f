...
contains
  ...
  subroutine srmhd_angmomfix(fC,x,wnew,ixImin1,ixImax1,ixOmin1,ixOmax1,idim)
    use mod_global_parameters
    real(kind=dp)   , intent(in)       :: x(ixImin1:ixImax1,1:ndim)
    real(kind=dp)   , intent(inout)    :: fC(ixImin1:ixImax1,1:nwflux,1:ndim),&
  ...
  subroutine srmhd_check_w(primitive,ixImin1,ixImax1,ixOmin1,ixOmax1,w,flag)
    ! made by Z. Meliani 10/02/2018
    use mod_global_parameters

    logical, intent(in)            :: primitive
    integer, intent(in)            :: ixImin1,ixImax1, ixOmin1,ixOmax1
    real(kind=dp)   , intent(in)   :: w(ixImin1:ixImax1,nw)
    ...
    if(primitive)then
     ...
     where(w(ixOmin1:ixOmax1, d_)/w(ixOmin1:ixOmax1,&
        lfac_) < small_density) flag(ixOmin1:ixOmax1) = rho_
    end if
    cond_energy : if (srmhd_energy) then
       cond_einternal : if (block%e_is_internal) then
          where(w(ixOmin1:ixOmax1, e_) < small_pressure*inv_srmhd_gamma_1) &
             flag(ixOmin1:ixOmax1) = e_
       else cond_einternal
         is_prim : if (primitive)then
           where(w(ixOmin1:ixOmax1, p_) < small_pressure) &
              flag(ixOmin1:ixOmax1) = p_ !p_=e_
         else is_prim
           where(w(ixOmin1:ixOmax1, e_) < small_e) flag(ixOmin1:ixOmax1) = e_
  ...
  subroutine srmhd_to_conserved(ixImin1,ixImax1,ixOmin1,ixOmax1,w,x)
    ! made by Z. Meliani 10/02/2018
    use mod_global_parameters
    implicit none

    integer, intent(in)                :: ixImin1,ixImax1, ixOmin1,ixOmax1
    real(kind=dp)   , intent(inout)    :: w(ixImin1:ixImax1, nw)
    real(kind=dp)   , intent(in)       :: x(ixImin1:ixImax1, 1:ndim)
    ! .. local ..
    integer                            :: idir, itr
    real(kind=dp)   , dimension(ixOmin1:ixOmax1) :: sqrU,B2,VdotB,rhoh,sqrV
    integer, dimension(ixOmin1:ixOmax1)          :: flag_error
    character(len=30)                  :: subname_loc
    !------------------------------------------

    subname_loc= 'srmhd_to_conserved'
    flag_error(ixOmin1:ixOmax1)=0
    where(w(ixOmin1:ixOmax1,rho_)<small_density)flag_error(&
       ixOmin1:ixOmax1)=rho_


    sqrU(ixOmin1:ixOmax1)    = sum(w(ixOmin1:ixOmax1, mom(:))**2, dim=ndim+1)
    w(ixOmin1:ixOmax1,lfac_) = dsqrt(1.0_dp+sqrU(ixOmin1:ixOmax1))
    sqrV=sqrU/w(ixOmin1:ixOmax1,lfac_)**2.0_dp
    ! fill the auxiliary variable xi and density D
    call srmhd_get_enthalpy(ixOmin1,ixOmax1,w(ixOmin1:ixOmax1,rho_),&
       w(ixOmin1:ixOmax1,p_),rhoh)
    ! with enthalpy w: xi= lfac^2 rhoh

    w(ixOmin1:ixOmax1,xi_) = w(ixOmin1:ixOmax1,&
       lfac_)**2.0D0*rhoh(ixOmin1:ixOmax1)


    ! density: d = lfac * rho
    w(ixOmin1:ixOmax1,rho_)=w(ixOmin1:ixOmax1,lfac_)*w(ixOmin1:ixOmax1,rho_)

    ! B2 and VdotB
    call srmhd_get_B2andVdotB(ixImin1,ixImax1,ixOmin1,ixOmax1,w,.false.,B2=B2,&
       VdotB=VdotB)
    ! Convert velocity to momentum
    ! s= (xi + B^2) * v - (v.B) * B
    ! re-use rhoh as rhoh=(xi+B^2)/lfac
    rhoh(ixOmin1:ixOmax1)= (w(ixOmin1:ixOmax1,&
       xi_)+B2(ixOmin1:ixOmax1))/w(ixOmin1:ixOmax1,lfac_)
    Loop_idirmom : do idir = 1, ndir
       w(ixOmin1:ixOmax1, mom(idir)) = rhoh(ixOmin1:ixOmax1) * &
          w(ixOmin1:ixOmax1, mom(idir))-VdotB(ixOmin1:ixOmax1)*w(&
          ixOmin1:ixOmax1,mag(idir))
    end do Loop_idirmom

    cond_energy : if (srmhd_energy) then
       ! re-use sqrU=v^2 B^2 - (v.B)^2
       sqrU(ixOmin1:ixOmax1) = B2(ixOmin1:ixOmax1)*sqrV(ixOmin1:ixOmax1)-&
          VdotB(ixOmin1:ixOmax1)**2.0_dp
       ! sqrU should positive
       where(sqrU(ixOmin1:ixOmax1)<0.0_dp) flag_error(ixOmin1:ixOmax1)=e_
       ! re-use sqrV=xi - p -D
       sqrV(ixOmin1:ixOmax1) = w(ixOmin1:ixOmax1,xi_) - w(ixOmin1:ixOmax1,&
          p_) - w(ixOmin1:ixOmax1,d_)
       where(sqrV(ixOmin1:ixOmax1)<0.0_dp) flag_error(ixOmin1:ixOmax1)=e_
       ! E = xi - p +(B^2+v^2 B^2 - (v.B)^2)/2- D
       w(ixOmin1:ixOmax1,e_)=sqrV(ixOmin1:ixOmax1) &
          +0.5_dp*(B2(ixOmin1:ixOmax1) + sqrU(ixOmin1:ixOmax1))
      !if(.not.block%e_is_internal) w(ixO^S,e_)=w(ixO^S,e_) + x
      if(type_divb==divb_glm2) w(ixOmin1:ixOmax1,e_)=w(ixOmin1:ixOmax1,&
         e_) + 0.5d0*w(ixOmin1:ixOmax1,psi_)**2
    end if cond_energy


    if (check_small_values) call srmhd_handle_small_values(.false., w, x,&
        ixImin1,ixImax1, ixOmin1,ixOmax1,trim(subname_loc),&
  ...
  subroutine srmhd_to_primitive(ixImin1,ixImax1,ixOmin1,ixOmax1,w,x)
    ! made by Z. Meliani 10/02/2018
    use mod_global_parameters
    integer, intent(in)             :: ixImin1,ixImax1, ixOmin1,ixOmax1
    real(kind=dp)   , intent(inout) :: w(ixImin1:ixImax1, nw)
    ...
    real(kind=dp)                   :: inv_rho(ixOmin1:ixOmax1),&
       B2(ixOmin1:ixOmax1),VdotB(ixOmin1:ixOmax1)
    integer                         :: itr, idir
    character(len=30)               :: subname_loc
    !--------------------------------------------------------
    ...
    call srmhd_get_auxiliary(.true.,w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,&
       subname_loc)
    ...
    if (srmhd_energy) then
      ...
      if(.not.block%e_is_internal) then
        call srmhd_get_pressure_fromprimitive(ixImin1,ixImax1,ixOmin1,ixOmax1,&
           w,inv_rho)
        w(ixOmin1:ixOmax1,p_)   = inv_rho(ixOmin1:ixOmax1)
      else
       w(ixOmin1:ixOmax1,p_)   = srmhd_gamma_1*w(ixOmin1:ixOmax1, e_)
      end if
    end if
    ! re-use inv_rho to store inverse of the density
    inv_rho(ixOmin1:ixOmax1) = 1.0_dp / w(ixOmin1:ixOmax1, rho_)

    call srmhd_get_B2andVdotB(ixImin1,ixImax1,ixOmin1,ixOmax1,w,.true.,B2=B2,&
       VdotB=VdotB)
    Loop_idir : do idir=1,ndir
     w(ixOmin1:ixOmax1,mom(idir)) = w(ixOmin1:ixOmax1,&
        lfac_)*(w(ixOmin1:ixOmax1,mom(idir))+VdotB*w(ixOmin1:ixOmax1,&
        mag(idir)))/(w(ixOmin1:ixOmax1,xi_)+B2)
    end do Loop_idir
  ...
  subroutine srmhd_handle_small_values(primitive, w, x, ixImin1,ixImax1,&
      ixOmin1,ixOmax1, subname,flag_error)
    ! made by Z. Meliani 10/02/2018
    use mod_global_parameters
    ...
    integer          :: idir, ierror,flag(ixImin1:ixImax1)
    ! ---------------------------------------------------


    if (small_values_method == "ignore") return
    if(present(flag_error)) then
     flag(ixOmin1:ixOmax1) = flag_error(ixOmin1:ixOmax1)
    else
     call srmhd_check_w(primitive, ixImin1,ixImax1, ixOmin1,ixOmax1, w, flag)
    end if
    ...
    if (any(flag(ixOmin1:ixOmax1) /= 0)) then
       ...
       case ("average")
          call small_values_average(ixImin1,ixImax1, ixOmin1,ixOmax1, subname,&
              w, x, flag,ierror)
          if(small_values_force_floor.and.crash)then
            call srmhd_small_values_floor(primitive, w, x, ixImin1,ixImax1,&
                ixOmin1,ixOmax1, flag)
          end if
       case default
  ...
  subroutine srmhd_small_values_floor(primitive, w, x, ixImin1,ixImax1,&
      ixOmin1,ixOmax1, flag)
    ! made by Z. Meliani 10/02/2018
    use mod_global_parameters
    use mod_small_values
    ...
    real(kind=dp)   , intent(inout) :: w(ixImin1:ixImax1,1:nw)
    real(kind=dp)   , intent(in)    :: x(ixImin1:ixImax1,1:ndim)
    integer, optional, intent(in)   :: flag(ixOmin1:ixOmax1)
    ! .. local ..
    integer                         :: idir
    real(kind=dp)                   :: smallone
    !-------------------------------------------------------------
    where(flag(ixOmin1:ixOmax1) /= 0) w(ixOmin1:ixOmax1,rho_) = small_density
    ...
    if (srmhd_energy) then
      ...
      where(flag(ixOmin1:ixOmax1) /= 0) w(ixOmin1:ixOmax1,e_) = smallone
    end if
  end subroutine srmhd_small_values_floor


 !> Calculate thermal pressure for enthalpy and density
  subroutine srmhd_get_pressure_fromprimitive(ixImin1,ixImax1,ixOmin1,ixOmax1,&
     w,pth)
    ! made by Z. Meliani 10/02/2018
    use mod_global_parameters
    implicit none
    integer, intent(in)            :: ixImin1,ixImax1, ixOmin1,ixOmax1
    real(kind=dp)   , intent(in)   :: w(ixImin1:ixImax1,nw)
    real(kind=dp)   , intent(out)  :: pth(ixOmin1:ixOmax1)

    real(kind=dp)                  :: rhoh(ixOmin1:ixOmax1)
    !--------------------------

    rhoh(ixOmin1:ixOmax1)  = w(ixOmin1:ixOmax1,xi_)/w(ixOmin1:ixOmax1,&
       lfac_)**2.0_dp
    call srmhd_get_pressure_primitive_eos(ixImin1,ixImax1,ixOmin1,ixOmax1,&
       w(ixOmin1:ixOmax1,rho_),rhoh,pth)

  end subroutine srmhd_get_pressure_fromprimitive


  !> Calculate thermal pressure=(gamma-1)*(e-0.5*m**2/rho-b**2/2) within ixO^L
  subroutine srmhd_get_pthermal(w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,pth)
    ! made by Z. Meliani 10/02/2018
    use mod_global_parameters
    implicit none
    ...
    real(kind=dp)                   :: rho(ixOmin1:ixOmax1),&
       rhoh(ixOmin1:ixOmax1)

    rho        = w(ixOmin1:ixOmax1,d_)/w(ixOmin1:ixOmax1,lfac_)
    rhoh       = w(ixOmin1:ixOmax1,xi_)/w(ixOmin1:ixOmax1,lfac_)**2.0d0
    call srmhd_get_pthermal_eos(ixImin1,ixImax1,ixOmin1,ixOmax1,x,rho,rhoh,&
       w(ixOmin1:ixOmax1,e_),pth)
  end subroutine srmhd_get_pthermal


 !> Calculate the square of the thermal sound speed csound2 within ixO^L.
  !> csound2=gamm*p/rho
  subroutine srmhd_get_csound2_prim(w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,&
     csound2)
    ...
    real(kind=dp)   ,dimension(ixOmin1:ixOmax1) :: rhoh


    rhoh=w(ixOmin1:ixOmax1,xi_)/w(ixOmin1:ixOmax1,lfac_)**2.0d0

    call srmhd_get_csound2_prim_eos(ixImin1,ixImax1,ixOmin1,ixOmax1,x,&
       w(ixOmin1:ixOmax1,rho_),rhoh,w(ixOmin1:ixOmax1,p_),csound2)
  ...
  subroutine e_to_rhos(ixImin1,ixImax1,ixOmin1,ixOmax1,w,x)
    ...
    if (srmhd_energy) then
      if(.not.block%e_is_internal) w(ixOmin1:ixOmax1, e_) = w(ixOmin1:ixOmax1,&
          e_) - srmhd_kin_en_primitive(w, ixImin1,ixImax1, ixOmin1,&
         ixOmax1) - srmhd_mag_en_primitive(w, ixImin1,ixImax1, ixOmin1,&
         ixOmax1)
      w(ixOmin1:ixOmax1, e_) = srmhd_gamma_1* w(ixOmin1:ixOmax1,&
          rho_)**(1.0d0 - srmhd_gamma) * w(ixOmin1:ixOmax1, e_)
    else
  ...
  subroutine rhos_to_e(ixImin1,ixImax1,ixOmin1,ixOmax1,w,x)
    use mod_global_parameters
    integer, intent(in) :: ixImin1,ixImax1, ixOmin1,ixOmax1
    real(kind=dp)    :: w(ixImin1:ixImax1,nw)
    real(kind=dp)   , intent(in)    :: x(ixImin1:ixImax1,1:ndim)

    if (srmhd_energy) then
       w(ixOmin1:ixOmax1, e_) = w(ixOmin1:ixOmax1,&
           rho_)**srmhd_gamma_1 * w(ixOmin1:ixOmax1, e_) * inv_srmhd_gamma_1
       if(.not.block%e_is_internal) w(ixOmin1:ixOmax1, e_) =w(ixOmin1:ixOmax1,&
           e_) + srmhd_kin_en_primitive(w, ixImin1,ixImax1, ixOmin1,&
          ixOmax1) + srmhd_mag_en_primitive(w, ixImin1,ixImax1, ixOmin1,&
          ixOmax1)
  ...
  !> Calculate v vector
  subroutine srmhd_get_v(w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,v,B2,VdotB)
    ! made by Z. Meliani 13/02/2018
    use mod_global_parameters
    ...
    is_B2_in : if(present(B2)) then
     Loop_idir_b_v1 : do idir=1,ndir
      v(ixOmin1:ixOmax1,idir) = (w(ixOmin1:ixOmax1,&
          mom(idir)) + VdotB*w(ixOmin1:ixOmax1,mag(idir)))/(w(ixOmin1:ixOmax1,&
         xi_)+B2)
     end do Loop_idir_b_v1
    else  is_B2_in
     call srmhd_get_B2andVdotB(ixImin1,ixImax1,ixOmin1,ixOmax1,w,.true.,&
        B2=sub_B2,VdotB=sub_VdotB)
     Loop_idir_b_v2: do idir=1,ndir
      v(ixOmin1:ixOmax1,idir) = (w(ixOmin1:ixOmax1,&
          mom(idir))+sub_VdotB*w(ixOmin1:ixOmax1,&
         mag(idir)))/(w(ixOmin1:ixOmax1,xi_)+sub_B2)
  ...
  subroutine srmhd_get_v_idim(w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,idim,v_idim)
    ! made by Z. Meliani 10/02/2018
    use mod_global_parameters
    ...
    real(kind=dp)                          :: sub_VdotB(ixOmin1:ixOmax1),&
     ...
     call srmhd_get_B2andVdotB(ixImin1,ixImax1,ixOmin1,ixOmax1,w,.true.,&
        B2=sub_B2,VdotB=sub_VdotB)
     v_idim(ixOmin1:ixOmax1) = (w(ixOmin1:ixOmax1,&
         mom(idim)) + sub_VdotB*w(ixOmin1:ixOmax1,&
        mag(idim)))/(w(ixOmin1:ixOmax1,xi_)+sub_B2)
  end subroutine srmhd_get_v_idim

  !> Calculate v component
  subroutine srmhd_get_v_idim_loc(w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,idim,&
     v_idim,B2,VdotB)
    ! made by Z. Meliani 13/02/2018
    use mod_global_parameters
    ...
    if(present(B2))then
     v_idim(ixOmin1:ixOmax1) = (w(ixOmin1:ixOmax1,&
         mom(idim)) + VdotB*w(ixOmin1:ixOmax1,mag(idim)))/(w(ixOmin1:ixOmax1,&
        xi_)+B2)
    else
     call srmhd_get_B2andVdotB(ixImin1,ixImax1,ixOmin1,ixOmax1,w,.true.,&
        B2=sub_B2,VdotB=sub_VdotB)
 ...
 !> Calculate v^2
  subroutine srmhd_get_v2(w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,v2,B2,VdotB)
    ! made by Z. Meliani 13/02/2018
    use mod_global_parameters

    integer, intent(in)           :: ixImin1,ixImax1, ixOmin1,ixOmax1
    real(kind=dp)   , intent(in)  :: w(ixImin1:ixImax1,nw), x(ixImin1:ixImax1,&
       1:ndim)
    real(kind=dp)   , intent(out) :: v2(ixOmin1:ixOmax1)

    real(kind=dp)   , optional, intent(in)  :: VdotB(ixOmin1:ixOmax1),&
       B2(ixOmin1:ixOmax1)
    real(kind=dp)                           :: sub_VdotB(ixOmin1:ixOmax1),&
       sub_B2(ixOmin1:ixOmax1)
    real(kind=dp)                           :: v_num(ixOmin1:ixOmax1,1:ndir)
    integer :: idir

    if(present(B2)) then
     Loop_idir_B2: do idir=1,ndir
       v_num(ixOmin1:ixOmax1,idir)=w(ixOmin1:ixOmax1,&
           mom(idir)) + VdotB*w(ixOmin1:ixOmax1,mag(idir))
     end do  Loop_idir_B2
     v2(ixOmin1:ixOmax1) = sum(v_num(ixOmin1:ixOmax1, 1:ndir)**2.0 ,&
        dim=ndim+1)/(w(ixOmin1:ixOmax1,xi_)+B2)**2.0
    else
     call srmhd_get_B2andVdotB(ixImin1,ixImax1,ixOmin1,ixOmax1,w,.true.,&
        B2=sub_B2,VdotB=sub_VdotB)
     Loop_idir_noB2: do idir=1,ndir
       v_num(ixOmin1:ixOmax1,idir)=w(ixOmin1:ixOmax1,&
           mom(idir)) + sub_VdotB*w(ixOmin1:ixOmax1,mag(idir))
     end do  Loop_idir_noB2

     v2(ixOmin1:ixOmax1) = sum(v_num(ixOmin1:ixOmax1, :)**2.0 ,&
        dim=ndim+1)/(w(ixOmin1:ixOmax1,xi_)+sub_B2)**2.0
    end if
  end subroutine srmhd_get_v2
  !========================================================================
  !> subroutine that calculate the characteristic speed
  subroutine srmhd_get_cmax_gammie(ixImin1,ixImax1,ixOmin1,ixOmax1,idim,x,w,&
     rhoh,B2,VdotB,calfven,vidim,csound2,cmax,patch_gammie,from_cbound,cmin)
    ! made by Z. Meliani 10/02/2018
    use mod_global_parameters
    implicit none
    ...
    real(kind=dp)   , intent(in) :: w(ixImin1:ixImax1, nw), x(ixImin1:ixImax1,&
       1:ndim)
    real(kind=dp)   , intent(in), dimension(ixOmin1:ixOmax1) :: rhoh,B2,VdotB&
    ...
    ! .. local ..
    real(kind=dp)   , dimension(ixOmin1:ixOmax1):: A,B
    real(kind=dp)   , dimension(ixOmin1:ixOmax1):: v2
    !-----------------------------------------------------------


   is_patch : if(present(patch_gammie))then
    where(patch_gammie(ixOmin1:ixOmax1))
     A(ixOmin1:ixOmax1) = csound2(ixOmin1:ixOmax1)+&
        calfven(ixOmin1:ixOmax1)-csound2(ixOmin1:ixOmax1)*calfven(&
        ixOmin1:ixOmax1)
     B(ixOmin1:ixOmax1)=vidim(ixOmin1:ixOmax1)**2.0d0
    end where
    !if(.not.present(from_cbound))&
    !       where(patch_gammie(ixO^S))vidim(ixO^S)=dabs(vidim(ixO^S))

    cond_onedir_patch: if(ndir==1)then
     where(patch_gammie(ixOmin1:ixOmax1))cmax(ixOmin1:ixOmax1)=(vidim(&
        ixOmin1:ixOmax1)+dsqrt(A))/(1.0d0+dsqrt(A*B))
     if(present(cmin))where(patch_gammie(ixOmin1:ixOmax1))cmin(&
        ixOmin1:ixOmax1)=(vidim(ixOmin1:ixOmax1)-dsqrt(A))/(1.0d0+dsqrt(A*B))
    else cond_onedir_patch

     call srmhd_get_v2(w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,v2,B2=B2,&
        VdotB=VdotB)
     where(patch_gammie(ixOmin1:ixOmax1))
      cmax(ixOmin1:ixOmax1)=(vidim(ixOmin1:ixOmax1)*(1.0d0-A)+&
         dsqrt(A*(1.0d0-v2(ixOmin1:ixOmax1))*((1.0d0-v2(ixOmin1:ixOmax1)*A)-&
         B(ixOmin1:ixOmax1)*(1.0d0-A))))/( 1.0d0-v2(ixOmin1:ixOmax1)*A)
     end where
     if(present(cmin))where(patch_gammie(ixOmin1:ixOmax1))cmin(&
        ixOmin1:ixOmax1)=(vidim(ixOmin1:ixOmax1)*(1.0d0-A)-dsqrt(A*(1.0d0-&
        v2(ixOmin1:ixOmax1))*(1.0d0-v2(ixOmin1:ixOmax1)*A&
                         -B(ixOmin1:ixOmax1)*(1.0d0-A))))/( &
                            1.0d0-v2(ixOmin1:ixOmax1)*A)
    end if cond_onedir_patch

   else is_patch


    A = csound2(ixOmin1:ixOmax1)+calfven(ixOmin1:ixOmax1)-&
       csound2(ixOmin1:ixOmax1)*calfven(ixOmin1:ixOmax1)
  ...
  !                                      vidim(ixO^S)=dabs(vidim(ixO^S))
    ...
    cond_onedir: if(ndir==1)then
     cmax(ixOmin1:ixOmax1)=(vidim(ixOmin1:ixOmax1)+dsqrt(A))/(1.0d0+&
        dsqrt(A*B))
     if(present(cmin))cmin(ixOmin1:ixOmax1)=(vidim(ixOmin1:ixOmax1)-&
        dsqrt(A))/(1.0d0+dsqrt(A*B))
    else cond_onedir

     call srmhd_get_v2(w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,v2,B2=B2,&
        VdotB=VdotB)

     cmax(ixOmin1:ixOmax1)=(vidim(ixOmin1:ixOmax1)*(1.0_dp-A)+&
        dsqrt(A*(1.0_dp-v2)*((1.0_dp-v2*A)-B*(1.0_dp-A))))/( 1.0_dp-v2*A)

     if(present(cmin))cmin(ixOmin1:ixOmax1)=(vidim(ixOmin1:ixOmax1)*(1.0d0-A)-&
        dsqrt(A*(1.0d0-v2(ixOmin1:ixOmax1))*(1.0d0-v2(ixOmin1:ixOmax1)*A&
                         -B(ixOmin1:ixOmax1)*(1.0d0-A))))/( &
                            1.0d0-v2(ixOmin1:ixOmax1)*A)
    end if cond_onedir
  ...
  subroutine srmhd_get_cmax(w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,idim,cmax)
    ! made by Z. Meliani 13/02/2018
    use mod_global_parameters
    ...
    real(kind=dp)   , intent(inout)   :: cmax(ixImin1:ixImax1)
    ! .. local ..
    real(kind=dp)   , dimension(ixOmin1:ixOmax1):: rhoh,B2,VdotB,calfven2,&
       vidim
    real(kind=dp)   , dimension(ixOmin1:ixOmax1):: v2,csound2
    !-------------------------------------------------------


    rhoh=w(ixOmin1:ixOmax1,xi_)/w(ixOmin1:ixOmax1,lfac_)**2.0d0
    call srmhd_get_csound2(w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,rhoh,csound2)
    call srmhd_get_B2andVdotB(ixImin1,ixImax1,ixOmin1,ixOmax1,w,.true.,B2=B2,&
       VdotB=VdotB)
    call srmhd_get_calfven2(ixImin1,ixImax1,ixOmin1,ixOmax1,x,w,.true.,&
       calfven2,rhoh=rhoh,B2=B2)
    call srmhd_get_v_idim_loc(w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,idim,vidim,&
       B2=B2,VdotB=VdotB)

    call srmhd_get_cmax_gammie(ixImin1,ixImax1,ixOmin1,ixOmax1,idim,x,w,rhoh,&
       B2,VdotB,calfven2,dabs(vidim),csound2,cmax)


  end subroutine srmhd_get_cmax

  !> Estimating bounds for the minimum and maximum signal velocities
  subroutine srmhd_get_cbounds(wLC,wRC,wLp,wRp,x,ixImin1,ixImax1,ixOmin1,&
    ...
    integer, intent(in)                       :: ixImin1,ixImax1, ixOmin1,&
       ixOmax1, idim
    real(kind=dp)   , intent(in)              :: wLC(ixImin1:ixImax1, nw),&
        wRC(ixImin1:ixImax1, nw)
    real(kind=dp)   , intent(in)              :: wLp(ixImin1:ixImax1, nw),&
        wRp(ixImin1:ixImax1, nw)
    real(kind=dp)   , intent(in)              :: x(ixImin1:ixImax1,1:ndim)
    real(kind=dp)   , intent(inout)           :: cmax(ixImin1:ixImax1)
    real(kind=dp)   , intent(inout), optional :: cmin(ixImin1:ixImax1)
    ! .. local ..
    real(kind=dp)                      :: wmean(ixImin1:ixImax1,nw)
    real(kind=dp)   , dimension(ixOmin1:ixOmax1) :: umean, dmean, csound2Lp,&
        csound2Rp, tmp1,tmp2,tmp3, B2Lp,B2Rp,VdotBLp,VdotBRp, rhohLp,rhohRp,&
        cmaxL,cmaxR,csound2,rhoh,B2,VdotB, vidim,vidimLp,vidimRp,calfvenLp,&
       calfvenRp,calfven
    character(len=30)                  :: subname_loc
    ...
    if (typeboundspeed/='cmaxmean') then
      ...
      rhohLp=wLp(ixOmin1:ixOmax1,xi_)/wLp(ixOmin1:ixOmax1,lfac_)**2.0d0
      rhohRp=wRp(ixOmin1:ixOmax1,xi_)/wRp(ixOmin1:ixOmax1,lfac_)**2.0d0

      call srmhd_get_csound2(wLp,x,ixImin1,ixImax1,ixOmin1,ixOmax1,rhohLp,&
         csound2Lp)
      call srmhd_get_csound2(wRp,x,ixImin1,ixImax1,ixOmin1,ixOmax1,rhohRp,&
         csound2Rp)
      call srmhd_get_B2andVdotB(ixImin1,ixImax1,ixOmin1,ixOmax1,wLp,.false.,&
         B2=B2Lp,VdotB=VdotBLp)
      call srmhd_get_B2andVdotB(ixImin1,ixImax1,ixOmin1,ixOmax1,wRp,.false.,&
         B2=B2Rp,VdotB=VdotBRp)

      call srmhd_get_calfven2(ixImin1,ixImax1,ixOmin1,ixOmax1,x,wLp,.false.,&
         calfvenLp,rhoh=rhohLp,B2=B2Lp)
      call srmhd_get_calfven2(ixImin1,ixImax1,ixOmin1,ixOmax1,x,wRp,.false.,&
         calfvenRp,rhoh=rhohRp,B2=B2Rp)

      call srmhd_get_v_idim_loc(wLp,x,ixImin1,ixImax1,ixOmin1,ixOmax1,idim,&
         vidimLp,B2=B2Lp,VdotB=VdotBLp)
      call srmhd_get_v_idim_loc(wRp,x,ixImin1,ixImax1,ixOmin1,ixOmax1,idim,&
         vidimRp,B2=B2Rp,VdotB=VdotBRp)

      tmp1(ixOmin1:ixOmax1)=sqrt(wLp(ixOmin1:ixOmax1,&
         xi_)+B2Lp(ixOmin1:ixOmax1))
      tmp2(ixOmin1:ixOmax1)=sqrt(wRp(ixOmin1:ixOmax1,&
         xi_)+B2Rp(ixOmin1:ixOmax1))
      tmp3(ixOmin1:ixOmax1)=1.d0/(sqrt(wLp(ixOmin1:ixOmax1,&
         xi_)+B2Lp(ixOmin1:ixOmax1))+sqrt(wRp(ixOmin1:ixOmax1,&
         rho_)+B2Rp(ixOmin1:ixOmax1)))
     ...
     call srmhd_get_cmax_gammie(ixImin1,ixImax1,ixOmin1,ixOmax1,idim,x,wLp,&
        rhoh=rhohLp,B2=B2Lp,VdotB=VdotBLp,calfven=calfvenLp,&
        vidim=dabs(vidimLp),csound2=csound2Lp,cmax=cmaxL)

     call srmhd_get_cmax_gammie(ixImin1,ixImax1,ixOmin1,ixOmax1,idim,x,wRp,&
        rhoh=rhohRp,B2=B2Rp,VdotB=VdotBRp,calfven=calfvenRp,&
        vidim=dabs(vidimRp),csound2=csound2Rp,cmax=cmaxR)


      dmean(ixOmin1:ixOmax1)=(tmp1(ixOmin1:ixOmax1)*cmaxL(ixOmin1:ixOmax1)**2+&
         tmp2(ixOmin1:ixOmax1)*cmaxR(ixOmin1:ixOmax1)**&
      ...
      wmean(ixOmin1:ixOmax1,1:nw)=0.5_dp*(wLC(ixOmin1:ixOmax1,&
         1:nw)+wRC(ixOmin1:ixOmax1,1:nw))
      ! get auxiliary variables

      call srmhd_get_auxiliary(.true.,wmean,x,ixImin1,ixImax1,ixOmin1,ixOmax1,&
         subname_loc)
      rhoh=wmean(ixOmin1:ixOmax1,xi_)/wmean(ixOmin1:ixOmax1,lfac_)**2.0d0

      call srmhd_get_csound2(wmean,x,ixImin1,ixImax1,ixOmin1,ixOmax1,rhoh,&
         csound2)
      call srmhd_get_B2andVdotB(ixImin1,ixImax1,ixOmin1,ixOmax1,wmean,.true.,&
         B2=B2,VdotB=VdotB)
      call srmhd_get_calfven2(ixImin1,ixImax1,ixOmin1,ixOmax1,x,wmean,.true.,&
         calfven,rhoh=rhoh,B2=B2)
      call srmhd_get_v_idim_loc(wmean,x,ixImin1,ixImax1,ixOmin1,ixOmax1,idim,&
         vidim,B2=B2,VdotB=VdotB)
      call srmhd_get_cmax_gammie(ixImin1,ixImax1,ixOmin1,ixOmax1,idim,x,wmean,&
         rhoh=rhoh,B2=B2,VdotB=VdotB,calfven=calfven,vidim=vidim,&
         csound2=csound2,cmax=cmax,from_cbound=.true.,cmin=cmin)


      if(present(cmin)) then
        cmax(ixOmin1:ixOmax1)=min(max(cmax(ixOmin1:ixOmax1),0.0D0),1.0d0)
        cmin(ixOmin1:ixOmax1)=max(min(cmin(ixOmin1:ixOmax1),0.0D0),-1.0d0)
      else
        cmax(ixOmin1:ixOmax1)=min(cmax(ixOmin1:ixOmax1),1.0d0)
      end if
    end if
  end subroutine srmhd_get_cbounds
    ! made by Z. Meliani 10/02/2018
  !> Calculate the Aflven speed
  subroutine srmhd_get_calfven2(ixImin1,ixImax1,ixOmin1,ixOmax1,x,w,conserve,&
     calfven,rhoh,B2)
   use mod_global_parameters
   implicit none
   integer, intent(in)           :: ixImin1,ixImax1, ixOmin1,ixOmax1
   ...
   real(kind=dp)   , intent(out) :: calfven(ixOmin1:ixOmax1)
   real(kind=dp)   , optional    :: rhoh(ixOmin1:ixOmax1),B2(ixOmin1:ixOmax1)

   real(kind=dp)                 :: sub_B2(ixOmin1:ixOmax1),&
      sub_rhoh(ixOmin1:ixOmax1)

   if(present(B2))then
    calfven=B2/(B2+rhoh)
   else
    sub_rhoh=w(ixOmin1:ixOmax1,xi_)/w(ixOmin1:ixOmax1,lfac_)**2.0d0
    call srmhd_get_B2andVdotB(ixImin1,ixImax1,ixOmin1,ixOmax1,w,conserve,&
       B2=sub_B2)
...
!  end subroutine srmhd_get_csound
  ...
  !> Calculate fast magnetosonic wave speed
  subroutine srmhd_get_csound_prim(w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,idim,&
     csound)
    ! made by Z. Meliani 10/02/2018
    use mod_global_parameters
    ...
    real(kind=dp)   , intent(in) :: w(ixImin1:ixImax1, nw), x(ixImin1:ixImax1,&
       1:ndim)
    real(kind=dp)   , intent(out):: csound(ixImin1:ixImax1)
    real(kind=dp)    :: cfast2(ixImin1:ixImax1), AvMinCs2(ixImin1:ixImax1),&
    ...
    if(srmhd_energy) then
      csound(ixOmin1:ixOmax1)=dsqrt(srmhd_gamma*w(ixOmin1:ixOmax1,p_)*inv_rho)
    else
      csound(ixOmin1:ixOmax1)=dsqrt(srmhd_gamma*srmhd_adiab*w(ixOmin1:ixOmax1,&
         rho_)**srmhd_gamma_1)
    end if
 ...
 !> Calculate magnetic pressure within ixO^L including magnetic pressure
  subroutine srmhd_get_p_mag(w,ixImin1,ixImax1,ixOmin1,ixOmax1,pmag)
    ! made by Z. Meliani 10/02/2018
    use mod_global_parameters
    ...
    pmag(ixOmin1:ixOmax1) = 0.5d0 * (sum(w(ixOmin1:ixOmax1, mag(:))**2,&
        dim=ndim+1)+sum(w(ixOmin1:ixOmax1, mag(:))*w(ixOmin1:ixOmax1, mom(:)),&
        dim=ndim+1)/w(ixOmin1:ixOmax1,lfac_))/ w(ixOmin1:ixOmax1,lfac_)

  end subroutine srmhd_get_p_mag

  !> Calculate the square of the thermal sound speed csound2 within ixO^L.
  !> csound2=gamma*p/rho
  subroutine srmhd_get_csound2(w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,rhoh,&
     csound2)
    ! made by Z. Meliani 13/02/2018
    use mod_global_parameters
    integer, intent(in)             :: ixImin1,ixImax1, ixOmin1,ixOmax1
    ...
    if(srmhd_energy) then
      rho=w(ixOmin1:ixOmax1,d_)/w(ixOmin1:ixOmax1,lfac_)
      call srmhd_get_csound2_eos(ixImin1,ixImax1,ixOmin1,ixOmax1,x,rho,rhoh,&
         csound2)
    else
      csound2(ixOmin1:ixOmax1)=srmhd_gamma*srmhd_adiab*rho(ixOmin1:ixOmax1)**&
         srmhd_gamma_1
    end if
  ...
  subroutine srmhd_get_p_total(w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,p,B2,VdotB)
    ! made by Z. Meliani 10/02/2018
    use mod_global_parameters
    implicit none
    ...
    real(kind=dp)   , intent(in)    :: w(ixImin1:ixImax1,nw)
    real(kind=dp)   , intent(in)    :: x(ixImin1:ixImax1,1:ndim)
    real(kind=dp)   , intent(out)   :: p(ixImin1:ixImax1)
    real(kind=dp)   , optional, intent(in) :: B2(ixOmin1:ixOmax1),&
       VdotB(ixOmin1:ixOmax1)

    real(kind=dp)                   :: pmag(ixImin1:ixImax1)
    call srmhd_get_pthermal(w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,p)
    call srmhd_get_pmag(w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,pmag,B2=B2,&
       VdotB=VdotB)
    p(ixOmin1:ixOmax1) = p(ixOmin1:ixOmax1) + pmag(ixOmin1:ixOmax1)
  end subroutine srmhd_get_p_total

  subroutine srmhd_get_pmag(w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,pmag,B2,VdotB)
    ! made by Z. Meliani 10/02/2018
   use mod_global_parameters
   implicit none
    integer, intent(in)                    :: ixImin1,ixImax1, ixOmin1,ixOmax1
    ...
    real(kind=dp)   , dimension(ixOmin1:ixOmax1)     :: sub_B2,sub_VdotB

    is_presentB2 : if(present(B2)) then
     pmag(ixOmin1:ixOmax1) = 0.5d0*(VdotB(ixOmin1:ixOmax1)**2.0d0  + &
        B2(ixOmin1:ixOmax1)/w(ixOmin1:ixOmax1,lfac_)**2.0d0)
    else  is_presentB2
     call srmhd_get_B2andVdotB(ixImin1,ixImax1,ixOmin1,ixOmax1,w,.true.,&
        B2=sub_B2,VdotB=sub_VdotB)
     pmag(ixOmin1:ixOmax1) = 0.5d0*(sub_VdotB(ixOmin1:ixOmax1)**2.0d0  + &
        sub_B2(ixOmin1:ixOmax1)/w(ixOmin1:ixOmax1,lfac_)**2.0d0)
    end if is_presentB2
  end subroutine srmhd_get_pmag

  !> Calculate B2 and/ot VdotB in ixO^S from conserve or primitive variables
  subroutine srmhd_get_B2andVdotB(ixImin1,ixImax1,ixOmin1,ixOmax1,w,wconserve,&
     B2,VdotB)
    ! made by Z. Meliani 13/02/2018
    use mod_global_parameters
    implicit none
    ...
    is_VdotB_present : if(present(VdotB))then
     is_conserve : if(wconserve)then
       VdotB = sum(w(ixOmin1:ixOmax1,mag(:))*w(ixOmin1:ixOmax1,mom(:)),&
          dim=ndim+1)/w(ixOmin1:ixOmax1,xi_)
     else is_conserve
       VdotB = sum(w(ixOmin1:ixOmax1,mag(:))*w(ixOmin1:ixOmax1,mom(:)),&
          dim=ndim+1)/w(ixOmin1:ixOmax1,lfac_)
     end if is_conserve
    end if is_VdotB_present
    if(present(B2))B2    = sum(w(ixOmin1:ixOmax1,mag(:))**2.0,dim=ndim+1)

  end subroutine srmhd_get_B2andVdotB


  !> Calculate fluxes within ixO^L.
  subroutine srmhd_get_flux(wC,wP,x,ixImin1,ixImax1,ixOmin1,ixOmax1,idim,f)
    ! made by Z. Meliani 10/02/2018
    use mod_global_parameters
    implicit none
    integer, intent(in)          :: ixImin1,ixImax1, ixOmin1,ixOmax1, idim
    ! conservative w
    real(kind=dp)   , intent(in) :: wC(ixImin1:ixImax1,nw)
    ! primitive w
    real(kind=dp)   , intent(in) :: wP(ixImin1:ixImax1,nw)
    real(kind=dp)   , intent(in) :: x(ixImin1:ixImax1,1:ndim)
    real(kind=dp)   ,intent(out) :: f(ixImin1:ixImax1,nwflux)

    real(kind=dp)                :: ptotal(ixImin1:ixImax1)
    real(kind=dp)                :: B2(ixOmin1:ixOmax1),VdotB(ixOmin1:ixOmax1)
    real(kind=dp)                :: v(ixImin1:ixImax1,1:ndir)
    integer                      :: idirmin, iw, idir


    call srmhd_get_B2andVdotB(ixImin1,ixImax1,ixOmin1,ixOmax1,wp,.false.,B2=B2,&
       VdotB=VdotB)
    call srmhd_get_p_total(wC,x,ixImin1,ixImax1,ixOmin1,ixOmax1,ptotal,B2=B2,&
       VdotB=VdotB)
    call srmhd_get_v(wC,x,ixImin1,ixImax1,ixOmin1,ixOmax1,v,B2=B2,VdotB=VdotB)
    ! Get flux of density
    !>@f[F(\rho) = u^{idims}\cdot D]@f
    f(ixOmin1:ixOmax1,rho_)=wp(ixOmin1:ixOmax1,mom(idim))*wp(ixOmin1:ixOmax1,&
       rho_)

    ! Get flux of tracer
    do iw=1,srmhd_n_tracer
      f(ixOmin1:ixOmax1,tracer(iw))=wp(ixOmin1:ixOmax1,&
         mom(idim))*wp(ixOmin1:ixOmax1,tracer(iw))
    end do

    ! Get flux of momentum
    ! f_i[m_k]=v_i*m_k-B_k*b_i [+ptotal if i==k]
    Loop_idir_mom : do idir=1,ndir
      f(ixOmin1:ixOmax1,mom(idir))= v(ixOmin1:ixOmax1,idir)*wC(ixOmin1:ixOmax1,&
         mom(idim))-wP(ixOmin1:ixOmax1,mag(idir))*(VdotB(ixOmin1:ixOmax1)*v(&
         ixOmin1:ixOmax1,idim)+wP(ixOmin1:ixOmax1,&
         mag(idim))/wP(ixOmin1:ixOmax1,lfac_)**2.0d0)

    end do Loop_idir_mom
    f(ixOmin1:ixOmax1,mom(idim))=ptotal(ixOmin1:ixOmax1)+f(ixOmin1:ixOmax1,&
       mom(idim))
    ...
    is_notiso : if (srmhd_energy) then
       is_internal : if (block%e_is_internal) then
          f(ixOmin1:ixOmax1,e_)=v(ixOmin1:ixOmax1,idim)*wP(ixOmin1:ixOmax1,p_)
       else is_internal
          f(ixOmin1:ixOmax1,e_)=v(ixOmin1:ixOmax1,idim)*(wC(ixOmin1:ixOmax1,&
             e_) + ptotal(ixOmin1:ixOmax1))- wP(ixOmin1:ixOmax1,&
             mag(idim))*VdotB(ixOmin1:ixOmax1)
          if(type_divb==divb_glm2) then
            f(ixOmin1:ixOmax1,e_) = f(ixOmin1:ixOmax1,&
               e_) + vmax_global*wP(ixOmin1:ixOmax1,psi_)*wP(ixOmin1:ixOmax1,&
               mag(idim))
          end if
    ...
    Loop_idir_Bflux : do idir=1,ndir
      is_idim_Bflux : if (idim==idir) then
        ...
        is_glm_Bflux : if (srmhd_glm) then
           if(type_divb==divb_glm1) then
             f(ixOmin1:ixOmax1,mag(idir))=wP(ixOmin1:ixOmax1,psi_)
           else
             f(ixOmin1:ixOmax1,mag(idir))=vmax_global*wP(ixOmin1:ixOmax1,psi_)
           end if
        else is_glm_Bflux
           f(ixOmin1:ixOmax1,mag(idir))=zero
        end if is_glm_Bflux
      else is_idim_Bflux
        f(ixOmin1:ixOmax1,mag(idir))=v(ixOmin1:ixOmax1,&
           idim)*wP(ixOmin1:ixOmax1,mag(idir))-wP(ixOmin1:ixOmax1,&
           mag(idim))*v(ixOmin1:ixOmax1,idir)
    ...
    if (srmhd_glm) then
      if(type_divb==divb_glm1) then
        ...
        f(ixOmin1:ixOmax1,psi_)  = cmax_global**2*wP(ixOmin1:ixOmax1,&
           mag(idim))
      else
        !f_i[psi]=Ch*b_{i} Eq. 3.16e Derigs et al 2018 JCP, 364, 420
        f(ixOmin1:ixOmax1,psi_)  = vmax_global*wP(ixOmin1:ixOmax1,mag(idim))
      end if
    end if
  ...
  subroutine srmhd_add_source(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,wCT,w,x,&
     qsourcesplit,active)
    ! made by Z. Meliani 10/02/2018
    use mod_global_parameters
...
!    end if
    ...
    if(srmhd_gravity) then
      call gravity_add_source(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,wCT,w,x,&
         srmhd_energy,qsourcesplit,active)
    end if
  ...
  subroutine internal_energy_add_source(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,&
    ...
    real(kind=dp)   , intent(in)    :: wCT(ixImin1:ixImax1,1:nw),&
        x(ixImin1:ixImax1,1:ndim)
    real(kind=dp)   , intent(inout) :: w(ixImin1:ixImax1,1:nw)
    real(kind=dp)                   :: pth(ixImin1:ixImax1),v(ixImin1:ixImax1,&
       1:ndir),divv(ixImin1:ixImax1)

    call srmhd_get_v(wCT,x,ixImin1,ixImax1,ixImin1,ixImax1,v)
    call divvector(v,ixImin1,ixImax1,ixOmin1,ixOmax1,divv)
    call srmhd_get_pthermal(wCT,x,ixImin1,ixImax1,ixOmin1,ixOmax1,pth)
    w(ixOmin1:ixOmax1,e_)=w(ixOmin1:ixOmax1,&
  ...
  subroutine add_source_glm1(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,wCT,w,x)
    ...
    integer, intent(in) :: ixImin1,ixImax1, ixOmin1,ixOmax1
    real(kind=dp)   , intent(in) :: qdt, wCT(ixImin1:ixImax1,1:nw),&
        x(ixImin1:ixImax1,1:ndim)
    real(kind=dp)   , intent(inout) :: w(ixImin1:ixImax1,1:nw)
    real(kind=dp)   :: divb(ixImin1:ixImax1)
    integer          :: idim,idir
    real(kind=dp)    :: gradPsi(ixImin1:ixImax1)
    ...
    if (srmhd_glm_alpha < zero) then
      w(ixOmin1:ixOmax1,psi_) = abs(srmhd_glm_alpha)*wCT(ixOmin1:ixOmax1,psi_)
    else
      ! implicit update of Psi variable
      ! equation (27) in Mignone 2010 J. Com. Phys. 229, 2117
      if(slab) then
        w(ixOmin1:ixOmax1,psi_) = dexp(-qdt*cmax_global*srmhd_glm_alpha/minval(&
    ...
    do idir=1,ndir
      w(ixOmin1:ixOmax1,mom(idir))=w(ixOmin1:ixOmax1,&
         mom(idir))-qdt*srmhd_mag_i_all(w,ixImin1,ixImax1,ixOmin1,ixOmax1,&
  ...
  subroutine add_source_glm2(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,wCT,w,x)
    ...
    real(kind=dp)   , intent(in) :: qdt, wCT(ixImin1:ixImax1,1:nw),&
        x(ixImin1:ixImax1,1:ndim)
    real(kind=dp)   , intent(inout) :: w(ixImin1:ixImax1,1:nw)
    real(kind=dp)   :: divb(ixImin1:ixImax1),v(ixImin1:ixImax1,1:ndir)
    integer          :: idim,idir
    real(kind=dp)    :: gradPsi(ixImin1:ixImax1,ndim), Bf(ixImin1:ixImax1,&
       1:ndir)
    ...
    ! calculate velocity
    call srmhd_get_v(wCT,x,ixImin1,ixImax1,ixOmin1,ixOmax1,v)
    ...
    do idim=1,ndim
       ...
       case("central")
         call gradient(wCT(ixImin1:ixImax1,psi_),ixImin1,ixImax1,ixOmin1,&
            ixOmax1,idim,gradPsi(ixImin1:ixImax1,idim))
       case("limited")
         call gradientS(wCT(ixImin1:ixImax1,psi_),ixImin1,ixImax1,ixOmin1,&
            ixOmax1,idim,gradPsi(ixImin1:ixImax1,idim))
       end select
    ...
    if (srmhd_energy .and. .not.block%e_is_internal) then
       ...
       w(ixOmin1:ixOmax1,e_)=w(ixOmin1:ixOmax1,&
          ...
          :)*Bf(ixOmin1:ixOmax1,:),dim=ndim+1) + wCT(ixOmin1:ixOmax1,&
          psi_)*sum(v(ixOmin1:ixOmax1,1:ndim)*gradPsi(ixOmin1:ixOmax1,1:ndim),&
          dim=ndim+1))
    end if
    ...
    do idir=1,ndir
      w(ixOmin1:ixOmax1,mom(idir))=w(ixOmin1:ixOmax1,&
         mom(idir))-qdt*Bf(ixOmin1:ixOmax1,idir)*divb(ixOmin1:ixOmax1)
    end do

    ! psi = psi - qdt * (v . grad(psi) + alpha * psi)
    w(ixOmin1:ixOmax1,psi_) = w(ixOmin1:ixOmax1,&
       psi_)-qdt*(sum(v(ixOmin1:ixOmax1,1:ndim)*gradPsi(ixOmin1:ixOmax1,&
       1:ndim),dim=ndim+1)+vmax_global/0.18d0*wCT(ixOmin1:ixOmax1,psi_))
  ...
  subroutine add_source_powel(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,wCT,w,x)
    ...
    real(kind=dp)   , intent(in)    :: qdt,   wCT(ixImin1:ixImax1,1:nw),&
        x(ixImin1:ixImax1,1:ndim)
    real(kind=dp)   , intent(inout) :: w(ixImin1:ixImax1,1:nw)
    real(kind=dp)                   :: divb(ixImin1:ixImax1),v(ixImin1:ixImax1,&
       1:ndir)
    integer                         :: idir
  ...
  subroutine add_source_linde(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,wCT,w,x)
    ...
    real(kind=dp)    :: divb(ixImin1:ixImax1),graddivb(ixImin1:ixImax1)
    ...
    do i1=-1,1
      if(i1==0) cycle
      if(neighbor_type(i1,saveigrid)==2 .or. neighbor_type(i1,&
         saveigrid)==4) then
        leveljump(i1)=.true.
      else
        leveljump(i1)=.false.
  ...
  subroutine get_divb(w,ixImin1,ixImax1,ixOmin1,ixOmax1,divb)
    ...
    real(kind=dp)                      :: bvec(ixImin1:ixImax1,1:ndir)
  ...
  subroutine get_normalized_divb(w,ixImin1,ixImax1,ixOmin1,ixOmax1,divb)
    ...
    if(slab) then
      divb(ixOmin1:ixOmax1)=0.5d0*abs(divb(ixOmin1:ixOmax1))/sqrt(&
         srmhd_mag_en_all(w,ixImin1,ixImax1,ixOmin1,&
      ...
      divb(ixOmin1:ixOmax1)=abs(divb(ixOmin1:ixOmax1))/sqrt(srmhd_mag_en_all(w,&
  ...
  subroutine get_current(w,ixImin1,ixImax1,ixOmin1,ixOmax1,idirmin,current)
    ...
    real(kind=dp)    :: current(ixImin1:ixImax1,7-2*ndir:3),&
  ...
  subroutine srmhd_get_dt(w,ixImin1,ixImax1,ixOmin1,ixOmax1,dtnew,dx1,x)
    use mod_global_parameters
    use mod_usr_methods
  ...
  subroutine srmhd_add_source_geom(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,wCT,w,&
     x)
    ! made by Z. Meliani 10/02/2018
    use mod_global_parameters

    integer, intent(in)             :: ixImin1,ixImax1, ixOmin1,ixOmax1
    real(kind=dp)   , intent(in)    :: qdt, x(ixImin1:ixImax1,1:ndim)
    real(kind=dp)   , intent(inout) :: wCT(ixImin1:ixImax1,1:nw),&
        w(ixImin1:ixImax1,1:nw)

    integer          :: iw,idir, h1xmin1,h1xmax1
    real(kind=dp)    :: tmp(ixImin1:ixImax1),ptot(ixImin1:ixImax1),&
       tmp2(ixImin1:ixImax1)
    real(kind=dp)    :: B2(ixOmin1:ixOmax1),VdotB(ixOmin1:ixOmax1),&
       VdotB2(ixOmin1:ixOmax1),xiplusB2(ixOmin1:ixOmax1)
    ...
    if(typeaxial /= 'slab')then
     ...
     ! get auxiliary variables
     call srmhd_get_auxiliary(.true.,wCT,x,ixImin1,ixImax1,ixOmin1,ixOmax1,&
        subname_loc)
     call srmhd_get_B2andVdotB(ixImin1,ixImax1,ixOmin1,ixOmax1,wCT,.true.,&
        B2=B2,VdotB=VdotB)
     call srmhd_get_p_total(wCT,x,ixImin1,ixImax1,ixOmin1,ixOmax1,ptot)
     VdotB2=VdotB**2.0
     xiplusB2(ixOmin1:ixOmax1) = wCT(ixOmin1:ixOmax1,&
        xi_)**2.0+B2(ixOmin1:ixOmax1)
      ...
      if (angmomfix) then
       ...
       if(phi_>0) then
         w(ixOmin1:ixOmax1,mr_)=w(ixOmin1:ixOmax1,mr_)+qdt/x(ixOmin1:ixOmax1,&
            1)*(ptot(ixOmin1:ixOmax1)+(wCT(ixOmin1:ixOmax1,&
            mphi_)**2-VdotB2(ixOmin1:ixOmax1)*wCT(ixOmin1:ixOmax1,&
            bphi_)**2)/xiplusB2(ixOmin1:ixOmax1)-(wCT(ixOmin1:ixOmax1,&
            bphi_)/wCT(ixOmin1:ixOmax1,lfac_))**2.0)

         w(ixOmin1:ixOmax1,mphi_)=w(ixOmin1:ixOmax1,&
            mphi_)+qdt/x(ixOmin1:ixOmax1,1)*(-(wCT(ixOmin1:ixOmax1,&
            mphi_)*wCT(ixOmin1:ixOmax1,mr_)-&
            VdotB2(ixOmin1:ixOmax1)*wCT(ixOmin1:ixOmax1,&
            br_)*wCT(ixOmin1:ixOmax1,bphi_))/xiplusB2(ixOmin1:ixOmax1) &
            +wCT(ixOmin1:ixOmax1,bphi_)*wCT(ixOmin1:ixOmax1,&
            br_)/wCT(ixOmin1:ixOmax1,lfac_)**2)
         ...
         w(ixOmin1:ixOmax1,bphi_)=w(ixOmin1:ixOmax1,&
            ...
            br_)*wCT(ixOmin1:ixOmax1,mphi_)) /xiplusB2(ixOmin1:ixOmax1)
       else
         w(ixOmin1:ixOmax1,mr_)=w(ixOmin1:ixOmax1,mr_)+qdt/x(ixOmin1:ixOmax1,&
            1)*ptot(ixOmin1:ixOmax1)
       end if
       if(srmhd_glm) w(ixOmin1:ixOmax1,br_)=w(ixOmin1:ixOmax1,&
          br_)+qdt*wCT(ixOmin1:ixOmax1,psi_)/x(ixOmin1:ixOmax1,1)
    case ('spherical')
       h1xmin1=ixOmin1-kr(1,1);h1xmax1=ixOmax1-kr(1,1); ;

       ! m1
       tmp(ixOmin1:ixOmax1)=ptot(ixOmin1:ixOmax1)*x(ixOmin1:ixOmax1,&
       ...
       if(ndir>1) then
         do idir=2,ndir
           tmp(ixOmin1:ixOmax1)=tmp(ixOmin1:ixOmax1)+(wCT(ixOmin1:ixOmax1,&
              mom(idir))**2.0-(VdotB2(ixOmin1:ixOmax1)*B2(ixOmin1:ixOmax1)))/(&
              xiplusB2(ixOmin1:ixOmax1))-wCT(ixOmin1:ixOmax1,&
              mag(idir))**2/wCT(ixOmin1:ixOmax1,lfac_)**2.0
         end do
       end if
       ...
       if(ndir==3) then
         ...
         if(.not.angmomfix) then
           tmp(ixOmin1:ixOmax1)=-((wCT(ixOmin1:ixOmax1,&
              ...
              mom(1))-VdotB2(ixOmin1:ixOmax1)*wCT(ixOmin1:ixOmax1,&
              mag(3))*wCT(ixOmin1:ixOmax1,&
              mag(1)))/xiplusB2(ixOmin1:ixOmax1) -wCT(ixOmin1:ixOmax1,&
              mag(3))*wCT(ixOmin1:ixOmax1,mag(1))/wCT(ixOmin1:ixOmax1,&
              lfac_)**2.0d0) 
  ...
  function srmhd_mag_en_all(w, ixImin1,ixImax1, ixOmin1,ixOmax1) result(mge)
    use mod_global_parameters
    integer, intent(in)           :: ixImin1,ixImax1, ixOmin1,ixOmax1
    real(kind=dp)   , intent(in)  :: w(ixImin1:ixImax1, nw)
    real(kind=dp)                 :: mge(ixOmin1:ixOmax1)
  ...
  function srmhd_mag_i_all(w, ixImin1,ixImax1, ixOmin1,ixOmax1,&
     idir) result(mgf)
    use mod_global_parameters
    integer, intent(in)           :: ixImin1,ixImax1, ixOmin1,ixOmax1, idir
    real(kind=dp)   , intent(in)  :: w(ixImin1:ixImax1, nw)
    real(kind=dp)                 :: mgf(ixOmin1:ixOmax1)
  ...
  function srmhd_mag_en_primitive(w, ixImin1,ixImax1, ixOmin1,&
     ixOmax1) result(mge)
    ...
    mge = sum(w(ixOmin1:ixOmax1, mag(:))**2_dp, dim=ndim+1)
    mge = 0.5_dp*(mge(ixOmin1:ixOmax1)+ mge(ixOmin1:ixOmax1)*sum(w(&
       ixOmin1:ixOmax1, mom(:))**2_dp, dim=ndim+1)/w(ixOmin1:ixOmax1,&
       lfac_)+sum(w(ixOmin1:ixOmax1, mag(:))*w(ixOmin1:ixOmax1,&
        mom(:))))/w(ixOmin1:ixOmax1,lfac_)
  end function srmhd_mag_en_primitive

  !> compute kinetic energy from primitive
  function srmhd_kin_en_primitive(w, ixImin1,ixImax1, ixOmin1,ixOmax1,&
      inv_rho) result(ke)
    ! made by Z. Meliani 10/02/2018
    use mod_global_parameters, only: nw, ndim
    integer, intent(in)           :: ixImin1,ixImax1, ixOmin1,ixOmax1
    real(kind=dp)   , intent(in)  :: w(ixImin1:ixImax1, nw)
    real(kind=dp)                 :: ke(ixOmin1:ixOmax1)
    real(kind=dp)   , intent(in), optional :: inv_rho(ixOmin1:ixOmax1)

    if (present(inv_rho)) then
       ke = (w(ixOmin1:ixOmax1,xi_)-1.0d0) * inv_rho
    else
       ke = (w(ixOmin1:ixOmax1,xi_)-1.0d0) / w(ixOmin1:ixOmax1, rho_)
    end if
...
!  end subroutine srmhd_getdt_Hall
  ...
  subroutine glmSolve(wLC,wRC,ixImin1,ixImax1,ixOmin1,ixOmax1,idir)
    use mod_global_parameters
    real(kind=dp)   , intent(inout) :: wLC(ixImin1:ixImax1,1:nw),&
        wRC(ixImin1:ixImax1,1:nw)
    integer, intent(in)             :: ixImin1,ixImax1, ixOmin1,ixOmax1, idir
    real(kind=dp)                   :: dB(ixImin1:ixImax1),&
  ...
  subroutine fixdivB_boundary(ixGmin1,ixGmax1,ixOmin1,ixOmax1,w,x,iB)
    ...
    real(kind=dp)   , intent(inout) :: w(ixGmin1:ixGmax1,1:nw)
    real(kind=dp)   , intent(in) :: x(ixGmin1:ixGmax1,1:ndim)
    ...
    select case(iB)
     case(1)
       ...
       if(srmhd_energy.and..not.block%e_is_internal) call &
          srmhd_to_primitive(ixGmin1,ixGmax1,ixOmin1,ixOmax1,w,x)
       
       
       if(srmhd_energy.and..not.block%e_is_internal) call &
          srmhd_to_conserved(ixGmin1,ixGmax1,ixOmin1,ixOmax1,w,x)
     case(2)
       if(srmhd_energy.and..not.block%e_is_internal) call &
          srmhd_to_primitive(ixGmin1,ixGmax1,ixOmin1,ixOmax1,w,x)
   ...
   subroutine srmhd_get_auxiliary(clipping,w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,&
      subname,use_oldaux,sqrB,SdotB)
    ! made by Z. Meliani 10/02/2018
    use mod_global_parameters
    ...
    logical         , intent(in), target,&
        optional :: use_oldaux(ixImin1:ixImax1)
    real(kind=dp)   , intent(in), target, optional :: sqrB(ixImin1:ixImax1),&
       SdotB(ixImin1:ixImax1)
    ! .. local
    integer                               :: ix1,ierror,idir
    ...
    real(kind=dp),target                  :: SdotB_loc(ixOmin1:ixOmax1),&
       sqrB_loc(ixOmin1:ixOmax1)
    real(kind=dp), dimension(ixOmin1:ixOmax1)       :: sqrS,SdotB2,E2
    real(kind=dp), dimension(ixOmin1:ixOmax1,1:ndir):: tmp,v
    !---------------------------------------------------------------------



where(dabs(w(ixOmin1:ixOmax1,mag(3)))<smalldouble)w(ixOmin1:ixOmax1,&
   mag(3))=0.0_dp
where(dabs(w(ixOmin1:ixOmax1,mag(1)))<smalldouble)w(ixOmin1:ixOmax1,&
   mag(1))=0.0_dp
where(dabs(w(ixOmin1:ixOmax1,mag(2)))<smalldouble)w(ixOmin1:ixOmax1,&
   mag(2))=0.0_dp
    ...
    if(present(sqrB))then
      ...
      sqrB_loc=SUM(w(ixOmin1:ixOmax1,mag(1):mag(ndir))**2.0_dp,dim=ndim+1)
      sqrB_in=>sqrB_loc
    end if
    ...
    if(present(SdotB))then
      ...
      SdotB_loc=SUM(w(ixOmin1:ixOmax1,mom(1):mom(ndir))*w(ixOmin1:ixOmax1,&
         mag(1):mag(ndir)),dim=ndim+1)
      SdotB_in=>SdotB_loc
    end if

    sqrS(ixOmin1:ixOmax1)  = sum(w(ixOmin1:ixOmax1,mom(1):mom(ndir))**2.0_dp,&
       dim=ndim+1)
    SdotB2(ixOmin1:ixOmax1)= SdotB_in(ixOmin1:ixOmax1)**2.0_dp
    E2(ixOmin1:ixOmax1)    = sqrB_in(ixOmin1:ixOmax1)*sqrS(ixOmin1:ixOmax1)-&
       SdotB2(ixOmin1:ixOmax1)
   ...
   !PRINT*,' is wrong ',ix^D,w(ix^D,:)
    call srmhd_con2prim(w(ix1,d_),w(ix1,e_),use_oldaux_in(ix1),sqrB_in(ix1),&
       sqrS(ix1),SdotB_in(ix1),SdotB2(ix1),E2(ix1),w(ix1,lfac_),w(ix1,xi_),&
       ierror)

    if(check_small_values)then
     if(ierror/=0) then
       flag_error(ix1) = ierror
     else
       flag_error(ix1) = 0
     end if
   ...
   enddo
    ...
    cond_isflow : if(any(sqrS(ixOmin1:ixOmax1) /= &
       zero.and.flag_error(ixOmin1:ixOmax1)/=0))then
        where(sqrS(ixOmin1:ixOmax1)/= zero.and.flag_error(ixOmin1:ixOmax1)/=0)
          E2(ixOmin1:ixOmax1) =sdotb_in(ixOmin1:ixOmax1)/w(ixOmin1:ixOmax1,&
             xi_)
        end where
        Loop_idir0 : do idir=1,ndir
         where(sqrS(ixOmin1:ixOmax1)/= zero.and.flag_error(ixOmin1:ixOmax1)/=&
            0)
           tmp(ixOmin1:ixOmax1,idir)=w(ixOmin1:ixOmax1,&
              mom(idir)) + w(ixOmin1:ixOmax1,mag(idir))*E2(ixOmin1:ixOmax1)
         end where
        end do Loop_idir0
        where(sqrS(ixOmin1:ixOmax1)/= zero.and.flag_error(ixOmin1:ixOmax1)/=0)
        ! reuse  E2,SdotB2
         E2(ixOmin1:ixOmax1)     = dsqrt(sum(tmp(ixOmin1:ixOmax1,&
            1:ndir)**2.0_dp,dim=ndim+1))
         SdotB2(ixOmin1:ixOmax1) = dsqrt(1.0_dp-1.0_dp/(w(ixOmin1:ixOmax1,&
            lfac_)**2))
         E2(ixOmin1:ixOmax1)     = SdotB2(ixOmin1:ixOmax1)/E2(ixOmin1:ixOmax1)
        endwhere
        Loop_idir : do idir=1,ndir
         where(sqrS(ixOmin1:ixOmax1)/= zero.and.flag_error(ixOmin1:ixOmax1)/=&
            0)
           v(ixOmin1:ixOmax1,idir)=E2(ixOmin1:ixOmax1)*tmp(ixOmin1:ixOmax1,&
              idir)
         end where
        end do  Loop_idir
        where(sqrS(ixOmin1:ixOmax1)/= zero.and.flag_error(ixOmin1:ixOmax1)/=0)
         E2(ixOmin1:ixOmax1)=sum(v(ixOmin1:ixOmax1,1:ndir)**2.0_dp,dim=ndim+1)
        end where
        where(sqrS(ixOmin1:ixOmax1)/= zero.and.flag_error(ixOmin1:ixOmax1)/=0)
         where(E2(ixOmin1:ixOmax1)>one)flag_error(ixOmin1:ixOmax1)=6
        end where
    end if cond_isflow

   is_check_small : if(check_small_values)then

    is_flag_on : if(any(flag_error(ixOmin1:ixOmax1)/=0))then
     subname_loc='srmhd_get_auxiliary from -> '//trim(subname)
     call srmhd_handle_small_values(.false., w, x, ixImin1,ixImax1, ixOmin1,&
        ixOmax1,subname_loc,flag_error=flag_error)
    end if is_flag_on
...
! end subroutine srmhd_get_auxiliary_loc
   ...
   subroutine srmhd_get_4u_from_3v(ixImin1,ixImax1,ixOmin1,ixOmax1,vtou,lfac)
    ! made by Z. Meliani 13/02/2018
    use mod_global_parameters
    implicit none
    integer, intent(in)             :: ixImin1,ixImax1,ixOmin1,ixOmax1
    real(kind=dp)   , intent(inout) :: vtou(ixImin1:ixImax1,1:ndir)
    real(kind=dp)   , intent(inout) :: lfac(ixImin1:ixImax1)
    !.. local ..
    integer                         :: idir
    !------------------------------------------------------

    lfac(ixOmin1:ixOmax1)= 1.0d0/dsqrt(1.0d0-sum(vtou(ixOmin1:ixOmax1,&
       1:ndir)**2.0,dim=ndim+1))
    Loop_idir: do idir=1,ndir
     vtou(ixOmin1:ixOmax1,idir)=lfac(ixOmin1:ixOmax1)*vtou(ixOmin1:ixOmax1,&
        idir)
    end do Loop_idir
   ...
   subroutine srmhd_get_3v_from_4u(ixImin1,ixImax1,ixOmin1,ixOmax1,lfac,vtou)
    ! made by Z. Meliani 13/02/2018
    use mod_global_parameters
    implicit none
    integer, intent(in)             :: ixImin1,ixImax1,ixOmin1,ixOmax1
    real(kind=dp)   , intent(in)    :: lfac(ixImin1:ixImax1)
    real(kind=dp)   , intent(inout) :: vtou(ixImin1:ixImax1,1:ndir)
    ...
    Loop_idir: do idir=1,ndir
     vtou(ixOmin1:ixOmax1,idir)=vtou(ixOmin1:ixOmax1,&
        idir)/lfac(ixOmin1:ixOmax1)
    end do Loop_idir
