...
111    close(unitpar)
       ...
       if (block%e_is_internal) then
        ...
        ! Calculate pressure=(gamma-1)*(e-0.5*(2ek+2eb))
           tmp(ixOmin1:ixOmax1)=w(ixOmin1:ixOmax1,e_) - mhd_kin_en(w,ixImin1,&
              ...
              ixOmax1)
           if(mhd_glm) tmp(ixOmin1:ixOmax1)=tmp(ixOmin1:ixOmax1)-&
              0.5d0*w(ixOmin1:ixOmax1,psi_)**2
           tmp(ixOmin1:ixOmax1)=gamma_1*tmp(ixOmin1:ixOmax1)
  ...
  subroutine mhd_to_primitive(ixImin1,ixImax1,ixOmin1,ixOmax1,w,x)
    ...
    if (mhd_energy) then
      ...
      if(.not.block%e_is_internal) then
        w(ixOmin1:ixOmax1, p_) = w(ixOmin1:ixOmax1, e_) - mhd_kin_en(w,&
            ixImin1,ixImax1, ixOmin1,ixOmax1, inv_rho) - mhd_mag_en(w, ixImin1,&
           ixImax1, ixOmin1,ixOmax1)
      ! Calculate pressure = (gamma-1) * (e-ek-eb-epsi)
        if(type_divb==divb_glm2) w(ixOmin1:ixOmax1, p_)=w(ixOmin1:ixOmax1,&
            p_)-0.5d0*w(ixOmin1:ixOmax1,psi_)**2
      end if
      w(ixOmin1:ixOmax1, p_) = gamma_1*w(ixOmin1:ixOmax1, p_)
    end if
  ...
  subroutine mhd_handle_small_values(primitive, w, x, ixImin1,ixImax1, ixOmin1,&
    ...
    if (any(flag(ixOmin1:ixOmax1) /= 0)) then
       ...
       case ("replace")
         call mhd_small_values_floor(primitive, w, x, ixImin1,ixImax1, ixOmin1,&
       ...
       case ("average")
          ...
          if(small_values_force_floor.and.ierror/=0)then
            call mhd_small_values_floor(primitive, w, x, ixImin1,ixImax1,&
                ixOmin1,ixOmax1, subname,flag)
          end if
       case default
 ...
 !-----------------------------------------------------------------
  ...
  subroutine mhd_small_values_floor(primitive, w, x, ixImin1,ixImax1, ixOmin1,&
     ixOmax1, subname,flag)
    use mod_global_parameters
    use mod_small_values
    ...
    integer                         :: idir
        ...
        if(mhd_dust)call dust_set_floor(primitive,ixImin1,ixImax1,ixOmin1,&
           ixOmax1,flag,x,w)
  ...
  subroutine mhd_get_flux(wC,w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,idim,f)
       ...
       if (block%e_is_internal) then
          ...
          if(type_divb==divb_glm2) then
            f(ixOmin1:ixOmax1,e_) = f(ixOmin1:ixOmax1,&
               e_) + vmax_global*w(ixOmin1:ixOmax1,psi_)*w(ixOmin1:ixOmax1,&
               mag(idim))
          end if
      ...
      if (idim==idir) then
        ...
        if (mhd_glm) then
           if(type_divb==divb_glm1) then
             ...
             f(ixOmin1:ixOmax1,mag(idir))=vmax_global*w(ixOmin1:ixOmax1,psi_)
           end if
        else
    ...
    if (mhd_glm) then
      if(type_divb==divb_glm1) then
        ...
        f(ixOmin1:ixOmax1,psi_)  = vmax_global*w(ixOmin1:ixOmax1,mag(idim))
      end if
    end if
  ...
  subroutine add_source_B0split(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,wCT,w,x)
    ...
    double precision :: wB0(ixImin1:ixImax1,1:ndir),dB0(ixImin1:ixImax1,&
       1:ndir),b1dB0(ixImin1:ixImax1)
    integer :: idir

   if(B0field_reset)then
     call set_B0_cell(wB0,x,ixImin1,ixImax1,ixOmin1,ixOmax1,global_time+qdt)
     dB0(ixOmin1:ixOmax1,1:ndir) = (wB0(ixOmin1:ixOmax1,&
        1:ndir)-block%B0(ixOmin1:ixOmax1,1:ndir,0))
   end if
    a=0.d0
    b=0.d0
    ...
    if(mhd_energy) then
      if(.not.block%e_is_internal) then
        ...
        if(B0field_reset)then
          b1dB0(ixOmin1:ixOmax1) = SUM(wCT(ixOmin1:ixOmax1,&
             mag(1:ndir))*dB0(ixOmin1:ixOmax1,1:ndir),dim=ndim+1)
          w(ixOmin1:ixOmax1,e_)=w(ixOmin1:ixOmax1,e_)-b1dB0(ixOmin1:ixOmax1)
        end if
      end if
    end if
    if(B0field_reset)w(ixOmin1:ixOmax1,mag(1:ndir))=w(ixOmin1:ixOmax1,&
       mag(1:ndir))-dB0(ixOmin1:ixOmax1,1:ndir)
  ...
  subroutine add_source_glm2(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,wCT,w,x)
    ...
    integer          :: idim,idir
    double precision :: gradPsi(ixImin1:ixImax1,ndim), Bf(ixImin1:ixImax1,&
       1:ndir)
  ...
  subroutine mhd_getdt_Hall(w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,dx1,dthall)
    ...
    if(slab) then
      dthall=dtdiffpar*minval(dxarr(1:ndim))**&
         2.0d0/(mhd_etah*maxval(bmag(ixOmin1:ixOmax1)/w(ixOmin1:ixOmax1,&
         rho_)))
    else
      dthall=dtdiffpar*minval(block%ds(ixOmin1:ixOmax1,&
         1:ndim))**2.0d0/(mhd_etah*maxval(bmag(ixOmin1:ixOmax1)/w(&
         ixOmin1:ixOmax1,rho_)))
    end if
