...
if(nwauxio>0)then
  ...
  if (.not. associated(usr_aux_output)) then
     ...
     call usr_aux_output(ixGlo1,ixGhi1,ixMlo1-1,ixMhi1+1,w,pw(igrid)%x,&
        normconv)
  end if
endif
...
if(saveprim.and.first) call phys_to_primitive(ixGlo1,ixGhi1,ixMlo1-1,ixMhi1+1,&
   w(ixGlo1:ixGhi1,1:nw),pw(igrid)%x)
...
if(slab) then
   ...
   do iw=1,nw+nwauxio
     do ix1=ixCmin1,ixCmax1
       wC(ix1,iw)=sum(w(ix1:ix1+1,iw)*pw(igrid)%dvolume(ix1:ix1+1)) &
          /sum(pw(igrid)%dvolume(ix1:ix1+1))
     end do
   end do
endif
...
do iigrid=1,igridstail; igrid=igrids(iigrid)
   ...
   pw(igrid)%wio(ixGlo1:ixGhi1,1:nw)=pw(igrid)%w(ixGlo1:ixGhi1,1:nw)

   if (nwauxio > 0) then
      if (.not. associated(usr_aux_output)) then
         call mpistop("usr_aux_output not defined")
      else
         call usr_aux_output(ixGlo1,ixGhi1,ixMlo1-1,ixMhi1+1, pw(igrid)%wio,&
            pw(igrid)%x,normconv)
      end if
   end if
end do
...
if (saveprim) then
  do iigrid=1,igridstail; igrid=igrids(iigrid)
    if (.not.writeblk(igrid)) cycle
    call phys_to_primitive(ixGlo1,ixGhi1,ixGlo1+1,ixGhi1-1,pw(igrid)%wio,&
       pw(igrid)%x)
    if (allocated(pw(igrid)%B0)) then
      ! add background magnetic field B0 to B
      pw(igrid)%wio(ixGlo1:ixGhi1,iw_mag(:))=pw(igrid)%wio(ixGlo1:ixGhi1,&
         iw_mag(:))+pw(igrid)%B0(ixGlo1:ixGhi1,:,0)
    end if
  end do
else
  do iigrid=1,igridstail; igrid=igrids(iigrid)
    ...
    if (allocated(pw(igrid)%B0)) then
      ! add background magnetic field B0 to B
      if(phys_energy) pw(igrid)%wio(ixGlo1:ixGhi1,&
         iw_e)=pw(igrid)%wio(ixGlo1:ixGhi1,&
         iw_e)+0.5d0*sum(pw(igrid)%B0(ixGlo1:ixGhi1,:,0)**2,&
         dim=ndim+1) + sum(pw(igrid)%wio(ixGlo1:ixGhi1,&
         iw_mag(:))*pw(igrid)%B0(ixGlo1:ixGhi1,:,0),dim=ndim+1)
      pw(igrid)%wio(ixGlo1:ixGhi1,iw_mag(:))=pw(igrid)%wio(ixGlo1:ixGhi1,&
         iw_mag(:))+pw(igrid)%B0(ixGlo1:ixGhi1,:,0)
    end if
  end do
       ...
       do ig1=1,ng1(level_io)
         ...
         do ix1=ixMlo1,ixMhi1
           Master_write : if(mype==0) then
             select case(convert_type)
               case("oneblock")
                 write(qunit,fmt="(100(e14.6))") pw(igrid)%x(ix1,&
                    1:ndim)*normconv(0),(pw(igrid)%wio(ix1,&
                    iwrite(iw))*normconv(iwrite(iw)),iw=1,writenw)
               case("oneblockB")
                 write(qunit) real(pw(igrid)%x(ix1,1:ndim)*normconv(0)),&
                    (real(pw(igrid)%wio(ix1,iwrite(iw))*normconv(iwrite(iw))),&
                    iw=1,writenw)
             end select
           end if Master_write
         end do
...
do Morton_no=Morton_start(mype),Morton_stop(mype)
  igrid=sfc_to_igrid(Morton_no)
  if(saveprim) call phys_to_primitive(ixGlo1,ixGhi1,ixMlo1,ixMhi1,pw(igrid)%w,&
     pw(igrid)%x)
  if (mype/=0)then
      itag=Morton_no
   ...
   do ix1=ixMlo1,ixMhi1
      ...
      enddo
       write(qunit,fmt="(100(e14.6))") pw(igrid)%x(ix1,1:ndim),pw(igrid)%w(ix1,&
          1:nw)
   end do
  end if
end do
...
subroutine calc_x(igrid,xC,xCC)
  ...
  xCC(ixMlo1:ixMhi1,:)=pw(igrid)%x(ixMlo1:ixMhi1,:)

  ! coordinates of cell corners
  ixCmin1=ixMlo1-1; ixCmax1=ixMhi1;
  if(slab)then
     do idims=1,ndim
       xC(ixCmin1:ixCmax1,idims)=pw(igrid)%x(ixCmin1:ixCmax1,&
     ...
     do ix=ixCmin1,ixCmax1
       xC(ix,1)=pw(igrid)%x(ix,1)+0.5d0*pw(igrid)%dx(ix,1)
     end do
  endif
