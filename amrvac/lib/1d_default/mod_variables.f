...
contains
  ...
  !> Set auxiliary variable, which is not advected and has no boundary conditions.
  !> This has to be done after defining flux variables.
  function var_set_auxvar(name_aux, ix) result(iw)
    ...
    if (.not. present(ix)) then
      ...
      write(cons_wnames(iw),"(A,I0)") name_aux, ix
      write(prim_wnames(iw),"(A,I0)") name_aux, ix
    end if
