...
111    close(unitpar)
  ...
  subroutine mhd_phys_init()
    ...
    if (mhd_radiative_cooling) then
      call radiative_cooling_init(mhd_ind,mhd_config,mhd_gamma,He_abundance)
    end if
  ...
  subroutine mhd_handle_small_values(primitive, w, x, ixI^L, ixO^L, subname,&
    ...
    if (any(flag(ixO^S) /= 0)) then
       ...
       case ("replace")
         call mhd_small_values_floor(primitive, w, x, ixI^L, ixO^L, subname,&
                                      flag)
       case ("average")
 ...
 !-----------------------------------------------------------------
  ...
  subroutine mhd_small_values_floor(primitive, w, x, ixI^L, ixO^L, subname,&
                                   flag)
    use mod_global_parameters
    use mod_small_values
    ...
    integer                         :: idir
        ...
        if(mhd_dust)call dust_set_floor(primitive,ixI^L,ixO^L,flag,x,w)
  ...
  subroutine glmSolve(wLp, wRp,wCT,x,ixI^L,ixO^L,idir, qdt, qtC, qt,&
   dx^D, method, wnew, wold, local_block, wLC, wRC)
    use mod_global_parameters
