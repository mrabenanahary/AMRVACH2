...
contains
  ...
  subroutine mhd_ppm_flatcd(ixI^L,ixO^L,ixL^L,ixR^L,w,d2w,drho,dpres)
  ...
  subroutine mhd_ppm_flatsh(ixI^L,ixO^L,ixLL^L,ixL^L,ixR^L,ixRR^L,idims,w,drho,dpres,dv)
    use mod_global_parameters
    use mod_geometry
    ...
    if(mhd_energy) then
      ...
      dpres(ixO^S)=(mhd_gamma*w(ixO^S,p_)/w(ixO^S,rho_))
