...
111   close(unitpar)
  ...
  subroutine get_3d_dragforce(ixI^L, ixO^L, w, x, fdrag, ptherm, vgas,fd_flag)
    ...
    real(kind=dp)                   :: alpha_T(ixI^S, 1:dust_n_species)
    ...
    !                                                     fd,fd_flag,w)
         ...
         !  end if   test_todelete
          ...
          ! testttt
          if(it<dust_it_diff)fd(ixO^S) = coef*fd(ixO^S)
  ...
  subroutine get_sticking(w, x, ixI^L, ixO^L, alpha_T, ptherm)
    ...
    real(kind=dp), intent(in)  :: x(ixI^S, 1:ndim)
    real(kind=dp), intent(in)  :: w(ixI^S, 1:nw)
    real(kind=dp), intent(out) :: alpha_T(ixI^S, 1:dust_n_species)
  ...
  subroutine get_tdust(w, x, ixI^L, ixO^L, Td)
    ...
    real(kind=dp), intent(in)  :: x(ixI^S, 1:ndim)
    real(kind=dp), intent(in)  :: w(ixI^S, 1:nw)
    real(kind=dp), intent(out) :: Td(ixI^S, 1:dust_n_species)
  ...
  subroutine dust_add_source(qdt, ixI^L, ixO^L, wCT,w, x, qsourcesplit, active)
    ...
    real(kind=dp)    :: fdrag(ixI^S, ndir, dust_n_species)
  ...
  subroutine dust_get_dt(w, ixI^L, ixO^L, dtnew, dx^D, x)
    ...
    real(kind=dp), dimension(ixI^S,1:dust_n_species) :: alpha_T
    real(kind=dp)                              :: K
    integer                                    :: n, idir
