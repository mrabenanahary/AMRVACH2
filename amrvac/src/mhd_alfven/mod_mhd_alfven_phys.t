...
module mod_mhd_alfven_phys
  ...
  logical, public, protected              :: mhd_alfven_energy = .true.

  !> Whether thermal conduction is used
  logical, public, protected              :: mhd_alfven_thermal_conduction = .false.

  !> Whether radiative cooling is added
  logical, public, protected              :: mhd_alfven_radiative_cooling = .false.

  !> Whether viscosity is added
  logical, public, protected              :: mhd_alfven_viscosity = .false.

  !> Whether gravity is added
  logical, public, protected              :: mhd_alfven_gravity = .false.

  !> Whether Hall-MHD is used
  logical, public, protected              :: mhd_alfven_Hall = .false.

  !> Whether particles module is added
  logical, public, protected              :: mhd_alfven_particles = .false.

  !> Whether magnetofriction is added
  logical, public, protected              :: mhd_alfven_magnetofriction = .false.

  !> Whether GLM-MHD is used
  logical, public, protected              :: mhd_alfven_glm = .false.
  ...
  double precision, public                :: mhd_alfven_glm_alpha = 0.5d0

  !> MHD fourth order
  logical, public, protected              :: mhd_alfven_4th_order = .false.

  !> Number of tracer species
  integer, public, protected              :: mhd_alfven_n_tracer = 0
  ...
  !> Indices of the alfven wave
  integer, public, protected :: epsmin_
  integer, public, protected :: epsplus_
  ...
  !> The adiabatic index
  double precision, public                :: mhd_alfven_gamma = 5.d0/3.0d0

  !> The adiabatic constant
  double precision, public                :: mhd_alfven_adiab = 1.0d0

  !> The MHD resistivity
  double precision, public                :: mhd_alfven_eta = 0.0d0

  !> The MHD hyper-resistivity
  double precision, public                :: mhd_alfven_eta_hyper = 0.0d0

  !> TODO: what is this?
  double precision, public                :: mhd_alfven_etah = 0.0d0

  !> The lambda
  double precision, public                :: mhd_alfven_lambda = 0.5_dp
  ...
  logical, allocatable,target                    :: mhd_alfven_iw_average(:)
  type(physconfig),target,public                 :: mhd_alfven_config
  type(phys_variables_indices),target,public     :: mhd_alfven_ind
  ...
  logical, public, protected  :: mhd_alfven_dust=.false.
  ! Public methods
  public :: mhd_alfven_phys_init
  public :: mhd_alfven_kin_en
  public :: mhd_alfven_get_pthermal
  public :: mhd_alfven_get_v
  public :: mhd_alfven_to_conserved
  public :: mhd_alfven_to_primitive
  public :: mhd_alfven_get_csound2
  public :: get_divb
  public :: get_current
...
contains
  ...
  subroutine mhd_alfven_read_params(files)
    use mod_global_parameters
    character(len=*), intent(in) :: files(:)
    integer                      :: n

    namelist /mhd_alfven_list/ mhd_alfven_energy, mhd_alfven_n_tracer, mhd_alfven_gamma, mhd_alfven_adiab,&
      mhd_alfven_eta, mhd_alfven_eta_hyper, mhd_alfven_etah, mhd_alfven_glm_alpha, mhd_alfven_magnetofriction,&
      mhd_alfven_thermal_conduction, mhd_alfven_radiative_cooling, mhd_alfven_Hall, mhd_alfven_gravity,&
      mhd_alfven_viscosity, mhd_alfven_4th_order, typedivbfix, source_split_divb, divbdiff,&
      typedivbdiff, compactres, divbwave, He_abundance, SI_unit, B0field,&
      B0field_forcefree, B0field_reset,Bdip, Bquad, Boct, Busr, &
      mu0dip,mu0theta,mu0phi,muphi_period, mutheta_period,mhd_alfven_particles,&
      boundary_divbfix, boundary_divbfix_skip,unit_velocity,unit_length,&
      unit_numberdensity,ndir

    do n = 1, size(files)
       open(unitpar, file=trim(files(n)), status="old")
       read(unitpar, mhd_alfven_list, end=111)
111    close(unitpar)
    end do

  end subroutine mhd_alfven_read_params

  !> Write this module's parameters to a snapsoht
  subroutine mhd_alfven_write_info(fh)
    use mod_global_parameters
    ...
    names(1) = "gamma"
    values(1) = mhd_alfven_gamma
    call MPI_FILE_WRITE(fh, values, n_par, MPI_DOUBLE_PRECISION, st, er)
    call MPI_FILE_WRITE(fh, names, n_par * name_len, MPI_CHARACTER, st, er)
  end subroutine mhd_alfven_write_info

  subroutine mhd_alfven_angmomfix(fC,x,wnew,ixI^L,ixO^L,idim)
    use mod_global_parameters
  ...
  end subroutine mhd_alfven_angmomfix

  subroutine mhd_alfven_phys_init()
    use mod_global_parameters
    ...
    call mhd_alfven_read_params(par_files)

    physics_type = "mhd_alfven"
    phys_energy=mhd_alfven_energy
    mhd_alfven_config%dust_on = .false.
    ! set default gamma for polytropic/isothermal process
    if(.not.mhd_alfven_energy) mhd_alfven_gamma=1.d0
    use_particles=mhd_alfven_particles
    ...
    !
    ! if (mhd_alfven_glm) then
    !   psi_ = var_set_fluxvar('psi', 'psi', need_bc=.false.)
    ! else
    !   psi_ = -1
    ! end if
    !
    ! allocate(tracer(mhd_alfven_n_tracer))
    !
    ! ! Set starting index of tracers
    ! do itr = 1, mhd_alfven_n_tracer
    !   tracer(itr) = var_set_fluxvar("trc", "trp", itr, need_bc=.false.)
    ! end do
    call mhd_alfven_fill_phys_indices
    ...
    if(mhd_alfven_glm .and. ndim>1) flux_type(:,psi_)=flux_tvdlf
    flux_type(idir,epsplus_)=flux_tvdlf
    flux_type(idir,epsmin_)=flux_tvdlf
    
    allocate(mhd_alfven_iw_average(nw))
    mhd_alfven_iw_average         = .true.
    mhd_alfven_iw_average(mag(:)) = .false.


    phys_get_dt              => mhd_alfven_get_dt
    phys_get_cmax            => mhd_alfven_get_cmax
    phys_get_cbounds         => mhd_alfven_get_cbounds
    phys_get_flux            => mhd_alfven_get_flux
    phys_get_v_idim          => mhd_alfven_get_v_idim
    phys_add_source_geom     => mhd_alfven_add_source_geom
    phys_add_source          => mhd_alfven_add_source
    phys_to_conserved        => mhd_alfven_to_conserved
    phys_to_primitive        => mhd_alfven_to_primitive
    phys_check_params        => mhd_alfven_check_params
    phys_check_w             => mhd_alfven_check_w
    phys_get_pthermal        => mhd_alfven_get_pthermal
    phys_boundary_adjust     => mhd_alfven_boundary_adjust
    phys_write_info          => mhd_alfven_write_info
    phys_angmomfix           => mhd_alfven_angmomfix
    phys_handle_small_values => mhd_alfven_handle_small_values
    ...
    call mhd_alfven_physical_units()

    if(.not. mhd_alfven_energy .and. mhd_alfven_thermal_conduction) then
      call mpistop("thermal conduction needs mhd_alfven_energy=T")
    end if
    if(.not. mhd_alfven_energy .and. mhd_alfven_radiative_cooling) then
      call mpistop("radiative cooling needs mhd_alfven_energy=T")
    end if

    ! initialize thermal conduction module
    if (mhd_alfven_thermal_conduction) then
      phys_req_diagonal = .true.
      call thermal_conduction_init(mhd_alfven_gamma)
    end if

    ! Initialize radiative cooling module
    if (mhd_alfven_radiative_cooling) then
      call radiative_cooling_init(mhd_alfven_gamma,He_abundance)
    end if

    ! Initialize viscosity module
    if (mhd_alfven_viscosity) call viscosity_init(phys_wider_stencil,phys_req_diagonal)

    ! Initialize gravity module
    if(mhd_alfven_gravity) then
      call gravity_init()
    end if

    ! Initialize particles module
    if(mhd_alfven_particles) then
      call particles_init()
      phys_req_diagonal = .true.
    end if

    ! initialize magnetofriction module
    if(mhd_alfven_magnetofriction) then
      phys_req_diagonal = .true.
    ...
    if (mhd_alfven_hall) then
       if (mhd_alfven_4th_order) then
   ...
   mhd_alfven_config%dust_n_species = 0!dust_n_species
   mhd_alfven_config%energy         = mhd_alfven_energy
   phys_iw_average => mhd_alfven_iw_average
   phys_config     => mhd_alfven_config
   phys_ind        => mhd_alfven_ind
  end subroutine mhd_alfven_phys_init
  subroutine mhd_alfven_fill_phys_indices
    use mod_global_parameters
    implicit none
    ! .. local ..
    integer :: itr, idir
    ...
    allocate(mag(ndir))
    mag(:) = var_set_bfield(ndir)
    epsplus_=var_set_fluxvar('epsplus', 'epsplus', need_bc=.true.)
    epsmin_=var_set_fluxvar('epsmin', 'epsmin', need_bc=.true.)
    if (mhd_alfven_glm) then
      psi_ = var_set_fluxvar('psi', 'psi', need_bc=.false.)
    else
      psi_ = -1
    ...
    allocate(mhd_alfven_ind%mom(ndir),mhd_alfven_ind%mag(ndir))
    mhd_alfven_ind%rho_  =rho_
    mhd_alfven_ind%mom(:)=mom(:)
    mhd_alfven_ind%mag(:)=mag(:)
    mhd_alfven_ind%e_    =e_
    mhd_alfven_ind%pressure_   =p_
    mhd_alfven_ind%psi_  =psi_
    mhd_alfven_ind%lfac_ =-1
    mhd_alfven_ind%xi_   =-1
    if(mhd_alfven_n_tracer>0)then
      allocate(mhd_alfven_ind%tracer(mhd_alfven_n_tracer))
      mhd_alfven_ind%tracer(:) = tracer(:)
    end if
  end subroutine mhd_alfven_fill_phys_indices

  subroutine mhd_alfven_check_params
    use mod_global_parameters

    ! after user parameter setting
    gamma_1=mhd_alfven_gamma-1.d0

    if (.not. mhd_alfven_energy) then
       if (mhd_alfven_gamma <= 0.0d0) call mpistop ("Error: mhd_alfven_gamma <= 0")
       if (mhd_alfven_adiab < 0.0d0) call mpistop ("Error: mhd_alfven_adiab < 0")
       small_pressure = mhd_alfven_adiab*small_density**mhd_alfven_gamma
    else
       if (mhd_alfven_gamma <= 0.0d0 .or. mhd_alfven_gamma == 1.0d0) &
            call mpistop ("Error: mhd_alfven_gamma <= 0 or mhd_alfven_gamma == 1")
       inv_gamma_1=1.d0/gamma_1
       small_e = small_pressure * inv_gamma_1
    end if

  end subroutine mhd_alfven_check_params

  subroutine mhd_alfven_physical_units()
    use mod_global_parameters
  ...
  end subroutine mhd_alfven_physical_units

  subroutine mhd_alfven_check_w(primitive,ixI^L,ixO^L,w,flag)
    use mod_global_parameters
       ...
       if (block%e_is_internal) then
        ...
        ! Calculate pressure=(gamma-1)*(e-0.5*(2ek+2eb))
           tmp(ixO^S)=w(ixO^S,e_) - &
              mhd_alfven_kin_en(w,ixI^L,ixO^L)-mhd_alfven_mag_en(w,ixI^L,ixO^L)
           if(mhd_alfven_glm) tmp(ixO^S)=tmp(ixO^S)-0.5d0*w(ixO^S,psi_)**2
           tmp(ixO^S)=gamma_1*tmp(ixO^S)
           where(tmp(ixO^S) < small_pressure) flag(ixO^S) = e_
         end if
       end if
    end if
  end subroutine mhd_alfven_check_w

  !> Transform primitive variables into conservative ones
  subroutine mhd_alfven_to_conserved(ixI^L,ixO^L,w,x)
    use mod_global_parameters
    integer, intent(in)             :: ixI^L, ixO^L
    ...
    if (mhd_alfven_energy) then
      ...
      if(.not.block%e_is_internal) w(ixO^S,e_)=w(ixO^S,e_) + &
        ...
        mhd_alfven_mag_en(w, ixI^L, ixO^L)
    ...
    if (check_small_values) call mhd_alfven_handle_small_values(.false., w, x, ixI^L, ixO^L,'mhd_alfven_to_conserved')
  end subroutine mhd_alfven_to_conserved

  !> Transform conservative variables into primitive ones
  subroutine mhd_alfven_to_primitive(ixI^L,ixO^L,w,x)
    use mod_global_parameters
    integer, intent(in)             :: ixI^L, ixO^L
    double precision, intent(inout) :: w(ixI^S, nw)
    double precision, intent(in)    :: x(ixI^S, 1:ndim)
    ...
    if (mhd_alfven_energy) then
      ...
      if(.not.block%e_is_internal) then
        w(ixO^S, p_) = w(ixO^S, e_) &
              - mhd_alfven_kin_en(w, ixI^L, ixO^L, inv_rho) &
              - mhd_alfven_mag_en(w, ixI^L, ixO^L)
    ...
    if (check_small_values) call mhd_alfven_handle_small_values(.true., w, x, ixI^L, ixO^L,'mhd_alfven_to_primitive')
  end subroutine mhd_alfven_to_primitive

  subroutine mhd_alfven_handle_small_values(primitive, w, x, ixI^L, ixO^L, subname,&
                                     flag_error)
    use mod_global_parameters
    ...
    call mhd_alfven_check_w(primitive, ixI^L, ixO^L, w, flag)

    if (any(flag(ixO^S) /= 0)) then
       select case (small_values_method)
       case ("replace")
         call mhd_alfven_small_values_floor(primitive, w, x, ixI^L, ixO^L, subname,&
                                      flag)
       case ("average")
  ...
  end subroutine mhd_alfven_handle_small_values

 !-----------------------------------------------------------------
  !> reset floor variables
  subroutine mhd_alfven_small_values_floor(primitive, w, x, ixI^L, ixO^L, subname,&
                                   flag)
    use mod_global_parameters
    use mod_small_values
    ...
    integer                         :: idir
        ...
        if(mhd_alfven_dust)call dust_set_floor(primitive,ixI^L,ixO^L,flag,x,w)
  end subroutine mhd_alfven_small_values_floor
  ...
  subroutine e_to_rhos(ixI^L,ixO^L,w,x)
    ...
    if (mhd_alfven_energy) then
      if(.not.block%e_is_internal) &
        w(ixO^S, e_) = w(ixO^S, e_) - mhd_alfven_kin_en(w, ixI^L, ixO^L) &
              - mhd_alfven_mag_en(w, ixI^L, ixO^L)
      w(ixO^S, e_) = gamma_1* w(ixO^S, rho_)**(1.0d0 - mhd_alfven_gamma) * &
            w(ixO^S, e_)
    else
  ...
  !> Calculate v vector
  subroutine mhd_alfven_get_v(w,x,ixI^L,ixO^L,v)
    use mod_global_parameters
  ...
  end subroutine mhd_alfven_get_v

  !> Calculate v component
  subroutine mhd_alfven_get_v_idim(w,x,ixI^L,ixO^L,idim,v)
    use mod_global_parameters
  ...
  end subroutine mhd_alfven_get_v_idim

  !> Calculate cmax_idim=csound+abs(v_idim) within ixO^L
  subroutine mhd_alfven_get_cmax(w,x,ixI^L,ixO^L,idim,cmax)
    use mod_global_parameters
    ...
    call mhd_alfven_get_csound(w,x,ixI^L,ixO^L,idim,cmax)

    cmax(ixO^S)=abs(w(ixO^S,mom(idim))/w(ixO^S,rho_))+cmax(ixO^S)

  end subroutine mhd_alfven_get_cmax

  !> Estimating bounds for the minimum and maximum signal velocities
  subroutine mhd_alfven_get_cbounds(wLC,wRC,wLp,wRp,x,ixI^L,ixO^L,idim,cmax,cmin)
    use mod_global_parameters
    ...
    if (typeboundspeed/='cmaxmean') then
      ...
      call mhd_alfven_get_csound_prim(wLp,x,ixI^L,ixO^L,idim,csoundL)
      call mhd_alfven_get_csound_prim(wRp,x,ixI^L,ixO^L,idim,csoundR)
      ...
      call mhd_alfven_get_csound(wmean,x,ixI^L,ixO^L,idim,csoundR)
      if(present(cmin)) then
  ...
  end subroutine mhd_alfven_get_cbounds

  !> Calculate fast magnetosonic wave speed
  subroutine mhd_alfven_get_csound(w,x,ixI^L,ixO^L,idim,csound)
    use mod_global_parameters
    ...
    call mhd_alfven_get_csound2(w,x,ixI^L,ixO^L,csound)
    ! store |B|^2 in v
    b2(ixO^S)        = mhd_alfven_mag_en_all(w,ixI^L,ixO^L)
    cfast2(ixO^S)   = b2(ixO^S) * inv_rho+csound(ixO^S)
    AvMinCs2(ixO^S) = cfast2(ixO^S)**2-4.0d0*csound(ixO^S) &
         * mhd_alfven_mag_i_all(w,ixI^L,ixO^L,idim)**2 &
         * inv_rho
    ...
    if (.not. mhd_alfven_Hall) then
       ...
       csound(ixO^S) = max(sqrt(half*(cfast2(ixO^S)+AvMinCs2(ixO^S))), &
            mhd_alfven_etah * sqrt(b2(ixO^S))*inv_rho*kmax)
    end if

  end subroutine mhd_alfven_get_csound

  !> Calculate fast magnetosonic wave speed
  subroutine mhd_alfven_get_csound_prim(w,x,ixI^L,ixO^L,idim,csound)
    use mod_global_parameters
    ...
    if(mhd_alfven_energy) then
      csound(ixO^S)=mhd_alfven_gamma*w(ixO^S,p_)*inv_rho
    else
      csound(ixO^S)=mhd_alfven_gamma*mhd_alfven_adiab*w(ixO^S,rho_)**gamma_1
    end if
    ! store |B|^2 in v
    b2(ixO^S)        = mhd_alfven_mag_en_all(w,ixI^L,ixO^L)
    cfast2(ixO^S)   = b2(ixO^S) * inv_rho+csound(ixO^S)
  ...
  end subroutine mhd_alfven_get_csound_prim

  !> Calculate thermal pressure=(gamma-1)*(e-0.5*m**2/rho-b**2/2) within ixO^L
  subroutine mhd_alfven_get_pthermal(w,x,ixI^L,ixO^L,pth)
    use mod_global_parameters
    ...
    if(mhd_alfven_energy) then
      if(block%e_is_internal) then
        ...
        pth(ixO^S)=gamma_1*(w(ixO^S,e_)&
           - mhd_alfven_kin_en(w,ixI^L,ixO^L)&
           - mhd_alfven_mag_en(w,ixI^L,ixO^L))
      end if
    else
      pth(ixO^S)=mhd_alfven_adiab*w(ixO^S,rho_)**mhd_alfven_gamma
    end if
  end subroutine mhd_alfven_get_pthermal

  !> Calculate the square of the thermal sound speed csound2 within ixO^L.
  !> csound2=gamma*p/rho
  subroutine mhd_alfven_get_csound2(w,x,ixI^L,ixO^L,csound2)
    use mod_global_parameters
    integer, intent(in)             :: ixI^L, ixO^L
    double precision, intent(in)    :: w(ixI^S,nw)
    double precision, intent(in)    :: x(ixI^S,1:ndim)
    ...
    if(mhd_alfven_energy) then
      call mhd_alfven_get_pthermal(w,x,ixI^L,ixO^L,csound2)
      csound2(ixO^S)=mhd_alfven_gamma*csound2(ixO^S)/w(ixO^S,rho_)
    else
      csound2(ixO^S)=mhd_alfven_gamma*mhd_alfven_adiab*w(ixO^S,rho_)**gamma_1
    end if
  end subroutine mhd_alfven_get_csound2

  !> Calculate total pressure within ixO^L including magnetic pressure
  subroutine mhd_alfven_get_p_total(w,x,ixI^L,ixO^L,p)
    use mod_global_parameters
    ...
    call mhd_alfven_get_pthermal(w,x,ixI^L,ixO^L,p)

    p(ixO^S) = p(ixO^S) + 0.5d0 * sum(w(ixO^S, mag(:))**2, dim=ndim+1)

  end subroutine mhd_alfven_get_p_total

  !> Calculate fluxes within ixO^L.
  subroutine mhd_alfven_get_flux(wC,w,x,ixI^L,ixO^L,idim,f)
    use mod_global_parameters
    ...
    double precision             :: ptotal(ixO^S),tmp(ixI^S),valfven(ixI^S)
    double precision, allocatable:: vHall(:^D&,:)
    integer                      :: idirmin, iw, idir

    if (mhd_alfven_Hall) then
      allocate(vHall(ixI^S,1:ndir))
      call mhd_alfven_getv_Hall(w,x,ixI^L,ixO^L,vHall)
    end if
    ...
    if(mhd_alfven_energy) then
      ...
      ptotal(ixO^S)=mhd_alfven_adiab*w(ixO^S,rho_)**mhd_alfven_gamma+0.5d0*sum(w(ixO^S, mag(:))**2, dim=ndim+1)
    end if
    ...
    ! Get flux of epsplus
    valfven(ixO^S)=dsqrt(sum((w(ixO^S, mag(:))+block%B0(ixO^S,:,idim))**2, dim=ndim+1)&
                            /w(ixO^S,rho_))
    f(ixO^S,epsplus_)=(w(ixO^S,mom(idim))+valfven(ixO^S))*w(ixO^S,epsplus_)
    ! Get flux of epsmin
    f(ixO^S,epsmin_)=(w(ixO^S,mom(idim))-valfven(ixO^S))*w(ixO^S,epsplus_)

    ! Get flux of tracer
    do iw=1,mhd_alfven_n_tracer
  ...
  end subroutine mhd_alfven_get_flux

  !> w[iws]=w[iws]+qdt*S[iws,wCT] where S is the source based on wCT within ixO
  subroutine mhd_alfven_add_source(qdt,ixI^L,ixO^L,wCT,w,x,qsourcesplit,active)
    use mod_global_parameters
    ...
    if (.not. qsourcesplit) then
      ...
      if (mhd_alfven_energy .and. block%e_is_internal) then
        active = .true.
      ...
      if (abs(mhd_alfven_eta)>smalldouble)then
        active = .true.
        call add_source_res2(qdt,ixI^L,ixO^L,wCT,w,x)
      end if

      if (mhd_alfven_eta_hyper>0.d0)then
        active = .true.
        call add_source_hyperres(qdt,ixI^L,ixO^L,wCT,w,x)
      end if
      call add_source_afven_propagation(qdt,ixI^L,ixO^L,wCT,w,x)
    end if
    ...
    if(mhd_alfven_viscosity) then
      call viscosity_add_source(qdt,ixI^L,ixO^L,wCT,&
           w,x,mhd_alfven_energy,qsourcesplit,active)
    end if
  ...
  end subroutine mhd_alfven_add_source
  ...
  subroutine internal_energy_add_source(qdt,ixI^L,ixO^L,wCT,w,x)
    ...
    call mhd_alfven_get_v(wCT,x,ixI^L,ixI^L,v)
    call divvector(v,ixI^L,ixO^L,divv)
    call mhd_alfven_get_pthermal(wCT,x,ixI^L,ixO^L,pth)
  ...
  subroutine add_source_B0split(qdt,ixI^L,ixO^L,wCT,w,x)
    ...
    double precision :: wB0(ixI^S,1:ndir),dB0(ixI^S,1:ndir),b1dB0(ixI^S)
    integer :: idir

   if(B0field_reset)then
     call set_B0_cell(wB0,x,ixI^L,ixO^L,global_time+qdt)
     dB0(ixO^S,1:ndir) = (wB0(ixO^S,1:ndir)-block%B0(ixO^S,1:ndir,0))
   end if
    a=0.d0
    b=0.d0
    ...
    if(mhd_alfven_energy) then
      if(.not.block%e_is_internal) then
        ...
        if(B0field_reset)then
          b1dB0(ixO^S) = SUM(wCT(ixO^S,mag(1:ndir))*dB0(ixO^S,1:ndir)&
                               ,dim=ndim+1)
          w(ixO^S,e_)=w(ixO^S,e_)-b1dB0(ixO^S)
        end if
      end if
    end if
    if(B0field_reset)w(ixO^S,mag(1:ndir))=w(ixO^S,mag(1:ndir))&
                                          -dB0(ixO^S,1:ndir)
    if (check_small_values) call mhd_alfven_handle_small_values(.false.,w,x,ixI^L,ixO^L,'add_source_B0')
  ...
  subroutine add_source_res1(qdt,ixI^L,ixO^L,wCT,w,x)
    ...
    if (mhd_alfven_eta>zero)then
       eta(ixA^S)=mhd_alfven_eta
    ...
    if (check_small_values) call mhd_alfven_handle_small_values(.false.,w,x,ixI^L,ixO^L,'add_source_res1')
  ...
  subroutine add_source_res2(qdt,ixI^L,ixO^L,wCT,w,x)
    ...
    if (check_small_values) call mhd_alfven_handle_small_values(.false.,w,x,ixI^L,ixO^L,'add_source_res2')
  ...
  subroutine add_source_hyperres(qdt,ixI^L,ixO^L,wCT,w,x)
    ...
    ehyper(ixA^S,1:ndir) = - tmpvec(ixA^S,1:ndir)*mhd_alfven_eta_hyper
    ...
    if (check_small_values)  call mhd_alfven_handle_small_values(.false.,w,x,ixI^L,ixO^L,'add_source_hyperres')
  ...
  subroutine add_source_glm1(qdt,ixI^L,ixO^L,wCT,w,x)
    ...
    if (mhd_alfven_glm_alpha < zero) then
      w(ixO^S,psi_) = abs(mhd_alfven_glm_alpha)*wCT(ixO^S,psi_)
    else
      ! implicit update of Psi variable
      ! equation (27) in Mignone 2010 J. Com. Phys. 229, 2117
      if(slab) then
        w(ixO^S,psi_) = dexp(-qdt*cmax_global*mhd_alfven_glm_alpha/minval(dxlevel(:)))*w(ixO^S,psi_)
      else
        w(ixO^S,psi_) = dexp(-qdt*cmax_global*mhd_alfven_glm_alpha/minval(block%ds(ixO^S,:),dim=ndim+1))*w(ixO^S,psi_)
      end if
    end if
    ...
    do idim=1,ndim
       ...
       if (mhd_alfven_energy .and. .not.block%e_is_internal) then
       ! e  = e  -qdt (b . grad(Psi))
    ...
    ! m = m - qdt b div b
    do idir=1,ndir
      w(ixO^S,mom(idir))=w(ixO^S,mom(idir))-qdt*mhd_alfven_mag_i_all(w,ixI^L,ixO^L,idir)*divb(ixO^S)
    end do

    if (check_small_values) call mhd_alfven_handle_small_values(.false.,w,x,ixI^L,ixO^L,'add_source_glm1')
  ...
  subroutine add_source_glm2(qdt,ixI^L,ixO^L,wCT,w,x)
    ...
    ! calculate velocity
    call mhd_alfven_get_v(wCT,x,ixI^L,ixO^L,v)
    ...
    if (check_small_values) call mhd_alfven_handle_small_values(.false.,w,x,ixI^L,ixO^L,'add_source_glm')
  ...
  subroutine add_source_powel(qdt,ixI^L,ixO^L,wCT,w,x)
    ...
    if (check_small_values) call mhd_alfven_handle_small_values(.false.,w,x,ixI^L,ixO^L,'add_source_powel')
  ...
  subroutine add_source_janhunen(qdt,ixI^L,ixO^L,wCT,w,x)
    ...
    if (check_small_values) call mhd_alfven_handle_small_values(.false.,w,x,ixI^L,ixO^L,'add_source_janhunen')

  end subroutine add_source_janhunen

  subroutine  add_source_afven_propagation(qdt,ixI^L,ixO^L,wCT,w,x)
    ! Add divB related sources to w within ixO
    ! corresponding to Janhunen, just the term in the induction equation.
    use mod_global_parameters
    ...
    double precision, intent(in)    :: qdt,   wCT(ixI^S,1:nw), x(ixI^S,1:ndim)
    double precision, intent(inout) :: w(ixI^S,1:nw)
    double precision                :: divv(ixI^S),zplus(ixI^S),zmin(ixI^S)
    double precision                :: Qplus(ixI^S),Qmin(ixI^S),Qw(ixI^S),Qh(ixI^S)
    integer                         :: idir

    ! We calculate now div B
    call get_divv(wCT,ixI^L,ixO^L,divv)

    ! b = b - qdt v * div b

    w(ixO^S,epsplus_)= w(ixO^S,epsplus_)-qdt*wCT(ixO^S,epsplus_)/2.0_dp*divv(ixO^S)
    w(ixO^S,epsmin_) = w(ixO^S,epsmin_)-qdt*wCT(ixO^S,epsmin_)/2.0_dp*divv(ixO^S)

    zplus(ixO^S) = dsqrt(8.0_dp*dabs(wCT(ixO^S,epsplus_))/wCT(ixO^S,rho_))
    zmin(ixO^S) = dsqrt(8.0_dp*dabs(wCT(ixO^S,epsmin_))/wCT(ixO^S,rho_))
    Qwplus(ixO^S) = dabs(wCT(ixO^S,epsplus_))*zplus(ixO^S)/mhd_alfven_lambda
    Qwmin(ixO^S) = dabs(wCT(ixO^S,epsmin_))*zmin(ixO^S)/mhd_alfven_lambda
    Qw(ixO^S)    = Qwplus(ixO^S)+Qwmin(ixO^S)


    w(ixO^S,epsplus_)= w(ixO^S,epsplus_)-qdt*Qwplus(ixO^S)
    w(ixO^S,epsmin_) = w(ixO^S,epsmin_)-qdt* Qwmin(ixO^S)

    w(ixO^S,e_) = w(ixO^S,e_)+qdt*Qw(ixO^S)
    if (check_small_values) call mhd_alfven_handle_small_values(.false.,w,x,ixI^L,ixO^L,'add_source_afven_propagation')

  end subroutine   add_source_afven_propagation
  subroutine add_source_linde(qdt,ixI^L,ixO^L,wCT,w,x)
    ...
    do idim=1,ndim
       ...
       if (mhd_alfven_energy .and. typedivbdiff=='all' .and. .not.block%e_is_internal) then
         ! e += B_idim*eta*grad_idim(divb)
         w(ixp^S,e_)=w(ixp^S,e_)+wCT(ixp^S,mag(idim))*graddivb(ixp^S)
       end if
    end do

    if (check_small_values) call mhd_alfven_handle_small_values(.false.,w,x,ixI^L,ixO^L,'add_source_linde')
  ...
  !> Calculate div V within ixO
  subroutine get_divv(w,ixI^L,ixO^L,divv)
    ...
    vvec(ixI^S,:)=w(ixI^S,mom(:))/w(ixO^S,rho_)

    select case(typediv)
    case("central")
      call divvector(vvec,ixI^L,ixO^L,divv)
    case("limited")
      call divvectorS(vvec,ixI^L,ixO^L,divv)
    end select
  end subroutine get_divv
  ...
  subroutine get_normalized_divb(w,ixI^L,ixO^L,divb)
    ...
    if(slab) then
      divb(ixO^S)=0.5d0*abs(divb(ixO^S))/sqrt(mhd_alfven_mag_en_all(w,ixI^L,ixO^L))/sum(1.d0/dxlevel(:))
    else
      ixAmin^D=ixOmin^D-1;
      ixAmax^D=ixOmax^D-1;
      ...
      divb(ixO^S)=abs(divb(ixO^S))/sqrt(mhd_alfven_mag_en_all(w,ixI^L,ixO^L))*&
  ...
  subroutine mhd_alfven_get_dt(w,ixI^L,ixO^L,dtnew,dx^D,x)
    use mod_global_parameters
    use mod_usr_methods
    ...
    ^D&dxarr(^D)=dx^D;
    if (mhd_alfven_eta>zero)then
       dtnew=dtdiffpar*minval(dxarr(1:ndim))**2/mhd_alfven_eta
    else if (mhd_alfven_eta<zero)then
    ...
    if(mhd_alfven_eta_hyper>zero) then
      if(slab) then
        dtnew=min(dtdiffpar*minval(dxarr(1:ndim))**4/mhd_alfven_eta_hyper,dtnew)
      else
        dtnew=min(dtdiffpar*minval(block%ds(ixO^S,1:ndim))**4/mhd_alfven_eta_hyper,dtnew)
      end if
    end if
  ...
  end subroutine mhd_alfven_get_dt

  ! Add geometrical source terms to w
  subroutine mhd_alfven_add_source_geom(qdt,ixI^L,ixO^L,wCT,w,x)
    use mod_global_parameters
    ...
    case ('cylindrical')
      if (angmomfix) then
       ...
       call mhd_alfven_get_p_total(wCT,x,ixI^L,ixO^L,tmp)
       if(phi_>0) then
       ...
       if(mhd_alfven_glm) w(ixO^S,br_)=w(ixO^S,br_)+qdt*wCT(ixO^S,psi_)/x(ixO^S,1)
    case ('spherical')
       h1x^L=ixO^L-kr(1,^D); {^NOONED h2x^L=ixO^L-kr(2,^D);}
       call mhd_alfven_get_p_total(wCT,x,ixI^L,ixO^L,tmp1)
  ...
  end subroutine mhd_alfven_add_source_geom

  !> Compute 2 times total magnetic energy
  function mhd_alfven_mag_en_all(w, ixI^L, ixO^L) result(mge)
    use mod_global_parameters
    integer, intent(in)           :: ixI^L, ixO^L
    double precision, intent(in)  :: w(ixI^S, nw)
  ...
  end function mhd_alfven_mag_en_all

  !> Compute full magnetic field by direction
  function mhd_alfven_mag_i_all(w, ixI^L, ixO^L,idir) result(mgf)
    use mod_global_parameters
  ...
  end function mhd_alfven_mag_i_all

  !> Compute evolving magnetic energy
  function mhd_alfven_mag_en(w, ixI^L, ixO^L) result(mge)
  ...
  end function mhd_alfven_mag_en

  !> compute kinetic energy
  function mhd_alfven_kin_en(w, ixI^L, ixO^L, inv_rho) result(ke)
    use mod_global_parameters, only: nw, ndim
    integer, intent(in)           :: ixI^L, ixO^L
    double precision, intent(in)  :: w(ixI^S, nw)
  ...
  end function mhd_alfven_kin_en

  subroutine mhd_alfven_getv_Hall(w,x,ixI^L,ixO^L,vHall)
    use mod_global_parameters
    ...
    vHall(ixO^S,idirmin:3) = - mhd_alfven_etah*current(ixO^S,idirmin:3)
    do idir = idirmin, 3
       vHall(ixO^S,idir) = vHall(ixO^S,idir)/w(ixO^S,rho_)
    end do

  end subroutine mhd_alfven_getv_Hall

  subroutine mhd_alfven_getdt_Hall(w,x,ixI^L,ixO^L,dx^D,dthall)
    use mod_global_parameters
    ...
    if(slab) then
      dthall=dtdiffpar*minval(dxarr(1:ndim))**2.0d0/(mhd_alfven_etah*maxval(bmag(ixO^S)/w(ixO^S,rho_)))
    else
      dthall=dtdiffpar*minval(block%ds(ixO^S,1:ndim))**2.0d0/(mhd_alfven_etah*maxval(bmag(ixO^S)/w(ixO^S,rho_)))
    end if

  end subroutine mhd_alfven_getdt_Hall
  ...
  subroutine mhd_alfven_boundary_adjust
    use mod_global_parameters
  ...
  end subroutine mhd_alfven_boundary_adjust
  ...
  subroutine fixdivB_boundary(ixG^L,ixO^L,w,x,iB)
    ...
    select case(iB)
     case(1)
       ...
       if(mhd_alfven_energy.and..not.block%e_is_internal) call mhd_alfven_to_primitive(ixG^L,ixO^L,w,x)
       {^IFTWOD
       ixFmin1=ixOmin1+1
       ixFmax1=ixOmax1+1
       ixFmin2=ixOmin2+1
       ...
       if(mhd_alfven_energy.and..not.block%e_is_internal) call mhd_alfven_to_conserved(ixG^L,ixO^L,w,x)
     case(2)
       if(mhd_alfven_energy.and..not.block%e_is_internal) call mhd_alfven_to_primitive(ixG^L,ixO^L,w,x)
       {^IFTWOD
       ixFmin1=ixOmin1-1
...
end module mod_mhd_alfven_phys
