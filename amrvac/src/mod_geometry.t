...
contains
  ...
  subroutine set_coordinate_system(geom)
    ...
    case ("spherical_1.75D")
      if (ndim /= 1) &
           call mpistop("Geometry spherical_1.75D requires ndim == 2")
      ndir = 3
      r_   = 1; x_     = 1
      phi_ = 3; y_     = 3
      z_   = 2; theta_ = 2
...
{^IFTHREED
 subroutine  transV_cartesian_to_spherical(ixG^L,ixO^L,x,v_cart,v_sphe)

    use mod_global_parameters
    integer, intent(in)               :: ixG^L,ixO^L
    real(dp), intent(in)              :: v_cart(1:ndim), x(ixG^S,1:ndim)
    real(dp), intent(inout)           :: v_sphe(ixG^S,1:ndim)

    real(dp), dimension(ixO^S,1:ndim) :: eR,etheta,ePhi
    integer                           :: idim
!---------------------------------------------------------------------------
    eR(ixO^S,x_) =  dsin(x(ixO^S,theta_))*dcos(x(ixO^S,phi_))
    eR(ixO^S,y_) =  dsin(x(ixO^S,theta_))*dsin(x(ixO^S,phi_))
    eR(ixO^S,z_) =  dcos(x(ixO^S,theta_))


    eTheta(ixO^S,x_) =   dcos(x(ixO^S,theta_))*dcos(x(ixO^S,phi_))
    eTheta(ixO^S,y_) =   dcos(x(ixO^S,theta_))*dsin(x(ixO^S,phi_))
    eTheta(ixO^S,z_) =  -dsin(x(ixO^S,theta_))



    ePhi(ixO^S,x_) =  -dsin(x(ixO^S,phi_))
    ePhi(ixO^S,y_) =   dcos(x(ixO^S,phi_))
    ePhi(ixO^S,z_) =   0.0_dp


    v_sphe(ixO^S,r_) =  eR(ixO^S,1)*v_cart(1)
    v_sphe(ixO^S,theta_) =eTheta(ixO^S,1)*v_cart(1)
    v_sphe(ixO^S,phi_) = ePhi(ixO^S,1)*v_cart(1)

    Loop_dim : do idim=2,ndir
      v_sphe(ixO^S,r_) = v_sphe(ixO^S,r_) +eR(ixO^S,idim)*v_cart(idim)
      v_sphe(ixO^S,theta_) = v_sphe(ixO^S,theta_) +eTheta(ixO^S,idim)*v_cart(idim)
      v_sphe(ixO^S,phi_) = v_sphe(ixO^S,phi_) + ePhi(ixO^S,idim)*v_cart(idim)
    end do Loop_dim
end subroutine  transV_cartesian_to_spherical
}
