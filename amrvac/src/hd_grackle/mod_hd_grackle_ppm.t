...
contains
  ...
  subroutine hd_ppm_flatcd(ixI^L,ixO^L,ixL^L,ixR^L,w,d2w,drho,dpres)
  ...
  subroutine hd_ppm_flatsh(ixI^L,ixO^L,ixLL^L,ixL^L,ixR^L,ixRR^L,idims,w,drho,dpres,dv)
    use mod_global_parameters
    use mod_geometry
    ...
    double precision, intent(inout) :: drho(ixG^T), dpres(ixG^T), dv(ixG^T)
