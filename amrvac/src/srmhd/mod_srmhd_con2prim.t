...
subroutine srmhd_con2prim(d,tau,flag_use_old,Bsqr,Ssqr,sdotb,sb2,E2,&
                          lfac,xi,ierror)
...
cond_useold_0 : if(flag_use_old)then
 if (xi>smalldouble) then
    ...
    call srmhd_funcd(xi,fin,dfin,lfacin,d,ssqr,tau,taud,bsqr,sdotb,sb2,E2,ierror)
...
call srmhd_initialise_con2prim(d,tau,taud,small_pressure,bsqr,ssqr,sdotb,&
                               E2,lfacmin,lfacmax,dplus,xi1,lfacl,ierror)
...
cond_sdotB : if(dabs(sdotb)<srmhd_limitvalue)then
 ...
 xi1bsqr=xi1+bsqr
 vsqr = ((ssqr + ((xi1+xi1bsqr)*sdotb**2.0d0)/xi1**2.0d0)&
      / (xi1bsqr**2.0d0))
 ...
 cond_speed1 : if(vsqr > srmhd_maxspeed2 )then
  ...
  LoopVmax:  do ni = 1,srmhd_maxiterationNR
      ...
      xi1bsqr=xi1+bsqr
      ! v^2(xi1) - maxvel^2
      fv= (ssqr + (xi1+xi1bsqr)*sb2/xi1**2.0d0)&
        /xi1bsqr**2.0d0-srmhd_maxspeed2
     ...
     ! TESSTSSS=========
     vsqr = ((ssqr + ((xi1+xi1bsqr)*sb2)/xi1**2.0d0)&
       /(xi1bsqr**2.0d0))
     if(vsqr<srmhd_maxspeed2)then
...
if(niiter>1)call srmhd_funcd(xi1,fl,df,lfacl,d,ssqr,tau,taud,bsqr,sdotb,sb2,E2,ierror)
...
call  Newton_Ralphson(srmhd_funcd,d,ssqr,tau,taud,bsqr,sdotb,sb2,E2,lfacmax,dplus, &
                           xil,xih,xii,fl,fh,xi,lfac,ierror)
...
!================================================================================
subroutine Newton_Ralphson(fmhd_l,d,ssqr,tau,taud,bsqr,sdotb,sb2,E2,lfacmax,dplus, &
                           xil,xih,xii,fl,fh,xi,lfac,ierror)
...
real(kind=dp)   , intent(in)    :: d,ssqr,tau,taud,bsqr,sdotb,sb2,E2&
                                   ,lfacmax,dplus
...
LoopNRRMHD :  do ni=1,srmhd_maxiterationNR
      ...
      er=dabs(f/df)/xii
      cond_converge : if((er<srmhd_tolernr*er1).or.(dabs(f/df)<srmhd_absaccnr))then
            call fmhd_l(xii,f,df,lfaci,d,ssqr,tau,taud,&
                                  bsqr,sdotb,sb2,E2,ierror)
            tests=(xii>=dplus*lfaci.and.lfaci<=lfacmax)
            if(tests)then
...
integer, intent(inout)          :: ierror
! .. local ..
real(kind=dp)                   :: xibsqr,taudp,F,dF,lfacn,&
                                   dxidlfac,er,er1,rho,rhoh,a0,b0,x0,dx0
integer                         :: ni,n_h
...
elseif(a0>1.0_dp/(max(smallratio,smalldouble)))then
...
subroutine srmhd_con2prim_hd(d,tau,taud,flag_use_old,Ssqr,&
                          lfac,pressure,ierror)
!use ieee_arithmetic
use mod_global_parameters
implicit none
...
! check old auxilairies
 ...
 LoopNR:  do ni=1,srmhd_maxiterationNR
  ...
  cond_noconverge1: if(pnew == pLabs .and. pcurrent==pnew .and. &
        abs(ff)> srmhd_absaccnr .and. Ssqr > 0.0_dp)then
        pnewi=pnew
    ...
    enddo LoopCor
        ...
        if( Nff*ff>0.0_dp)then
           ...
           pressure = (srmhd_gamma-1.0_dp)*2.0d0*(1.0_dp + 10.0d0 * small_e) * small_e
           lfac     = 1.0_dp
           ierror=7
  ...
  if((dabs(oldff2-ff) < 1.0d-8 .or. niiter >= srmhd_maxiterationNR-srmhd_maxiterationNR/20).and.&
  ff * oldff1 < 0.0_dp    .and.  dabs(ff)>srmhd_absaccnr)then
