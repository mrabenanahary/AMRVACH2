...
contains
  ...
  subroutine srmhd_check_w(primitive,ixI^L,ixO^L,w,flag)
    ...
    cond_energy : if (srmhd_energy) then
       cond_einternal : if (block%e_is_internal) then
          where(w(ixO^S, e_) < small_pressure*inv_srmhd_gamma_1) flag(ixO^S) = e_
       else cond_einternal
  ...
  subroutine srmhd_to_conserved(ixI^L,ixO^L,w,x)
    ...
    ! .. local ..
    integer                            :: idir, itr
    real(kind=dp)   , dimension(ixO^S) :: sqrU,B2,VdotB,rhoh,sqrV
    ...
    sqrV=sqrU/w(ixO^S,lfac_)**2.0_dp
  ...
  subroutine srmhd_to_primitive(ixI^L,ixO^L,w,x)
    ...
    real(kind=dp)   , intent(in)    :: x(ixI^S, 1:ndim)
    ! .. local ..
    real(kind=dp)                   :: inv_rho(ixO^S),B2(ixO^S),VdotB(ixO^S)
    integer                         :: itr, idir
    character(len=30)               :: subname_loc
    !--------------------------------------------------------
    ...
    if (srmhd_energy) then
      ...
      if(.not.block%e_is_internal) then
        call srmhd_get_pressure_fromprimitive(ixI^L,ixO^L,w,inv_rho)
        w(ixO^S,p_)   = inv_rho(ixO^S)
      else
       w(ixO^S,p_)   = srmhd_gamma_1*w(ixO^S, e_)
      end if
    end if
    ! re-use inv_rho to store inverse of the density
    inv_rho(ixO^S) = 1.0_dp / w(ixO^S, rho_)

    call srmhd_get_B2andVdotB(ixI^L,ixO^L,w,.true.,B2=B2,VdotB=VdotB)
    ...
    if (any(flag(ixO^S) /= 0)) then
       ...
       case ("average")
          ...
          if(small_values_force_floor.and.crash)then
            call srmhd_small_values_floor(primitive, w, x, ixI^L, ixO^L, flag)
          end if
       case default
  ...
  subroutine srmhd_small_values_floor(primitive, w, x, ixI^L, ixO^L, flag)
    ! made by Z. Meliani 10/02/2018
    use mod_global_parameters
    use mod_small_values
    logical, intent(in)             :: primitive
    ...
    real(kind=dp)   , intent(inout) :: w(ixI^S,1:nw)
    real(kind=dp)   , intent(in)    :: x(ixI^S,1:ndim)
    integer, optional, intent(in)   :: flag(ixO^S)
    ! .. local ..
    integer                         :: idir
    real(kind=dp)                   :: smallone
    !-------------------------------------------------------------
 ...
 !> Calculate thermal pressure for enthalpy and density
  subroutine srmhd_get_pressure_fromprimitive(ixI^L,ixO^L,w,pth)
    ...
    real(kind=dp)                  :: rhoh(ixO^S)
    !--------------------------

    rhoh(ixO^S)  = w(ixO^S,xi_)/w(ixO^S,lfac_)**2.0_dp
  ...
  subroutine srmhd_get_pthermal(w,x,ixI^L,ixO^L,pth)
    ...
    real(kind=dp)                   :: rho(ixO^S),rhoh(ixO^S)
 ...
 !> Calculate the square of the thermal sound speed csound2 within ixO^L.
  ...
  subroutine srmhd_get_csound2_prim(w,x,ixI^L,ixO^L,csound2)
    ...
    real(kind=dp)   ,dimension(ixO^S) :: rhoh
  ...
  subroutine e_to_rhos(ixI^L,ixO^L,w,x)
    ...
    if (srmhd_energy) then
      ...
      w(ixO^S, e_) = srmhd_gamma_1* w(ixO^S, rho_)**(1.0d0 - srmhd_gamma) * &
            w(ixO^S, e_)
    else
  ...
  subroutine rhos_to_e(ixI^L,ixO^L,w,x)
    use mod_global_parameters
    integer, intent(in) :: ixI^L, ixO^L
    real(kind=dp)    :: w(ixI^S,nw)
    real(kind=dp)   , intent(in)    :: x(ixI^S,1:ndim)

    if (srmhd_energy) then
       w(ixO^S, e_) = w(ixO^S, rho_)**srmhd_gamma_1 * w(ixO^S, e_) &
            * inv_srmhd_gamma_1
       if(.not.block%e_is_internal) &
         w(ixO^S, e_) =w(ixO^S, e_) + srmhd_kin_en_primitive(w, ixI^L, ixO^L) + &
            srmhd_mag_en_primitive(w, ixI^L, ixO^L)
  ...
  subroutine srmhd_get_v(w,x,ixI^L,ixO^L,v,B2,VdotB)
    ...
    integer, intent(in)           :: ixI^L, ixO^L
    real(kind=dp)   , intent(in)  :: w(ixI^S,1:nw), x(ixI^S,1:ndim)
    real(kind=dp)   , intent(out) :: v(ixI^S,1:ndir)

    real(kind=dp)   , optional, intent(in)  :: VdotB(ixO^S),B2(ixO^S)
    real(kind=dp)                           :: sub_VdotB(ixO^S)&
                                              ,sub_B2(ixO^S)
    integer :: idir
  ...
  subroutine srmhd_get_v_idim(w,x,ixI^L,ixO^L,idim,v_idim)
    ...
    real(kind=dp)   , intent(out)          :: v_idim(ixI^S)

    real(kind=dp)                          :: sub_VdotB(ixO^S),sub_B2(ixO^S)
  ...
  subroutine srmhd_get_v_idim_loc(w,x,ixI^L,ixO^L,idim,v_idim,B2,VdotB)
    ...
    integer, intent(in)                    :: ixI^L, ixO^L, idim
    real(kind=dp)   , intent(in)           :: w(ixI^S,nw), x(ixI^S,1:ndim)
    real(kind=dp)   , intent(out)          :: v_idim(ixO^S)
    real(kind=dp)   , optional, intent(in) :: VdotB(ixO^S),B2(ixO^S)
 ...
 !> Calculate v^2
  subroutine srmhd_get_v2(w,x,ixI^L,ixO^L,v2,B2,VdotB)
    ...
    integer, intent(in)           :: ixI^L, ixO^L
    real(kind=dp)   , intent(in)  :: w(ixI^S,nw), x(ixI^S,1:ndim)
    real(kind=dp)   , intent(out) :: v2(ixO^S)

    real(kind=dp)   , optional, intent(in)  :: VdotB(ixO^S),B2(ixO^S)
    real(kind=dp)                           :: sub_VdotB(ixO^S),sub_B2(ixO^S)
    real(kind=dp)                           :: v_num(ixO^S,1:ndir)
    integer :: idir
  ...
  subroutine srmhd_get_cmax_gammie(ixI^L,ixO^L,idim,x,w,rhoh,B2,&
    ...
    integer, intent(in)          :: ixI^L, ixO^L, idim
    real(kind=dp)   , intent(in) :: w(ixI^S, nw), x(ixI^S,1:ndim)
    real(kind=dp)   , intent(in), dimension(ixO^S) :: rhoh,B2,VdotB&
                                                     ,vidim,calfven, csound2
    real(kind=dp)   , intent(inout)          :: cmax(ixI^S)
    real(kind=dp)   , optional, intent(inout):: cmin(ixI^S)
    logical, optional, intent(in)            :: from_cbound
    logical, optional, intent(in)            :: patch_gammie(ixO^S)
    ! .. local ..
    real(kind=dp)   , dimension(ixO^S):: A,B
    real(kind=dp)   , dimension(ixO^S):: v2
    !-----------------------------------------------------------
  ...
  !                                      vidim(ixO^S)=dabs(vidim(ixO^S))
    ...
    cond_onedir: if(ndir==1)then
     ...
     cmax(ixO^S)=(vidim(ixO^S)*(1.0_dp-A)&
                  +dsqrt(A*(1.0_dp-v2)*((1.0_dp-v2*A)&
                         -B*(1.0_dp-A))))&
                 /( 1.0_dp-v2*A)
  ...
  subroutine srmhd_get_cmax(w,x,ixI^L,ixO^L,idim,cmax)
    ...
    real(kind=dp)   , intent(inout)   :: cmax(ixI^S)
    ! .. local ..
    real(kind=dp)   , dimension(ixO^S):: rhoh,B2,VdotB,calfven2,vidim
    real(kind=dp)   , dimension(ixO^S):: v2,csound2
    !-------------------------------------------------------
    ...
    call srmhd_get_B2andVdotB(ixI^L,ixO^L,w,.true.,B2=B2,VdotB=VdotB)
    call srmhd_get_calfven2(ixI^L,ixO^L,x,w,.true.,calfven2,rhoh=rhoh,B2=B2)
    call srmhd_get_v_idim_loc(w,x,ixI^L,ixO^L,idim,vidim,B2=B2,VdotB=VdotB)

    call srmhd_get_cmax_gammie(ixI^L,ixO^L,idim,x,w,rhoh,B2,&
                               VdotB,calfven2,dabs(vidim),&
                               csound2,cmax)
  ...
  subroutine srmhd_get_cbounds(wLC,wRC,wLp,wRp,x,ixI^L,ixO^L,idim,cmax,cmin)
    ...
    ! .. local ..
    real(kind=dp)                      :: wmean(ixI^S,nw)
    real(kind=dp)   , dimension(ixO^S) :: umean, dmean, csound2Lp, &
    ...
    if (typeboundspeed/='cmaxmean') then
     ...
     call srmhd_get_cmax_gammie(ixI^L,ixO^L,idim,x,wLp,rhoh=rhohLp,B2=B2Lp,&
                                VdotB=VdotBLp,calfven=calfvenLp,vidim=dabs(vidimLp),&
                                csound2=csound2Lp,cmax=cmaxL)

     call srmhd_get_cmax_gammie(ixI^L,ixO^L,idim,x,wRp,rhoh=rhohRp,B2=B2Rp,&
                                VdotB=VdotBRp,calfven=calfvenRp,vidim=dabs(vidimRp),&
      ...
      wmean(ixO^S,1:nw)=0.5_dp*(wLC(ixO^S,1:nw)+wRC(ixO^S,1:nw))
      ! get auxiliary variables
  ...
  subroutine srmhd_get_calfven2(ixI^L,ixO^L,x,w,conserve,calfven,rhoh,B2)
   ...
   real(kind=dp)   , intent(out) :: calfven(ixO^S)
   real(kind=dp)   , optional    :: rhoh(ixO^S),B2(ixO^S)

   real(kind=dp)                 :: sub_B2(ixO^S),sub_rhoh(ixO^S)
...
!  end subroutine srmhd_get_csound
  ...
  subroutine srmhd_get_csound_prim(w,x,ixI^L,ixO^L,idim,csound)
    ...
    if(srmhd_energy) then
      ...
      csound(ixO^S)=dsqrt(srmhd_gamma*srmhd_adiab*w(ixO^S,rho_)**srmhd_gamma_1)
    end if
 ...
 !> Calculate magnetic pressure within ixO^L including magnetic pressure
  subroutine srmhd_get_p_mag(w,ixI^L,ixO^L,pmag)
    ...
    integer, intent(in)             :: ixI^L, ixO^L
    real(kind=dp)   , intent(in)    :: w(ixI^S,nw)
    real(kind=dp)   , intent(out)   :: pmag(ixI^S)
  ...
  subroutine srmhd_get_csound2(w,x,ixI^L,ixO^L,rhoh,csound2)
    ...
    use mod_global_parameters
    integer, intent(in)             :: ixI^L, ixO^L
    real(kind=dp)   , intent(in)    :: w(ixI^S,nw),rhoh(ixO^S)
    real(kind=dp)   , intent(in)    :: x(ixI^S,1:ndim)
    real(kind=dp)   , intent(out)   :: csound2(ixO^S)

    real(kind=dp)                   :: rho(ixO^S)

    if(srmhd_energy) then
      rho=w(ixO^S,d_)/w(ixO^S,lfac_)
      call srmhd_get_csound2_eos(ixI^L,ixO^L,x,rho,rhoh,csound2)
    else
      csound2(ixO^S)=srmhd_gamma*srmhd_adiab*rho(ixO^S)**srmhd_gamma_1
    end if
  ...
  subroutine srmhd_get_p_total(w,x,ixI^L,ixO^L,p,B2,VdotB)
    ...
    real(kind=dp)   , intent(in)    :: w(ixI^S,nw)
    real(kind=dp)   , intent(in)    :: x(ixI^S,1:ndim)
    real(kind=dp)   , intent(out)   :: p(ixI^S)
    real(kind=dp)   , optional, intent(in) :: B2(ixO^S),VdotB(ixO^S)

    real(kind=dp)                   :: pmag(ixI^S)
  ...
  subroutine srmhd_get_pmag(w,x,ixI^L,ixO^L,pmag,B2,VdotB)
   ...
   implicit none
    ...
    real(kind=dp)   , dimension(ixO^S)     :: sub_B2,sub_VdotB
  ...
  subroutine srmhd_get_B2andVdotB(ixI^L,ixO^L,w,wconserve,B2,VdotB)
    ...
    real(kind=dp)   ,optional, intent(inout) :: B2(ixO^S),VdotB(ixO^S)
  ...
  subroutine srmhd_get_flux(wC,wP,x,ixI^L,ixO^L,idim,f)
    ...
    integer, intent(in)          :: ixI^L, ixO^L, idim
    ! conservative w
    real(kind=dp)   , intent(in) :: wC(ixI^S,nw)
    ! primitive w
    real(kind=dp)   , intent(in) :: wP(ixI^S,nw)
    real(kind=dp)   , intent(in) :: x(ixI^S,1:ndim)
    real(kind=dp)   ,intent(out) :: f(ixI^S,nwflux)

    real(kind=dp)                :: ptotal(ixI^S)
    real(kind=dp)                :: B2(ixO^S),VdotB(ixO^S)
    real(kind=dp)                :: v(ixI^S,1:ndir)
    ...
    Loop_idir_mom : do idir=1,ndir
      f(ixO^S,mom(idir))= v(ixO^S,idir)*wC(ixO^S,mom(idim))&
                         -wP(ixO^S,mag(idir))*(VdotB(ixO^S)*v(ixO^S,idim)&
...
!    end if
  ...
  subroutine internal_energy_add_source(qdt,ixI^L,ixO^L,wCT,w,x)
    ...
    real(kind=dp)   , intent(in)    :: wCT(ixI^S,1:nw), x(ixI^S,1:ndim)
    real(kind=dp)   , intent(inout) :: w(ixI^S,1:nw)
    real(kind=dp)                   :: pth(ixI^S),v(ixI^S,1:ndir),divv(ixI^S)
  ...
  subroutine add_source_glm1(qdt,ixI^L,ixO^L,wCT,w,x)
    ...
    integer, intent(in) :: ixI^L, ixO^L
    real(kind=dp)   , intent(in) :: qdt, wCT(ixI^S,1:nw), x(ixI^S,1:ndim)
    real(kind=dp)   , intent(inout) :: w(ixI^S,1:nw)
    real(kind=dp)   :: divb(ixI^S)
    integer          :: idim,idir
    real(kind=dp)    :: gradPsi(ixI^S)
  ...
  subroutine add_source_glm2(qdt,ixI^L,ixO^L,wCT,w,x)
    ...
    real(kind=dp)   , intent(in) :: qdt, wCT(ixI^S,1:nw), x(ixI^S,1:ndim)
    real(kind=dp)   , intent(inout) :: w(ixI^S,1:nw)
    real(kind=dp)   :: divb(ixI^S),v(ixI^S,1:ndir)
    integer          :: idim,idir
    real(kind=dp)    :: gradPsi(ixI^S,ndim), Bf(ixI^S,1:ndir)
  ...
  subroutine add_source_linde(qdt,ixI^L,ixO^L,wCT,w,x)
    ...
    real(kind=dp)    :: divb(ixI^S),graddivb(ixI^S)
  ...
  subroutine get_divb(w,ixI^L,ixO^L,divb)
    ...
    real(kind=dp)                      :: bvec(ixI^S,1:ndir)
  ...
  subroutine get_normalized_divb(w,ixI^L,ixO^L,divb)
    ...
    integer, intent(in)                :: ixI^L, ixO^L
    real(kind=dp)   , intent(in)       :: w(ixI^S,1:nw)
    real(kind=dp)                      :: divb(ixI^S), dsurface(ixI^S)
  ...
  subroutine get_current(w,ixI^L,ixO^L,idirmin,current)
    ...
    real(kind=dp)    :: current(ixI^S,7-2*ndir:3),bvec(ixI^S,1:ndir)
  ...
  subroutine srmhd_get_dt(w,ixI^L,ixO^L,dtnew,dx^D,x)
    ...
    real(kind=dp)                 :: current(ixI^S,7-2*ndir:3),eta(ixI^S)
  ...
  subroutine srmhd_add_source_geom(qdt,ixI^L,ixO^L,wCT,w,x)
    ...
    real(kind=dp)    :: tmp(ixI^S),ptot(ixI^S),tmp2(ixI^S)
    real(kind=dp)    :: B2(ixO^S),VdotB(ixO^S),VdotB2(ixO^S),xiplusB2(ixO^S)
      ...
      if (angmomfix) then
       ...
       if(phi_>0) then
         w(ixO^S,mr_)=w(ixO^S,mr_)+qdt/x(ixO^S,1)*(ptot(ixO^S)+&
                   ...
                   /xiplusB2(ixO^S)-(wCT(ixO^S,bphi_)/wCT(ixO^S,lfac_))**2.0)
  ...
  function srmhd_kin_en_primitive(w, ixI^L, ixO^L, inv_rho) result(ke)
    ...
    integer, intent(in)           :: ixI^L, ixO^L
    real(kind=dp)   , intent(in)  :: w(ixI^S, nw)
    real(kind=dp)                 :: ke(ixO^S)
    real(kind=dp)   , intent(in), optional :: inv_rho(ixO^S)
...
!  end subroutine srmhd_getdt_Hall
  ...
  subroutine fixdivB_boundary(ixG^L,ixO^L,w,x,iB)
    ...
    real(kind=dp)   , intent(inout) :: w(ixG^S,1:nw)
    real(kind=dp)   , intent(in) :: x(ixG^S,1:ndim)
   ...
   subroutine srmhd_get_auxiliary(clipping,w,x,ixI^L,ixO^L,subname,&
    ...
    real(kind=dp),target                  :: SdotB_loc(ixO^S),sqrB_loc(ixO^S)
    real(kind=dp), dimension(ixO^S)       :: sqrS,SdotB2,E2
    real(kind=dp), dimension(ixO^S,1:ndir):: tmp,v
    !---------------------------------------------------------------------



where(dabs(w(ixO^S,mag(3)))<smalldouble)w(ixO^S,mag(3))=0.0_dp
where(dabs(w(ixO^S,mag(1)))<smalldouble)w(ixO^S,mag(1))=0.0_dp
where(dabs(w(ixO^S,mag(2)))<smalldouble)w(ixO^S,mag(2))=0.0_dp
    ...
    if(present(sqrB))then
      ...
      sqrB_loc=SUM(w(ixO^S,mag(1):mag(ndir))**2.0_dp,dim=ndim+1)
      sqrB_in=>sqrB_loc
    end if
    ...
    if(present(SdotB))then
      ...
      SdotB_loc=SUM(w(ixO^S,mom(1):mom(ndir))*w(ixO^S,mag(1):mag(ndir)),dim=ndim+1)
      SdotB_in=>SdotB_loc
    end if

    sqrS(ixO^S)  = sum(w(ixO^S,mom(1):mom(ndir))**2.0_dp,dim=ndim+1)
    SdotB2(ixO^S)= SdotB_in(ixO^S)**2.0_dp
    E2(ixO^S)    = sqrB_in(ixO^S)*sqrS(ixO^S)-SdotB2(ixO^S)
   ...
   !PRINT*,' is wrong ',ix^D,w(ix^D,:)
    call srmhd_con2prim(w(ix^D,d_),w(ix^D,e_),&
             use_oldaux_in(ix^D),&
             sqrB_in(ix^D),sqrS(ix^D),SdotB_in(ix^D),SdotB2(ix^D),E2(ix^D),&
             w(ix^D,lfac_),w(ix^D,xi_),ierror)
   ...
   {enddo^D&\}
    ...
    ! compute v*(xi+B^2)
    cond_isflow : if(any(sqrS(ixO^S) /= zero.and.flag_error(ixO^S)/=0))then
        where(sqrS(ixO^S)/= zero.and.flag_error(ixO^S)/=0)
          E2(ixO^S) =sdotb_in(ixO^S)/w(ixO^S,xi_)
        end where
        Loop_idir0 : do idir=1,ndir
         where(sqrS(ixO^S)/= zero.and.flag_error(ixO^S)/=0)
           tmp(ixO^S,idir)=w(ixO^S,mom(idir)) &
            + w(ixO^S,mag(idir))*E2(ixO^S)
         end where
        end do Loop_idir0
        where(sqrS(ixO^S)/= zero.and.flag_error(ixO^S)/=0)
        ! reuse  E2,SdotB2
         E2(ixO^S)     = dsqrt(sum(tmp(ixO^S,1:ndir)**2.0_dp,dim=ndim+1))
         SdotB2(ixO^S) = dsqrt(1.0_dp-1.0_dp/(w(ixO^S,lfac_)**2))
         E2(ixO^S)     = SdotB2(ixO^S)/E2(ixO^S)
        endwhere
        Loop_idir : do idir=1,ndir
         where(sqrS(ixO^S)/= zero.and.flag_error(ixO^S)/=0)
           v(ixO^S,idir)=E2(ixO^S)*tmp(ixO^S,idir)
         end where
        end do  Loop_idir
        where(sqrS(ixO^S)/= zero.and.flag_error(ixO^S)/=0)
         E2(ixO^S)=sum(v(ixO^S,1:ndir)**2.0_dp,dim=ndim+1)
        end where
        where(sqrS(ixO^S)/= zero.and.flag_error(ixO^S)/=0)
         where(E2(ixO^S)>one)flag_error(ixO^S)=6
        end where
    end if cond_isflow
...
! end subroutine srmhd_get_auxiliary_loc
   ...
   subroutine srmhd_get_4u_from_3v(ixI^L,ixO^L,vtou,lfac)
    ! made by Z. Meliani 13/02/2018
    use mod_global_parameters
    implicit none
    integer, intent(in)             :: ixI^L,ixO^L
    ...
    lfac(ixO^S)= 1.0d0/dsqrt(1.0d0-sum(vtou(ixO^S,1:ndir)**2.0,dim=ndim+1))
    Loop_idir: do idir=1,ndir
   ...
   subroutine srmhd_get_3v_from_4u(ixI^L,ixO^L,lfac,vtou)
    ! made by Z. Meliani 13/02/2018
    use mod_global_parameters
    implicit none
    integer, intent(in)             :: ixI^L,ixO^L
    ...
    Loop_idir: do idir=1,ndir
     vtou(ixO^S,idir)=vtou(ixO^S,idir)/lfac(ixO^S)
    end do Loop_idir
