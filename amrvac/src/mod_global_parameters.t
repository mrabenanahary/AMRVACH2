...
module mod_global_parameters
  ...
  real(kind=dp)   , dimension(:), allocatable :: dg^D
  ...
  logical          :: small_getdt_average =.false.
  ...
  type fluxalloc
     real(kind=dp)   , dimension(:^D&,:), pointer:: flux => null()
  end type fluxalloc
  ...
  logical, allocatable, target, save    :: grid_logical_false(:^D&)
  logical, allocatable, target, save    :: grid_logical_true(:^D&)
  !$OMP THREADPRIVATE(dxlevel)
  !$OMP THREADPRIVATE(saveigrid)
  !$OMP THREADPRIVATE(typelimiter,typegradlimiter)
  type mesh_config
    real(kind=dp)   :: box_limit(2,ndim)
    integer         :: domain_npoint(ndim)
    integer         :: block_npoint(ndim)
    integer         :: nbufferx(ndim)
    !> The maximum number of grid blocks in a processor
    integer         :: max_blocks
    ...
    integer         :: refine_max_level
    integer         :: refine_criterion
    real(kind=dp)   :: refine_threshold(nlevelshi)
    real(kind=dp)   :: derefine_ratio(nlevelshi)
    real(kind=dp)   :: w_refine_weight(nlevelshi)
    real(kind=dp)   :: amr_wavefilter(nlevelshi)
...
contains
  ...
  pure subroutine cross_product(ixI^L,ixO^L,a,b,axb)
    ...
    real(kind=dp)   , intent(in) :: a(ixI^S,3), b(ixI^S,3)
    real(kind=dp)   , intent(out) :: axb(ixI^S,3)
    !-------------------------------------------------------------------------
