...
contains
  ...
  self%myconfig%chemical_gas_type       = 'fullymolecular'
  ...
  subroutine usr_ism_set_complet(self)
    ...
    cond_csound_set : if(self%myconfig%c_sound>0.0_dp) then
     ...
     self%myconfig%pressure = self%myconfig%mean_mup*self%myconfig%number_density*&
                             kB* self%myconfig%temperature
     else
      self%myconfig%temperature = self%myconfig%pressure/( (self%myconfig%mean_mup)*&
      kB*self%myconfig%number_density)
     end if
     self%myconfig%c_sound = sqrt(phys_config%gamma*self%myconfig%pressure/self%myconfig%density)
    end if cond_csound_set


if(mype==0)PRINT*,' is at ism teststtss' ,self%myconfig%mean_mup,self%myconfig%temperature
      ...
      Loop_iside_shift :  do iside = 1,2
        ...
        if(dabs( self%myconfig%profile_shift_pressure(idim,iside))<smalldouble.and. &
          ...
          if(dabs( self%myconfig%profile_shift_pressure(idim,iside))<smalldouble)then
           self%myconfig%profile_shift_pressure(idim,iside) =self%myconfig%mean_mup* &
              self%myconfig%profile_shift_number_density(idim,iside)*kB* &
              self%myconfig%profile_shift_temperature(idim,iside)
          else
            self%myconfig%profile_shift_temperature(idim,iside) =  &
              self%myconfig%profile_shift_pressure(idim,iside)  / &
              (self%myconfig%mean_mup* self%myconfig%profile_shift_number_density(idim,iside)*kB)
          end if
        end if
      ...
      where(self%patch(ixO^S))
        ...
        elsewhere
            w(ixO^S,phys_ind%rho_)      = &
               self%myconfig%profile_shift_density(self%myconfig%profile_idir,1)
            w(ixO^S,phys_ind%pressure_) = &
               self%myconfig%profile_shift_pressure(self%myconfig%profile_idir,1)
        end where
      endwhere
...
if(self%myconfig%ism_display_parameters) then
   ...
   subroutine usr_ism_get_pforce_profile(ixI^L,ixO^L,qt,qdt,x,w,f_profile,divpv,self)
    ...
    real(kind=dp), dimension(ixI^S)      :: ptherm,gradp
       ...
       cond_force1: if(dabs(self%myconfig%profile_kappa)&
        ...
        where(self%patch(ixO^S))
         f_profile(ixO^S,self%myconfig%profile_idir) =&
           ...
           (x(ixO^S,self%myconfig%profile_idir)&
            /self%myconfig%profile_zc)**(-(self%myconfig%profile_kappa+1))
        end  where
       else cond_force1
        where(self%patch(ixO^S))
    ...
    !  if(it<1)PRINT*,' is the energie ',w_init(ixO^S,phys_ind%e_)
...
!
      ...
      call gradient(ptherm,ixI^L,ixO^L,self%myconfig%profile_idir,gradp)

      Loop_idir : do  idir = 1,ndir
        pv(ixI^S,idir) =  ptherm(ixI^S)*w(ixI^S,phys_ind%mom(idir))/w(ixI^S,phys_ind%rho_)
      end do Loop_idir

      call  divvector(pv,ixI^L,ixO^L,divpv)

      where(self%patch(ixO^S))
        f_profile(ixO^S,self%myconfig%profile_idir) = gradp(ixO^S)
      end where
        ...
        where(self%patch(ixO^S))
          where(x(ixO^S,self%myconfig%profile_idir)&
            ...
            f_profile(ixO^S,self%myconfig%profile_idir) = -phys_config%gamma/&
                   self%myconfig%profile_zc*self%myconfig%profile_kappa*&
              (1.0_dp+(x(ixO^S,self%myconfig%profile_idir)&
              -self%myconfig%profile_shiftstart(self%myconfig%profile_idir))&
        ...
        cond_inside_prof: if(any(self%patch(ixO^S)))then
          ...
          where(self%patch(ixO^S))
            w(ixO^S,imom_profile_) = w(ixO^S,imom_profile_)&
                +qdt*f_profile(ixO^S,i_idir_prof_)
