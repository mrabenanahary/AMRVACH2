  ...
  case('conical')
    ...
    case('slab','cylindrical')
      if(ndim>1)then
        ...
        !   self%patch(ixO^S)  = self%patch(ixO^S).and. jet_radius(ixO^S)&
        !                        <=dsqrt((self%myconfig%z_impos+z0)**2.0_dp+self%myconfig%r_out_init**2.0_dp) .and. &
        !                     x(ixO^S,zjet_)>=z_down
          self%patch(ixO^S)  = self%patch(ixO^S).and.  x(ixO^S,zjet_)<z_out
        end if
      else
