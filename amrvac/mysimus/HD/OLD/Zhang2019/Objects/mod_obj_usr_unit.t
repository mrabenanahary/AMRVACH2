...
!> subroutine default setting for cloud
 subroutine usr_physical_unit_set_default(self)
  ...
  self%myconfig%mean_mass      = &
                    (1.0_dp+4.0_dp*self%myconfig%chemical_He_abundance)
  self%myconfig%mean_mup      = &
                    (2.0_dp+3.0_dp*self%myconfig%chemical_He_abundance)
  self%myconfig%mean_ne_to_nH     = &
                    (1.0_dp+2.0_dp*self%myconfig%chemical_He_abundance)
...
case('mks')
 ...
 subroutine usr_physical_unit_complet(the_physics_type,self)
   ...
   if(self%myconfig%temperature>smalldouble)then
    self%myconfig%pressure      = self%myconfig%mean_mup  &
   ...
   self%myconfig%luminosity       = self%myconfig%energy/self%myconfig%time
     ...
     ! phys_config%mean_frac_electron = self%myconfig%mean_ne_to_nH

   !print*, 'in mod_obj_usr_unit.t usr_physical_unit_complet subroutine: '
   ...
   subroutine unit_chemical_ionisation(He_abundance_sub,chemical_gas_type,mean_nall_to_nH &
    ...
    if(He_abundance_sub>0.0_dp)then
      ...
      select case(trim(chemical_gas_type))
        case('fullymolecular')
          mean_mup = mean_mass/(0.5+He_abundance_sub)
        ...
        case('fullyatomic')
          ...
          mean_ne_to_nH =0.0 ! is the value used in implimentation of low temperarure cooling table DM2
        case('ionised')
          mean_mup = mean_mass/(2.1_dp+He_abundance_sub)
          mean_ne_to_nH = 1.0_dp+He_abundance_sub
        case('fullyionised')
          mean_mup = mean_mass/(2.2_dp+He_abundance_sub)
