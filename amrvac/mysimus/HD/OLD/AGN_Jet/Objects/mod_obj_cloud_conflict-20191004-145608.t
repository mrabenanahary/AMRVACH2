 ...
 subroutine usr_cloud_patch(ixI^L,ixO^L,x,self,usr_patch)
  ...
  case('usr')
    if(present(usr_patch))then
     ...
     write(*,*)'this cloud shape ','usr',' is not implimented in mod_obj_cloud.t at subroutine  usr_cloud_patch'
     call mpistop('This cloud shape is not implimented in  mod_usr.t, stop at usr_cloud_patch')
    end if
  case default
...
!  call usr_cloud_patch(ixI^L,ixO^L,x,self)
  ...
  cond_inside_cloud: if(any(self%patch(ixO^S))) then
   ...
   cond_dust_on : if(self%myconfig%dust_on)then
      ...
      self%mydust%subname=trim(self%subname)//'_'//'cloud_set_w'
