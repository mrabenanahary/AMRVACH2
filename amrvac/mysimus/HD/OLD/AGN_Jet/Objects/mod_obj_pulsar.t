...
use mod_obj_readrebuilddata
implicit none

  type pulsar_parametres
    character(len=20)    :: unit                  !> physical unit at parameter file
    integer              :: myindice              !> pulsar associated indice
    character(len=78)    :: obj_name              !> pulsar obj name
    ...
    logical              :: rebuild_on            !> pulsar to rebuild from NDIM-1 simulation
    real(kind=dp)        :: t_rebuild             !> pulsar to to rebuild from NDIM-1 simulation
  ...
  type pulsar
    ...
    logical, allocatable              :: patch_rebuild(:^D&)   !> pulsar rebuild zone
    ...
    type(readrebuild)       :: myreadrebuild         !> pulsar associate rebuild
    contains
...
!> Read the ism parameters  from a parfile
 subroutine usr_pulsar_read_p(self,pulsar_config,files)
   ...
   Loop_read_file : do i_file = 1, size(files)
      ...
      read(unitpar, usr_pulsar_list, iostat=i_error_read)
   ...
   cond_wind_on : if(pulsar_config%wind_on)then
      ...
      call self%mywind%read_parameters(&
                            self%mywind%myconfig,files)
   end if cond_wind_on

   cond_envelope_on : if(pulsar_config%envelope_on)then
      self%myenvelope%myconfig%time_set = 0.0_dp
      call self%myenvelope%read_parameters(&
                              self%myenvelope%myconfig,files)
   end if cond_envelope_on

   cond_sn_on : if(pulsar_config%supernovae_remnant_on)then
   ...
   cond_rebuild_on : if(pulsar_config%rebuild_on)then
      self%myreadrebuild%myconfig%time_start = self%myconfig%t_rebuild
      self%myreadrebuild%myconfig%myindice   =  0
      call self%myreadrebuild%read_parameters(&
                                self%myreadrebuild%myconfig,files)
   end if cond_rebuild_on
...
subroutine usr_pulsar_write_setting(self,unit_config)
  ...
  write(unit_config,*) 'position     = ', self%myconfig%center*self%myphysunit%myconfig%length,&
                                          '  ',self%myphysunit%myunit%length
  write(unit_config,*) 'radius       = ', self%myconfig%radius*self%myphysunit%myconfig%length,&
                                          '  ',self%myphysunit%myunit%length
  write(unit_config,*) 'Omega        = ', self%myconfig%omega
  ...
  if(self%myconfig%supernovae_remnant_on)call self%mysupernovae_remnant%write_setting(unit_config)
  if(self%myconfig%rebuild_on)call self%myreadrebuild%write_setting(unit_config)
...
subroutine usr_pulsar_set_default(self)
 ...
 self%myconfig%rebuild_on                = .false.
 ...
 self%myconfig%t_rebuild                  = bigdouble
 ...
 call self%myreadrebuild%set_default
...
subroutine usr_pulsar_set_complet(self)
 ...
 if(self%myconfig%supernovae_remnant_on)then
    if(self%mysupernovae_remnant%myconfig%tracer_on)self%myconfig%tracer_on=.true.
 end if
 ...
 if(self%myconfig%wind_on)then
   if(self%mywind%myconfig%tracer_on)self%mywind%myconfig%itr = self%myconfig%itr
   self%mywind%myconfig%time_start = self%myconfig%t_start_pulsar_wind
   self%mywind%myconfig%time_end   = self%myconfig%t_end_pulsar_wind
 ...
 if(self%myconfig%supernovae_remnant_on)then
    ...
    if(self%mysupernovae_remnant%myconfig%tracer_on)self%myconfig%itr = self%myconfig%itr+1
 end if


 if(self%myconfig%envelope_on)then
    if(self%myenvelope%myconfig%tracer_on)self%myenvelope%myconfig%itr = self%myconfig%itr
    call self%myenvelope%set_complet

    if(self%myenvelope%myconfig%tracer_on)self%myconfig%itr = self%myconfig%itr+1
 end if
 cond_rebuild_on : if(self%myconfig%rebuild_on) then
    if(self%myreadrebuild%myconfig%tracer_on)self%myreadrebuild%myconfig%itr = self%myconfig%itr
    call self%myreadrebuild%set_complet
    if(self%myenvelope%myconfig%tracer_on)self%myconfig%itr = self%myconfig%itr+1
    cond_wind1 : if(self%myconfig%wind_on)then
      cond_wlate: if(self%myconfig%t_start_pulsar_wind<self%myconfig%t_rebuild)then
         self%myconfig%t_start_pulsar_wind=self%myconfig%t_rebuild
      end if cond_wlate
    end if cond_wind1
 end if  cond_rebuild_on
...
subroutine usr_pulsar_normalize(self,physunit_inuse)
 ...
 self%myconfig%radius           = self%myconfig%radius    / physunit_inuse%myconfig%length
 self%myconfig%velocity         = self%myconfig%velocity  / physunit_inuse%myconfig%velocity
 self%myconfig%omega            = self%myconfig%omega     / physunit_inuse%myconfig%angular_velocity
 ...
 self%myconfig%t_start_star_wind   = self%myconfig%t_start_star_wind    / physunit_inuse%myconfig%time
 self%myconfig%t_end_star_wind     = self%myconfig%t_end_star_wind      / physunit_inuse%myconfig%time
 self%myconfig%t_sn                = self%myconfig%t_sn                 / physunit_inuse%myconfig%time
 self%myconfig%t_start_pulsar_wind = self%myconfig%t_start_pulsar_wind  / physunit_inuse%myconfig%time
 self%myconfig%t_end_pulsar_wind   = self%myconfig%t_end_pulsar_wind    / physunit_inuse%myconfig%time
 self%myconfig%t_rebuild           = self%myconfig%t_rebuild            / physunit_inuse%myconfig%time

 if(self%myconfig%wind_on)call self%mywind%normalize(physunit_inuse)
 if(self%myconfig%envelope_on)call self%myenvelope%normalize(physunit_inuse)
 if(self%myconfig%supernovae_remnant_on)call self%mysupernovae_remnant%normalize(physunit_inuse)
 if(self%myconfig%rebuild_on)call self%myreadrebuild%normalize(physunit_inuse)
...
!  call self%mystar%set_w(ixI^L,ixO^L,.false.,qt,x,w)
 ...
 cond_rebuild_on: if(self%myconfig%rebuild_on)then
   cond_time_rebuild : if(dabs(qt-self%myconfig%t_rebuild)<smalldouble)then
     self%myreadrebuild%subname = self%subname
     if(allocated(self%patch_escape))then
       allocate(self%myreadrebuild%patch_escape(ixI^S))
       self%myreadrebuild%patch_escape(ixO^S) = self%patch_escape(ixO^S)
     end if
     call self%myreadrebuild%set_w(ixI^L,ixO^L,qt,x,w)
   end if cond_time_rebuild
 end if cond_rebuild_on
end subroutine usr_pulsar_set_w

subroutine usr_pulsar_get_dt(self,ixI^L,ixO^L,dx^D,x,w,qt,dtnew)
  use mod_global_parameters
  class(pulsar)                   :: self
  integer, intent(in)             :: ixI^L, ixO^L
  ...
  cond_sn_on : if(self%myconfig%supernovae_remnant_on)then
   self%mysupernovae_remnant%subname = self%subname
   call self%mysupernovae_remnant%get_dt(ixI^L,ixO^L,dx^D,x,w,qt,dtnew)
  end if cond_sn_on
  cond_wind_on : if(self%myconfig%wind_on)then
    self%mywind%subname = self%subname
    call self%mywind%get_dt(ixI^L,ixO^L,dx^D,x,w,qt,dtnew)
  end if cond_wind_on
  cond_envelope_on : if(self%myconfig%envelope_on)then
    self%myenvelope%subname = self%subname
    call self%myenvelope%get_dt(ixI^L,ixO^L,dx^D,x,w,qt,dtnew)
  end if cond_envelope_on
  
  cond_rebuild_on : if(self%myconfig%rebuild_on) then
    self%myenvelope%subname = self%subname
    call self%myreadrebuild%get_dt(ixI^L,ixO^L,dx^D,x,w,qt,dtnew)
  end if cond_rebuild_on
