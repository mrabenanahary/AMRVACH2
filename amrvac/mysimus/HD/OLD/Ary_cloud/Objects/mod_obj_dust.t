...
contains
  ...
  subroutine usr_dust_set_w_zero(ixI^L,ixO^L,x,w,self)
   ...
   !----------------------------------------------------
    ...
    Loop_idust01 : do idust = 1,phys_config%dust_n_species
  ...
  !> subroutine normalize setting for ISM
   subroutine usr_dust_normalize(self,physunit_inuse)
    ...
    self%myconfig%sizes            = self%myconfig%sizes         / physunit_inuse%length
    self%myconfig%density          = self%myconfig%density       / physunit_inuse%density
    self%myconfig%number_density   = self%myconfig%number_density/ physunit_inuse%volume
    self%myconfig%grain_density    = self%myconfig%grain_density / physunit_inuse%density
    self%myconfig%temperature      = self%myconfig%temperature   / physunit_inuse%temperature
    self%myconfig%velocity         = self%myconfig%velocity      / physunit_inuse%velocity
    self%myconfig%min_limit_abs    = self%myconfig%min_limit_abs / physunit_inuse%density
    self%myconfig%extend           = self%myconfig%extend        / physunit_inuse%length
  ...
  !> Subroutine set the patch array memory of associated with cloud object
   ...
   !--------------------------------------------------------------------
   !> Subroutine set the patch array memory of associated with cloud object
    subroutine usr_dust_set_allpatch(ixI^L,ixO^L,all_dust_inuse,all_dust_obj,self)
     implicit none
     integer, intent(in)          :: ixI^L,ixO^L
     logical, intent(in)          :: all_dust_inuse(:)
     type(dust), intent(in)       :: all_dust_obj(:)
     class(dust)                  :: self
     !.. local ..
     integer                      :: idust,i_obj,n_obj,i_dust_loc,i_obj_start
     logical, allocatable         :: dust_isuse(:)
     !-------------------------------------------------------------

     n_obj=size(all_dust_obj)
     allocate(dust_isuse(n_obj))
     ...
     Loop_idust :  do idust = self%myconfig%idust_first, self%myconfig%idust_last
      ...
      if(n_obj>1)then
       Loop_allobj_2: do i_obj=1,n_obj
        if(.not.all_dust_inuse(i_obj))cycle
