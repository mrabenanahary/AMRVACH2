...
!-------------------------------------------------------------------------
  ...
  subroutine initonegrid_usr(ixI^L,ixO^L,w,x)
    ...
    if(any(flag(ixO^S)/=0)) then
      PRINT*,' is error',maxloc(abs(flag(ixO^S)))
      if(any(ism_orion%patch(ixO^S)))PRINT*,'is in ism',maxval(abs(flag(ixO^S)),mask=ism_orion%patch(ixO^S))
      if(any(cloud_ary%patch(ixO^S)))PRINT*,'is in cloud',maxval(abs(flag(ixO^S)),mask=cloud_ary%patch(ixO^S))
    end if
