  ...
  !--------------------------------------------------------------------
   ...
   subroutine usr_dust_handel_small_val(ixI^L,ixO^L,qt,x,w,self)
     ...
     Loop_idust : do idust =self%myconfig%idust_first, self%myconfig%idust_last
      where(w(ixI^S, dust_rho(idust))<max(small_dust_rho*w(ixI^S,phys_ind%rho_),&
         self%myconfig%min_limit_abs))
        w(ixI^S, dust_rho(idust))= 0.8* min(small_dust_rho*w(ixI^S,phys_ind%rho_),&
             self%myconfig%min_limit_abs)
      ...
      where(w(ixI^S, dust_rho(idust))>self%myconfig%max_limit_rel*w(ixI^S,phys_ind%rho_))
        w(ixI^S, dust_rho(idust))=0.8*self%myconfig%max_limit_rel*w(ixI^S,phys_ind%rho_)
      ...
      Loop_idir1 : do idir = 1,ndir
       ...
       elsewhere(patch_slow(ixI^S))
        w(ixI^S, dust_mom(idir,idust))=w(ixI^S,phys_ind%mom(idir))
       end where
      end do Loop_idir1
     end do  Loop_idust
