...
!  use mod_srmhd_parameters!, only: mag,lfac_,psi_,xi_
  ...
  type cloud_parameters
    ...
    real(dp)             :: density         !> cloud density  (g/cm3)
    real(dp)             :: number_density  !> cloud number density (1/cm3)
...
!> subroutine default setting for cloud
 ...
 subroutine usr_cloud_set_complet(self)
   ...
   PRINT*,' is test volume',cloud_volume,self%myconfig%dust_frac,&
      self%myconfig%mass/cloud_volume,self%myconfig%mass/(&
      cloud_volume*unit_density),cloud_volume/unit_length**3.0_dp
   if (dabs(self%myconfig%mass)>smalldouble)then
    self%myconfig%density        = self%myconfig%mass*(1.0_dp-&
       self%myconfig%dust_frac)/cloud_volume
    ...
    if(self%myconfig%dust_frac<1.0_dp)then
       self%myconfig%mass        = self%myconfig%density*cloud_volume/(1.0_dp-&
          self%myconfig%dust_frac)
     else
       self%myconfig%mass        = 0.0_dp
     end if
   else
   ...
   if(dabs(self%myconfig%pressure)<smalldouble) then
    self%myconfig%pressure =(2.d0+3.d0*phys_config%He_abundance)&
...
!--------------------------------------------------------------------
 ...
 subroutine usr_cloud_alloc_set_patch(ixImin1,ixImax1,ixOmin1,ixOmax1,qt,x,&
    escape_patch,self)
   implicit none
   ...
   call self%the_patch(ixImin1,ixImax1,ixOmin1,ixOmax1,x)
...
!--------------------------------------------------------------------
 ...
 subroutine usr_cloud_patch(ixImin1,ixImax1,ixOmin1,ixOmax1,x,self)
  implicit none
  integer, intent(in)  :: ixImin1,ixImax1,ixOmin1,ixOmax1
  ...
  if(.not.allocated(self%patch))allocate(self%patch(ixGlo1:ixGhi1))

  self%patch              = .false.
  select case(trim(self%myconfig%shape))
  case('sphere')
    call usr_set_patch_sphere(ixImin1,ixImax1,ixOmin1,ixOmax1,typeaxial,&
       self%myconfig%center,    self%myconfig%extend,x,self%patch)

  case('cylinder')
    call usr_set_patch_cylinder(ixImin1,ixImax1,ixOmin1,ixOmax1,typeaxial,&
       self%myconfig%center,  self%myconfig%extend,x,self%patch)
  case('cube')
    call usr_set_patch_cube(ixImin1,ixImax1,ixOmin1,ixOmax1,typeaxial,&
       self%myconfig%center,      self%myconfig%extend,x,self%patch)
  case default
     write(*,*)'this cloud shape ',trim(self%myconfig%shape),&
        ' is not implimented'
     call mpistop&
        ('This cloud shape is not implimented in usr_cloud_patch at mod_usr.t')

  end select
 end subroutine usr_cloud_patch
!--------------------------------------------------------------------
 !> subroutine setting for cloud
 subroutine usr_cloud_set_w(ixImin1,ixImax1,ixOmin1,ixOmax1,qt,x,w,self)
  implicit none
  integer, intent(in)        :: ixImin1,ixImax1,ixOmin1,ixOmax1
  real(kind=dp), intent(in)  :: qt
  real(kind=dp)              :: x(ixImin1:ixImax1,1:ndir)
...
!  call usr_cloud_patch(ixI^L,ixO^L,x,self)
  if(.not.allocated(self%patch))then
    call usr_cloud_patch(ixImin1,ixImax1,ixOmin1,ixOmax1,x,self)
  end if
  cond_inside_cloud: if(any(self%patch(ixOmin1:ixOmax1))) then
   ...
   Loop_idir : do idir=1,ndir
       where(self%patch(ixOmin1:ixOmax1))
         w(ixOmin1:ixOmax1,phys_ind%mom(idir)) = self%myconfig%velocity(idir)
       end where
   end do Loop_idir
   if(self%myconfig%profile/='none') then
    call usr_mat_profile(ixImin1,ixImax1,ixOmin1,ixOmax1,typeaxial,&
       self%myconfig%profile,self%myconfig%center,self%myconfig%extend,x,&
       fprofile)
    where(self%patch(ixOmin1:ixOmax1))
      w(ixOmin1:ixOmax1,phys_ind%rho_)=w(ixOmin1:ixOmax1,&
         phys_ind%rho_)*fprofile(ixOmin1:ixOmax1)
   ...
   cond_tracer_on :if(self%myconfig%tracer_on.and.phys_config%n_tracer>0.and.&
    ...
    where(self%patch(ixOmin1:ixOmax1))
     w(ixOmin1:ixOmax1,phys_ind%tracer(self%myconfig%itr)) = w(ixOmin1:ixOmax1,&
        phys_ind%rho_)
    elsewhere
   ...
   cond_dust_on : if(self%myconfig%dust_on)then
      if(self%myconfig%dust_profile/='none') then
        call usr_mat_profile(ixImin1,ixImax1,ixOmin1,ixOmax1,typeaxial,&
           ...
           self%myconfig%extend,x,fprofile)
      else
        fprofile = 1.0_dp
      end if
