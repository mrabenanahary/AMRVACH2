...
implicit none
  ...
  type rel_wind
    ...
    logical, allocatable              :: patch(:)         !> wind is on cell
    logical, allocatable              :: patch_escape(:)  !> wind is on cell
...
subroutine usr_wind_write_setting(self,unit_config)
  ...
  if(self%myconfig%unit/='code')then
    ...
    write(unit_config,*) 'Power       = ',&
        self%myconfig%power*self%myphysunit%myconfig%luminosity,'  ',&
 ...
 subroutine usr_wind_set_complet(self,star_speed)
   ...
   if (dabs(self%myconfig%mass_flux)>smalldouble)then
    self%myconfig%density_init        = self%myconfig%mass_flux/(4.0_dp*dpi*   &
         self%myconfig%r_in**2.0_dp *  dabs(self%myconfig%velocity(&
       r_))*self%myconfig%lfac)
...
!--------------------------------------------------------------------
 subroutine usr_wind_normalize(self,physunit_inuse)
  ...
  self%myconfig%r_in             = self%myconfig%r_in             &
     /physunit_inuse%myconfig%length
  self%myconfig%r_out_impos      = self%myconfig%r_out_impos      &
     /physunit_inuse%myconfig%length
  self%myconfig%r_out_init       = self%myconfig%r_out_init       &
     /physunit_inuse%myconfig%length
  self%myconfig%power            = self%myconfig%power            &
     /physunit_inuse%myconfig%luminosity
  self%myconfig%power_at_t       = self%myconfig%power_at_t       &
     /physunit_inuse%myconfig%luminosity
  self%myconfig%tau_spin_down    = self%myconfig%tau_spin_down    &
     /physunit_inuse%myconfig%time
  self%myconfig%mass_flux        = self%myconfig%mass_flux        &
     /physunit_inuse%myconfig%mass_flux
  self%myconfig%time_start       = self%myconfig%time_start       &
     /physunit_inuse%myconfig%time
  self%myconfig%time_end         = self%myconfig%time_end         &
     /physunit_inuse%myconfig%time
...
!--------------------------------------------------------------------
 ...
 subroutine usr_wind_patch(ixImin1,ixImax1,ixOmin1,ixOmax1,qt,x,self)
  implicit none
  ...
  if(qt<=self%myconfig%time_start)then
   self%patch(ixOmin1:ixOmax1) = Dist(ixOmin1:ixOmax1) &
      <self%myconfig%r_out_init .and. Dist(ixOmin1:ixOmax1) &
      >self%myconfig%r_in
  else
   self%patch(ixOmin1:ixOmax1) = Dist(ixOmin1:ixOmax1) &
      <self%myconfig%r_out_impos .and. Dist(ixOmin1:ixOmax1) &
      >self%myconfig%r_in
  end if
...
!--------------------------------------------------------------------
 ...
 subroutine usr_wind_set_w(ixImin1,ixImax1,ixOmin1,ixOmax1,qt,x,w,self)
  implicit none
  integer, intent(in)          :: ixImin1,ixImax1,ixOmin1,ixOmax1
  real(kind=dp), intent(in)    :: qt
  ...
  ! .. local..
  integer                                :: idir
  real(kind=dp), dimension(ixImin1:ixImax1)        :: energy_flux,sqrB,&
     kinetic_flux
  real(kind=dp), dimension(ixImin1:ixImax1)        :: theta,sintheta
  real(kind=dp), dimension(ixImin1:ixImax1,1:ndim) :: x_sphere
  !----------------------------------

  call usr_get_spherical(ixImin1,ixImax1,ixOmin1,ixOmax1,typeaxial,&
     self%myconfig%center,x,x_sphere)
  call usr_wind_patch(ixImin1,ixImax1,ixOmin1,ixOmax1,qt,x,self)
  call self%get_power(ixImin1,ixImax1,ixOmin1,ixOmax1,qt,x,w)
  if(z_in)then
   where(self%patch(ixOmin1:ixOmax1))
    sintheta(ixOmin1:ixOmax1) = dsin(x_sphere(ixOmin1:ixOmax1,theta_))
    theta(ixOmin1:ixOmax1)    = x_sphere(ixOmin1:ixOmax1,theta_)
   end where
  else
   where(self%patch(ixOmin1:ixOmax1))
    theta(ixOmin1:ixOmax1)    = dpi/2.0_dp
    sintheta(ixOmin1:ixOmax1) = 1.0_dp
   end where
  end if
  where(self%patch(ixOmin1:ixOmax1))
   energy_flux(ixOmin1:ixOmax1)      = self%myconfig%power_at_t/x_sphere(&
      ixOmin1:ixOmax1,r_)**2.0_dp *(sintheta(ixOmin1:ixOmax1)**2.0d0+&
      one/self%myconfig%sigma0)
   ...
   w(ixOmin1:ixOmax1,mag(phi_))      = dsqrt(4.0_dp*dpi*self%myconfig%power)* &
      self%myconfig%xisigma0/x_sphere(ixOmin1:ixOmax1,&
      r_)  * sintheta(ixOmin1:ixOmax1)*(1.0_dp-&
      2.0_dp*theta(ixOmin1:ixOmax1)/dpi)



   sqrB(ixOmin1:ixOmax1)             = SUM(w(ixOmin1:ixOmax1,&
      mag(1):mag(ndir))**2.0_dp,dim=ndim+1)
   kinetic_flux(ixOmin1:ixOmax1)     = energy_flux(ixOmin1:ixOmax1)-  &
      sqrB(ixOmin1:ixOmax1)/(4.0_dp*dpi)

   w(ixOmin1:ixOmax1,rho_)           = kinetic_flux(ixOmin1:ixOmax1)/(w(&
      ixOmin1:ixOmax1,lfac_)*w(ixOmin1:ixOmax1,mom(r_)))

   w(ixOmin1:ixOmax1,p_)             = max(self%myconfig%beta*half*sqrB(&
      ixOmin1:ixOmax1),1.0d-2*w(ixOmin1:ixOmax1,rho_),1.0d4*small_pressure)
 ...
 subroutine usr_wind_energy_variation(ixImin1,ixImax1,ixOmin1,ixOmax1,qt,x,w,&
    self)
    implicit none
    integer, intent(in)          :: ixImin1,ixImax1,ixOmin1,ixOmax1
    real(kind=dp), intent(in)    :: qt
    ...
    !--------------------------------------------
     select case(self%myconfig%variation_type)
     case('pulsar_spin_down')
       n_power=(self%myconfig%braking_index+&
          1.0_dp)/(self%myconfig%braking_index-1.0_dp)
       self%myconfig%power_at_t=self%myconfig%power &
          *(1.0_dp+(qt-self%myconfig%time_start)/self%myconfig%tau_spin_down)**&
          (-n_power)
     case default
       ! dummy
       self%myconfig%power_at_t  = self%myconfig%power
...
subroutine usr_wind_get_dt(self,ixImin1,ixImax1,ixOmin1,ixOmax1,dx1,x,w,qt,&
   dtnew)
  use mod_global_parameters
  class(rel_wind)                 :: self
  integer, intent(in)             :: ixImin1,ixImax1, ixOmin1,ixOmax1
...
subroutine usr_wind_spd_rad_to_cart(ixImin1,ixImax1,ixOmin1,ixOmax1,x,w,self)
  use mod_global_parameters
  implicit none
  integer, intent(in)             :: ixImin1,ixImax1,ixOmin1,ixOmax1
  real(kind=dp), intent(in)       :: x(ixImin1:ixImax1,1:ndir)
 ...
 select case(typeaxial)
  ...
  case('cylindrical')
   ...
   where(self%patch(ixOmin1:ixOmax1))
    w(ixOmin1:ixOmax1,mom(r_)) = v_spherical(ixOmin1:ixOmax1,&
       r_)*cos_phi(ixOmin1:ixOmax1)*sin_theta(ixOmin1:ixOmax1)
