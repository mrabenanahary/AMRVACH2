module mod_obj_usr_unit
 ...
 contains
   ...
   PROCEDURE, PASS(self) :: write_setting      => &
      usr_physical_unit_write_setting
   PROCEDURE, PASS(self) :: fillphysunit       => &
      usr_physical_unit_fillphysunit
...
!-------------------------------------------------------------------------
 !> Read the ism parameters  from a parfile
  subroutine usr_physical_unit_read_p(self,phys_unit,files)
  ...
  subroutine usr_physical_unit_write_setting(self,unit_config)
    ...
    write(unit_config,*) 'Length        = ', self%myconfig%length, '   ',&
        trim(self%myunit%length)
    write(unit_config,*) 'Volume        = ', self%myconfig%volume, '   ',&
       self%myunit%volume
    write(unit_config,*) 'Time          = ', self%myconfig%time, '   ',&
       self%myunit%time
    write(unit_config,*) 'Density       = ', self%myconfig%density,'   ',&
       self%myunit%density
    write(unit_config,*) 'Number Density= ', self%myconfig%number_density,&
       '   ',  self%myunit%number_density
    write(unit_config,*) 'Pressure      = ', self%myconfig%pressure, '   ',&
        self%myunit%pressure
    write(unit_config,*) 'Temperature   = ', self%myconfig%temperature, '   ',&
         self%myunit%temperature
    ...
    write(unit_config,*) 'Speed         = ', self%myconfig%velocity, '   ',&
        self%myunit%velocity
    write(unit_config,*) 'Angular Speed = ', self%myconfig%angular_velocity,&
       '   ', self%myunit%angular_velocity
    write(unit_config,*) 'Moment        = ', self%myconfig%momentum,'   ',&
        self%myunit%momentum
    write(unit_config,*) 'Magnetic      = ', self%myconfig%magnetic,'   ',&
        self%myunit%magnetic
    write(unit_config,*) 'Mass          = ', self%myconfig%mass_flux,'   ',&
       self%myunit%mass_flux
    write(unit_config,*) 'Luminosity    = ', self%myconfig%luminosity,'   ',&
       self%myunit%luminosity
    write(unit_config,*)'************************************'
...
case('mks')
 ...
 subroutine usr_physical_unit_complet(self)
   ...
   elseif(self%myconfig%time>smalldouble)then
      self%myconfig%length     = self%myconfig%time*dabs(&
   ...
   if(self%myconfig%temperature>smalldouble)then
    self%myconfig%pressure      =&
        mean_mass*self%myconfig%number_density*kB*self%myconfig%temperature
   else
    self%myconfig%pressure         =&
        self%myconfig%density*self%myconfig%velocity**2.0_dp
    self%myconfig%temperature   = self%myconfig%pressure/ &
       (mean_mass*Kb*self%myconfig%number_density)
   end if

   self%myconfig%momentum         =&
       self%myconfig%density*self%myconfig%velocity
   self%myconfig%angular_velocity =&
       self%myconfig%velocity/self%myconfig%length

   self%myconfig%volume           = self%myconfig%length**3.0_dp
   self%myconfig%mass             = self%myconfig%volume*self%myconfig%density

   self%myconfig%energy           = self%myconfig%mass*self%myconfig%velocity**&
      2.0_dp
   self%myconfig%energy_density   =&
       self%myconfig%density*self%myconfig%velocity**2.0_dp
