module mod_obj_dust
  ...
  type dust_parameters
     ...
     real(dp)             :: min_limit_abs      !> dust floor abs (g/cm3)
  ...
  type dust_ispecies
    logical, allocatable :: patch(:)
  end type dust_ispecies
  ! the dust type
  type dust
   logical, allocatable              :: patch(:)        !> dust is on cell
...
contains
  ...
  subroutine usr_dust_write_setting(self,unit_config)
    ...
    write(unit_config,*) 'Density       = ',&
        self%myconfig%density(1:self%myconfig%idust_last-&
       self%myconfig%idust_first+1)
    write(unit_config,*) 'Grain density = ',&
        self%myconfig%grain_density(1:self%myconfig%idust_last-&
       self%myconfig%idust_first+1)
  ...
  !> subroutine default setting for ISM
   subroutine usr_dust_set_default(self)
    ...
    self%myconfig%extend(1:2,1:ndim) = 0.0_dp !box_limit(1:2,1:ndim) ! it zero at this time
  ...
  subroutine usr_dust_set_w_zero(ixImin1,ixImax1,ixOmin1,ixOmax1,x,w,self)
    use mod_dust
   implicit none
   ...
   !----------------------------------------------------
    ...
    Loop_idust01 : do idust = 1,dust_n_species
      if(all(self%the_ispecies(idust)%patch(ixOmin1:ixOmax1)))cycle &
         Loop_idust01
      where(.not.self%patch(ixOmin1:ixOmax1))
        w(ixOmin1:ixOmax1,dust_rho(idust)) = &
           1.1d-2*self%myconfig%min_limit_abs
      end where
      Loop_idir : do idir=1,ndir
          where(.not.self%patch(ixOmin1:ixOmax1))
           w(ixOmin1:ixOmax1,dust_mom(idir,&
              idust)) = self%myconfig%velocity(idir)
          end where
      end do Loop_idir
    end do Loop_idust01
  ...
  !--------------------------------------------------------------------
   ...
   subroutine usr_dust_set_w(ixImin1,ixImax1,ixOmin1,ixOmax1,qt,is_frac,&
      dust_frac,fprofile,x,w,self)
    implicit none
    integer, intent(in)          :: ixImin1,ixImax1,ixOmin1,ixOmax1
    real(kind=dp), intent(in)    :: qt
    logical                      :: is_frac
    real(kind=dp), intent(in)    :: dust_frac
    real(kind=dp), intent(in)    :: fprofile(ixImin1:ixImax1)
    real(kind=dp), intent(in)    :: x(ixImin1:ixImax1,1:ndir)
    real(kind=dp), intent(inout) :: w(ixImin1:ixImax1,1:nw)
    class(dust)                  :: self
    ! .. local..
    real(kind=dp)                :: slop,dr_dust,dust_frac_loc
    integer                      :: idir,idust,istart_dust,istop_dust,&
       i_dust_loc
    !----------------------------------

    if(.not.any(self%patch(ixOmin1:ixOmax1)))return
    istart_dust=1
    ...
    if(is_frac) then
     ...
     case('uniform')
      cond_fracdust : if(dust_frac_loc<0.0_dp) then
       ...
       Loop_idust2 : do idust = self%myconfig%idust_first,&
           self%myconfig%idust_last
        i_dust_loc = idust-self%myconfig%idust_first+1
        where(self%patch(ixOmin1:ixOmax1))
         w(ixOmin1:ixOmax1,dust_rho(idust)) = &
            self%myconfig%density(i_dust_loc)/ real(self%myconfig%n_species,&
            kind=dp)
        end where
      end do Loop_idust2
      else cond_fracdust

       Loop_idust3 : do idust = self%myconfig%idust_first,&
           self%myconfig%idust_last
        where(self%patch(ixOmin1:ixOmax1))
         w(ixOmin1:ixOmax1,dust_rho(idust)) = dust_frac_loc*w(ixOmin1:ixOmax1,&
            phys_ind%rho_)/ real(self%myconfig%n_species,kind=dp)
        end where
      end do Loop_idust3
     ...
     case('powerlaw')
      ...
      Loop_idust4 : do idust = self%myconfig%idust_first,&
          self%myconfig%idust_last
       dr_dust=0.5_dp*(dust_size(max(idust-1,&
          self%myconfig%idust_first))-dust_size(min(idust+1,&
          self%myconfig%idust_last)))
       where(self%patch(ixOmin1:ixOmax1))
        w(ixOmin1:ixOmax1,dust_rho(idust)) = w(ixOmin1:ixOmax1,&
           phys_ind%rho_)*dust_frac_loc*half*(dr_dust/dsqrt(dust_size(idust))/ &
           (dsqrt(dust_size(self%myconfig%idust_first))-&
           dsqrt(dust_size(self%myconfig%idust_last))))
        w(ixOmin1:ixOmax1,dust_rho(idust)) = w(ixOmin1:ixOmax1,&
           dust_rho(idust)) *fprofile(ixOmin1:ixOmax1)
       end where
      end do Loop_idust4
    ...
    cond_isassociated : if(self%myconfig%associate_med)then
     Loop_idust5 : do idust = self%myconfig%idust_first,&
         self%myconfig%idust_last
     ...
     Loop_idust6 : do idust = self%myconfig%idust_first,&
         self%myconfig%idust_last
      Loop_idir : do idir=1,ndir
       where(self%patch(ixOmin1:ixOmax1))
       w(ixOmin1:ixOmax1,dust_mom(idir,idust)) = self%myconfig%velocity(idir)
       end where
      end do Loop_idir
     end do Loop_idust6
   ...
   subroutine usr_dust_set_complet(self,is_frac,dust_frac,med_density,&
      med_velocity)
     implicit none
     class(dust)              :: self
     logical                  :: is_frac
     ...
     self%myconfig%idust_last=(self%myconfig%idust_first-1) &
        +self%myconfig%n_species
    if(is_frac) then
      dust_frac_loc=dust_frac/(1.0_dp-dust_frac)
    else
      dust_frac_loc=dust_frac
    end if
     cond_size_off : if(any(self%myconfig%sizes(1:self%myconfig%n_species)<=&
        smalldouble))then

      select case(self%myconfig%distrub_func)
      case('uniform')

       slop=(self%myconfig%max_radius-self%myconfig%min_radius)/dble(&
          self%myconfig%n_species-1)
       Loop_idust1 : do idust = self%myconfig%idust_first,&
           self%myconfig%idust_last
         i_dust_loc = idust-self%myconfig%idust_first+1
         self%myconfig%sizes(i_dust_loc)   = &
            self%myconfig%min_radius+slop*real(idust-1,kind=dp)
         self%myconfig%density(i_dust_loc) = dust_frac_loc* &
            med_density/real(self%myconfig%n_species,kind=dp)
       end do Loop_idust1

      case('powerlaw')
       call usr_mat_powerlaw_withX(self%myconfig%n_species,&
          self%myconfig%power_a,self%myconfig%min_radius,&
          self%myconfig%max_radius,   self%myconfig%sizes)


       Loop_idust2  : do idust = self%myconfig%idust_first,&
           self%myconfig%idust_last
        i_dust_loc = idust-self%myconfig%idust_first+1
        dr_dust=0.5_dp*(self%myconfig%sizes(max(i_dust_loc-1,&
           1))-self%myconfig%sizes(min(i_dust_loc+1,&
           self%myconfig%idust_last-self%myconfig%idust_first+1)))
     ...
     allocate(self%the_ispecies&
        (self%myconfig%idust_first:self%myconfig%idust_last))
  ...
  !> subroutine normalize setting for ISM
   subroutine usr_dust_normalize(self,physunit_inuse)
    ...
    self%myconfig%sizes            = self%myconfig%sizes         / &
       physunit_inuse%myconfig%length
    self%myconfig%density          = self%myconfig%density       / &
       physunit_inuse%myconfig%density
    self%myconfig%number_density   = self%myconfig%number_density/ &
       physunit_inuse%myconfig%number_density
    self%myconfig%grain_density    = self%myconfig%grain_density / &
       physunit_inuse%myconfig%density
    ...
    self%myconfig%min_limit_abs    = self%myconfig%min_limit_abs / &
       physunit_inuse%myconfig%density
  ...
  !> Subroutine set the patch array memory of associated with cloud object
   subroutine usr_dust_set_patch(ixImin1,ixImax1,ixOmin1,ixOmax1,imposed_patch,&
      self)
    implicit none
    integer, intent(in)          :: ixImin1,ixImax1,ixOmin1,ixOmax1
    logical, intent(in)          :: imposed_patch(ixImin1:ixImax1)
    class(dust)                  :: self
    !.. local ..
    integer                      :: idust
    !-------------------------------------------------------------
    ...
    self%patch(ixOmin1:ixOmax1) = imposed_patch(ixOmin1:ixOmax1)
    Loop_idust :  do idust = self%myconfig%idust_first,&
        self%myconfig%idust_last
     if(allocated(self%the_ispecies(idust)%patch))then
       deallocate(self%the_ispecies(idust)%patch)
     end if
     allocate(self%the_ispecies(idust)%patch(ixOmin1:ixOmax1))
     self%the_ispecies(idust)%patch(ixOmin1:ixOmax1)=self%patch(&
        ixOmin1:ixOmax1)
    end do Loop_idust
   ...
   !--------------------------------------------------------------------
   !> Subroutine set the patch array memory of associated with cloud object
    subroutine usr_dust_set_allpatch(ixImin1,ixImax1,ixOmin1,ixOmax1,&
       all_dust_obj,self)
     implicit none
     integer, intent(in)          :: ixImin1,ixImax1,ixOmin1,ixOmax1
     ...
     Loop_allobj_1: do i_obj=1,n_obj
       if(allocated(all_dust_obj(i_obj)%patch))then
        self%patch(ixOmin1:ixOmax1)=self%patch(ixOmin1:ixOmax1).or.&
           all_dust_obj(i_obj)%patch(ixOmin1:ixOmax1)
       end if
     end do Loop_allobj_1


     self%myconfig%idust_first=minval(all_dust_obj(&
        1:n_obj)%myconfig%idust_first)
     self%myconfig%idust_last=maxval(all_dust_obj(&
        1:n_obj)%myconfig%idust_last)
     ...
     Loop_idust :  do idust = self%myconfig%idust_first,&
      ...
      allocate(self%the_ispecies(idust)%patch(ixOmin1:ixOmax1))
      self%the_ispecies(idust)%patch(ixOmin1:ixOmax1)=.false.
      if(n_obj>1)then
       Loop_allobj_2: do i_obj=1,n_obj
        if(idust>=all_dust_obj(i_obj)%myconfig%idust_first.and.idust<=&
           all_dust_obj(i_obj)%myconfig%idust_last) then
           !i_dust_loc = idust-all_dust_obj(i_obj)%myconfig%idust_first+1
           cond_obj2_on : if(allocated(all_dust_obj(i_obj)%the_ispecies(&
              idust)%patch))then
            self%the_ispecies(idust)%patch(ixOmin1:ixOmax1)=self%the_ispecies(&
               idust)%patch(ixOmin1:ixOmax1).or.&
               all_dust_obj(i_obj)%the_ispecies(idust)%patch(ixOmin1:ixOmax1)
           end if  cond_obj2_on
