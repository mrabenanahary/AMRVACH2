module mod_obj_cla_jet
  ...
  type cla_jet
    ...
    contains
     ...
     PROCEDURE, PASS(self) :: compute_density => gr_compute_w_rho
...
print*,'the jet self%myconfig%r_out_init = ',self%myconfig%r_out_init
  ...
  self%myconfig%jet_surface_init = jet_surface_init
...
gr_solv%myconfig%densityElectrons(i_obj)*(mp/me))
       ...
       jet_pressu = gr_solv%myconfig%density_gas(i_obj)*kB*&
     ...
     write(*,*) 'jet set_complet rhotot',gr_solv%myconfig%density_tot(i_obj)
     write(*,*) 'jet set_complet HIdensity',self%myconfig%HI_density
     write(*,*) 'jet set_complet H2Idensity',self%myconfig%H2I_density
     write(*,*) 'jet set_complet HeIdensity',gr_solv%myconfig%densityHeI(i_obj)
     write(*,*) 'jet set_complet e_density',gr_solv%myconfig%densityElectrons(i_obj)
  ...
  cond_var1 : if(self%myconfig%variation_on)then
   ...
   cond_vpol_var : if (self%myconfig%variation_velocity_poloidal>0.0_dp) then
      self%myconfig%variation_velocity(r_)     = self%myconfig%variation_velocity_poloidal*&
            dsin(self%myconfig%open_angle*dpi/180.0_dp)
      self%myconfig%variation_velocity(theta_) = self%myconfig%variation_velocity_poloidal*&
             dcos(self%myconfig%open_angle*dpi/180.0_dp)
...
!--------------------------------------------------------------------
 ...
 subroutine usr_cla_jet_set_patch(ixI^L,ixO^L,qt,x,self,force_refine,dx_loc,&
  ...
  real(dp)                   :: r_in,r_out
  real(dp)                   :: z_down,z_out,min_dist,z0
  !----------------------------------
  ! check distance
...
!--------------------------------------------------------------------
 ...
 subroutine usr_cla_jet_set_w(ixI^L,ixO^L,qt,x,w,gr_solv,self)
  ...
  real(kind=dp), dimension(ixI^S) :: fprofile,angle_theta,&
                                    ...
                                    jet_radius_in,jet_radius,mmw_field
 ...
 subroutine gr_compute_w_rho(ixI^L,ixO^L,qt,x,w,rho_ind,densityref,&
 jet_srfce,self)
  implicit none
  integer, intent(in)          :: ixI^L,ixO^L,rho_ind
  real(kind=dp), intent(in)    :: qt
  real(kind=dp)                :: densityref
  real(kind=dp)                :: x(ixI^S,1:ndim)
  real(kind=dp)                :: w(ixI^S,1:nw)
  real(kind=dp)                :: jet_srfce(ixI^S)
  class(cla_jet)               :: self
  ! .. local..
 ...
 !--------------------------------------------------------------------
  ...
  !            angle_theta(ixO^S)          =  self%myconfig%open_angle
            case('slab','cylindrical')
             ...
             self%ejecta_patch(ixO^S) = (jet_radius(ixO^S)&
                                          -dsqrt((self%myconfig%variation_position(zjet_)+z0)**2.0_dp+&
