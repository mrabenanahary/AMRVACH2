...
type flux_parameters
    logical                         :: computeflux
    real(kind=dp)                   :: position(100,2,3)
    character(len=30)               :: surfacetype(100)
    character(len=30)               :: multiplecellsmethod
    character(len=30)               :: fluxdirection(100)
    integer                         :: nsurface

end type flux_parameters
type flux_type
    real(kind=dp),allocatable             :: position(:,:,:)
    character(len=30),allocatable         :: surfacetype(:)
    character(len=30),allocatable         :: fluxdirection(100)
    logical,allocatable                   :: surfaceix(:,:^D&,:)
    logical,allocatable                   :: surfacencells(:)
    real(kind=dp)                         :: surfaceflux(:)
    type(flux_parameters)                 :: myconfig
end type flux_type
...
subroutine usr_myflux_set_default(self)
  ...
  class(flux_type)              :: self
   ...
   self%myconfig%position(1:100,1:2,1:3) = (/0,0,0/)
   self%myconfig%surface_type = 'plane'
   ...
   !--------------------------------------------------------------------
    !> Read the ism parameters  from a parfile
     subroutine usr_fluxes_read_p(self,myflux_config,files)
       ...
       class(flux_type)                           :: self
       character(len=*),intent(in)                :: files(:)
       type(flux_parameters), intent(out)         :: myflux_config
       ! .. local ..
...
subroutine usr_fluxes_set_complet(self)
  ...
  integer                               :: idims,iside,idir,iw,isurface
   ...
   !pass it from myconfig to flux_type
   if(.not.associated(self%position))&
  ...
  !pass it from myconfig to flux_type
  if(.not.associated(self%surfacetype))&
  ...
  !pass it from myconfig to flux_type
  if(.not.associated(self%fluxdirection))&
...
subroutine usr_fluxes_set_w(ixI^L,ixO^L,&
  ...
  integer                                :: iside,ixbds,isurface,ix^D,ixold^D,ifluxes,idir
  real(kind=dp), allocatable              :: distancetox(:)
  real(kind=dp)                          :: xref(1:ndim)
  real(kind=dp)                          :: xbounds(1:ndim,1:ndim)
  real(kind=dp)                          :: maxflux
  real(kind=dp), allocatable             :: multiplecellsfluxes(:)
  logical, allocatable                   :: multiplecellsindexes(:,:)
  logical, allocatable                   :: surfaceixtmp(:^D&)
  integer                                :: ncellsforbounds(1:ndim),ncellsfound
  logical                                :: cellfound
  ...
  Loop_surface_flux : do isurface=1,self%myconfig%nsurface
    ...
    if(trim(self%surfacetype)=='plane'.or.(ndim==1))then
      ...
      {^IFONED if(ndim==1)then

      iside=1
      if(.not.allocated(distancetox))&
        allocate(distancetox(ixO^S))
      call usr_distance(ixI^L,ixO^L,typeaxial,&
                        self%position(isurface,iside,1:ndim),&
                        x,distancetox)
      where(DABS(distancetox(ixO^S)-MINVAL(distancetox(ixO^S)))<=smalldouble)
        self%surfaceix(isurface,ixO^S)=.true.
      end where

      !count the number of cells found to be closest to  position
      ncellsforbounds(^D) = COUNT(self%surfaceix(isurface,ixO^S))


      if(allocated(surfaceixtmp))deallocate(surfaceixtmp)
      if(.not.allocated(surfaceixtmp))&
        allocate(surfaceixtmp(ixO^S))


      surfaceixtmp(ixO^S)=self%surfaceix(isurface,ixO^S)


      if(allocated(multiplecellsfluxes))deallocate(multiplecellsfluxes)
      if(.not.allocated(multiplecellsfluxes))&
        allocate(multiplecellsfluxes(1:ncellsforbounds(^D)))

      !store the indexes of each multiple cell
      if(allocated(multiplecellsindexes))deallocate(multiplecellsindexes)
      if(.not.allocated(multiplecellsindexes))&
        allocate(multiplecellsindexes(1:ncellsforbounds(^D),1:ndim))

      ncellsfound=0
      do ixbds = 1,ncellsforbounds(^D)
        cellsfound = .false.

        {do ix^D=ixOmin^D,ixOmax^D\}
          if(self%surfaceix(isurface,ix^D))then
            cellsfound=.true.
            ncellsfound=ncellsfound+1
            {multiplecellsindexes(ixbds,^D)=ix^D\}
            !1D flux is assumed through a spherical 3D surface
            select case(self%fluxdirection(isurface))
            case('out')
              multiplecellsfluxes(ixbds)=0.0_dp

               !Just treat cylindrical coordinates
               multiplecellsfluxes(ixbds)=&
               multiplecellsfluxes(ixbds)+&
               w(ix^D,iw_rho)*&
            ...
            if(ncellsfound==1)self%surfaceflux(isurface)=multiplecellsfluxes(ixbds)
            if(ncellsfound>1)then
              select case(trim(self%myconfig%multiplecellsmethod))
              case('greatest')
                !Arbitrarilty select the cell with the greatest flux
                if(multiplecellsfluxes(ixbds)>multiplecellsfluxes(ixbds-1))then
                  surfaceixtmp(ixold^D)=.false.
                  self%surfaceflux(isurface)=multiplecellsfluxes(ixbds)
                else if(multiplecellsfluxes(ixbds)<=multiplecellsfluxes(ixbds-1))then
                  surfaceixtmp(ix^D)=.false.
                end if
              case default
         ...
         {if(cellsfound)cycle
          ...
          self%surfaceix(isurface,ixO^S)=surfaceixtmp(ixO^S)
      end do
      self%surfacencells(isurface) = COUNT(self%surfaceix(isurface,ixO^S))
      ...
      !{^NOONED
        ...
        {do ix^D=ixOmin^D,ixOmax^D\}
          ...
          do idims = 1:ndim
            xref(ix^D,idims) = x(ix^D,idims)
          end do

          if(.not.allocated(distancetox))&
            allocate(distancetox(ixI^S,1:ndim))
          call usr_distance(ixI^L,ixO^L,typeaxial,&
                            zero_dim,x,distancetox)
          self%surfaceix(self%position(isurface,1,1))=.true.
        {end do^D&\}
