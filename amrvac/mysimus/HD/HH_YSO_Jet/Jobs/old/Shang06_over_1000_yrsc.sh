#!/bin/bash
#SBATCH --job-name=Shang06_over_1000_yrsc
...
cd /data/$USER/Output
mkdir Shang06_over_1000_yrsc

cd $SCRATCH
srun --ntasks=$SLURM_JOB_NUM_NODES mkdir -p Output
srun --ntasks=$SLURM_JOB_NUM_NODES mkdir -p Output/Shang06_over_1000_yrsc
srun --ntasks=$SLURM_JOB_NUM_NODES cp $AMRVAC_DIR/mysimus/HD/HH_YSO_Jet/Parfiles/Article/Shang06/Shang06_over_1000_yrsc.par .
srun --ntasks=$SLURM_JOB_NUM_NODES cp /data/$USER/Output/Shang06b/Jet_Shang06_0100.dat .
...
srun --ntasks=$SLURM_JOB_NUM_NODES more Shang06_over_1000_yrsc.par
mpiexec amrvac -i Shang06_over_1000_yrsc.par
#srun --ntasks=$SLURM_JOB_NUM_NODES mv Output/Shang06_over_1000_yrs/* /data/$USER/Output/Shang06_over_1000_yrs/.
srun --ntasks=$SLURM_JOB_NUM_NODES cp $AMRVAC_DIR/mysimus/HD/HH_YSO_Jet/Jobs/slurm-${SLURM_JOBID}.out /data/$USER/Output/Shang06_over_1000_yrsc/.
cd ${SLURM_SUBMIT_DIR}
