...
type star_envelope
  ...
  logical, allocatable           :: patch(:^D&)         !> envelope is on cell
  logical, allocatable           :: patch_escape(:^D&)  !> envelope  not on cell
  type(star_envelope_parameters) :: myconfig    !> wind paramter to read
  ...
  contains
   ...
   PROCEDURE, PASS(theenvelope) :: read_parameters => usr_envelope_read_p
...
contains
  ...
  !-------------------------------------------------------------------------
   !> Read the ism parameters  from a parfile
    subroutine usr_envelope_read_p(theenvelope,envelope_config,files)
      class(star_envelope)                   :: theenvelope
      ...
      do i_file = 1, size(files)
         ...
         read(unitpar, usr_envelope_list, end=112)
  112    close(unitpar)
      end do
  ...
  !--------------------------------------------------------------------
   subroutine usr_envelope_normalize(self)
    implicit none
    class(star_envelope)          :: self
    !----------------------------------
