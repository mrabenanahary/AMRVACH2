module mod_usr
  ...
  type usr_config
    ...
    logical           :: grackle_force_ism_cool
  ...
  type(gr_solver),target             :: grackle_solver
...
contains
  ...
  subroutine usr_init
    ...
    use mod_hd_grackle, only : hd_activate
    ...
    if(phys_config%energy)then
      if(phys_config%use_grackle)then
        ...
        ! ensure that parameters rode are the same as input to hd_list 
        write(*,*) '0 phys_config%primordial_chemistry = ',phys_config%primordial_chemistry
        write(*,*) '0 grackle_config%gr_primordial_chemistry(1) = ',grackle_solver%myconfig%gr_primordial_chemistry(1)
        if(grackle_solver%myconfig%gr_primordial_chemistry(1)/=phys_config%primordial_chemistry)then
        write(*,*) 'grackle_config%gr_primordial_chemistry(1) = ',grackle_solver%myconfig%gr_primordial_chemistry(1)
        write(*,*) 'phys_config%primordial_chemistry = ',phys_config%primordial_chemistry
        call mpistop('must have the same parameters to both lists!')
        end if

        write(*,*) '>>>>>> 4 <<<<<<'

        if(grackle_solver%myconfig%gr_use_dust_density_field(1)/=phys_config%use_dust_density_field)then
        write(*,*) 'grackle_config%gr_use_dust_density_field(1) = ',grackle_solver%myconfig%gr_use_dust_density_field(1)
        write(*,*) 'phys_config%use_dust_density_field = ',phys_config%use_dust_density_field
        call mpistop('must have the same parameters to both lists!')
        end if  
  ...
  if(usrconfig%grackle_chemistry_on)then
      usrconfig%grackle_n_objects = usrconfig%ism_number+&
      usrconfig%jet_yso_number+usrconfig%cloud_number
      grackle_solver%myconfig          = grackle_default%myconfig
      grackle_solver%myboundaries%myconfig = grackle_default%myboundaries%myconfig
      call grackle_solver%read_parameters(grackle_solver%myconfig,files)
      grackle_solver%number_of_objects = usrconfig%grackle_n_objects
      call grackle_fields_config_allocate(usrconfig%ism_number,usrconfig%jet_yso_number,&
...
!> subroutine read unit used in the code
  ...
  subroutine usr_normalise_parameters()
   ...
   ! complet all physical unit in use
   if(usrconfig%physunit_on) then
      write(*,*) 'unit density from usr_yso 2 ',unit_density
    ...
    if(phys_config%use_grackle)then
      if(phys_config%primordial_chemistry>0)then
        !HI
        write(*,*) 'unit density from usr_yso 3',unit_density
        w_convert_factor(phys_ind%HI_density_) = unit_density
        write(*,*) 'unit density from usr_yso 4',w_convert_factor(phys_ind%HI_density_)
        !HII
    ...
    w_convert_factor(phys_ind%mup_) = phys_config%mean_mup
...
subroutine initglobaldata_usr
 ...
 real(dp) :: density_list(1:max_num_parameters)
 real(dp) :: densityunit_list(1:max_num_parameters)
 !------------------------------------
 !/
 ...
 if(mype==0)call usr_write_setting
  ...
  subroutine specialsource_usr(qdt,ixI^L,ixO^L,iw^LIM,qtC,wCT,qt,w,x)
    ...
    real(dp), dimension(ixI^S,1:nw)     :: w_init,w_tmp
    real(kind=dp), dimension(ixI^S) :: min_density,bg_lambda
        ...
        cond_ism_noescape : if(.not.(all(escape_patch(ixI^S))))then
          ...
          Loop_isms : do i_ism=0,usrconfig%ism_number-1
            ...
            !Add Grackle chemistry+heating+cooling module treatment here :
               ...
               ! force ism cooling/heating to maintain its original temperature : 
               if(usrconfig%grackle_force_ism_cool)then
                bg_lambda(ixI^S) = 0.0_dp
                bg_lambda(ixI^S) = 0.0_dp
                call ism_surround(i_ism)%set_w(ixI^L, ixO^L,qt,x,w_init,gr_solv=grackle_solver)
                
                call phys_to_conserved(ixI^L,ixI^L,w_init,x)
                w_tmp(ixI^S,1:nw) = w_init(ixI^S,1:nw)
                call grackle_solver%grackle_source(ixI^L,ixO^L,iw^LIM,x,qdt,qtC,&
                                    w_tmp,qt,w_tmp,dx_local,my_grackle_data,my_grackle_units)
                                    
                min_density(ixI^S)=0.0_dp         
            
                where(ism_surround(i_ism)%patch(ixO^S))
                bg_lambda(ixO^S) = w_tmp(ixO^S,phys_ind%e_)-&
                w_init(ixO^S,phys_ind%e_)
                min_density(ixO^S)=grackle_solver%myconfig%deviation_to_density(i_ism+1)*&
                w(ixO^S,phys_ind%rho_)
                !add cooling correction:
                w(ixO^S,phys_ind%e_)=w(ixO^S,phys_ind%e_)-&
                bg_lambda(ixO^S)
                end where
                ! add chemistry ism correction:
                if(phys_config%primordial_chemistry>0)then
                  where(ism_surround(i_ism)%patch(ixO^S))
                  w(ixO^S,phys_ind%HI_density_)        =  max(w(ixO^S,phys_ind%HI_density_)-&
                  (w_tmp(ixO^S,phys_ind%HI_density_)-w_init(ixO^S,phys_ind%HI_density_)),&
                  min_density(ixO^S))
                  w(ixO^S,phys_ind%HII_density_)        = max(w(ixO^S,phys_ind%HII_density_)-&
                  (w_tmp(ixO^S,phys_ind%HII_density_)-w_init(ixO^S,phys_ind%HII_density_)),&
                  min_density(ixO^S))
                  w(ixO^S,phys_ind%HeI_density_)        =  max(w(ixO^S,phys_ind%HeI_density_)-&
                  (w_tmp(ixO^S,phys_ind%HeI_density_)-w_init(ixO^S,phys_ind%HeI_density_)),&
                  min_density(ixO^S))
                  w(ixO^S,phys_ind%HeII_density_)        =  max(w(ixO^S,phys_ind%HeII_density_)-&
                  (w_tmp(ixO^S,phys_ind%HeII_density_)-w_init(ixO^S,phys_ind%HeII_density_)),&
                  min_density(ixO^S))
                  w(ixO^S,phys_ind%HeIII_density_)        =  max(w(ixO^S,phys_ind%HeIII_density_)-&
                  (w_tmp(ixO^S,phys_ind%HeIII_density_)-w_init(ixO^S,phys_ind%HeIII_density_)),&
                  min_density(ixO^S))
                  w(ixO^S,phys_ind%e_density_)        =  max(w(ixO^S,phys_ind%e_density_)-&
                  (w_tmp(ixO^S,phys_ind%e_density_)-w_init(ixO^S,phys_ind%e_density_)),&
                  min_density(ixO^S))
                  end where
                end if
                if(phys_config%primordial_chemistry>1)then
                  ...
                  (w_tmp(ixO^S,phys_ind%HM_density_)-w_init(ixO^S,phys_ind%HM_density_)),&
                  min_density(ixO^S))
                  w(ixO^S,phys_ind%H2I_density_)        =  max(w(ixO^S,phys_ind%H2I_density_)-&
                  (w_tmp(ixO^S,phys_ind%H2I_density_)-w_init(ixO^S,phys_ind%H2I_density_)),&
                  min_density(ixO^S))
                  w(ixO^S,phys_ind%H2II_density_)        =  max(w(ixO^S,phys_ind%H2II_density_)-&
                  (w_tmp(ixO^S,phys_ind%H2II_density_)-w_init(ixO^S,phys_ind%H2II_density_)),&
                  min_density(ixO^S))
                  end where
                end if
                if(phys_config%primordial_chemistry>2)then
                  ...
                  (w_tmp(ixO^S,phys_ind%DI_density_)-w_init(ixO^S,phys_ind%DI_density_)),&
                  min_density(ixO^S))
                  w(ixO^S,phys_ind%DII_density_) =  max(w(ixO^S,phys_ind%DII_density_)-&
                  (w_tmp(ixO^S,phys_ind%DII_density_)-w_init(ixO^S,phys_ind%DII_density_)),&
                  min_density(ixO^S))
                  w(ixO^S,phys_ind%HDI_density_) =  max(w(ixO^S,phys_ind%HDI_density_)-&
                  (w_tmp(ixO^S,phys_ind%HDI_density_)-w_init(ixO^S,phys_ind%HDI_density_)),&
                  min_density(ixO^S))
                  end where
                end if
                if(phys_config%use_metal_field==1)then
                  ...
                  (w_tmp(ixO^S,phys_ind%metal_density_)-w_init(ixO^S,phys_ind%metal_density_)),&
                  min_density(ixO^S))
                  end where
                end if
                if(phys_config%use_dust_density_field==1)then
                  ...
                  (w_tmp(ixO^S,phys_ind%dust_density_)-w_init(ixO^S,phys_ind%dust_density_)),&
                  min_density(ixO^S))
                  end where
                end if
...
!----------------------------------------------------------------
       ...
       !--------------------------------------------------------------
        ...
        if(usrconfig%grackle_chemistry_on)then
          if(grackle_solver%myconfig%use_grackle(1)==1)then
            ...
            !write(*,*) 'dtchem->dt = ', dtnew*time_convert_factor
          end if
        end if
  ...
  !/
    ...
    integer, parameter         :: irelerr_rho_           = 14
    integer, parameter         :: irelerr_p_           = 15
    integer, parameter         :: irelerr_v1_          = 16
    integer, parameter         :: irelerr_v2_          = 17
    integer, parameter         :: irelerr_v3_          = 18
    integer, parameter         :: irhod1torho_       = 19

    real(dp),dimension(ixI^S)                                  :: csound2,temperature, pressure_th
    real(dp),dimension(ixI^S)                                  :: gammaeff_usr, mup_eff
    ...
    case(icsound_)
      ...
      case(imeanmupeff_)
        ...
        win(ixO^S,nw+imeanmupeff_) = mup_eff(ixO^S)
        normconv(nw+imeanmupeff_)  = 1.0_dp
      case(ilevel_)
      ...
      case(indensity_)
        ...
        win(ixO^S,nw+indensity_)    = w(ixO^S,phys_ind%rho_)*&
        grackle_solver%myconfig%HydrogenFractionByMass(1)/mp
        if(phys_config%primordial_chemistry>0)then
          win(ixO^S,nw+indensity_)    = &
          (w(ixO^S,phys_ind%HI_density_)/(1.0_dp*mp))+&
          (w(ixO^S,phys_ind%HII_density_)/(1.0_dp*mp))
        end if
        ! ignores deuterium
        if(phys_config%primordial_chemistry>1)then
          win(ixO^S,nw+indensity_)= &
          win(ixO^S,nw+indensity_)+&
          (w(ixO^S,phys_ind%HM_density_)/(1.0_dp*mp))+&
          (2.0_dp * w(ixO^S,phys_ind%H2I_density_)/(2.0_dp*mp))+&
          (2.0_dp * w(ixO^S,phys_ind%H2II_density_)/(2.0_dp*mp))
        end if

        win(ixO^S,nw+indensity_)    = win(ixO^S,nw+indensity_)*&
      ...
      case(idensitygas)
        ...
        win(ixO^S,nw+idensitygas) = w(ixO^S,phys_ind%rho_)
        if(phys_config%primordial_chemistry>0)then
        ...
        if(phys_config%primordial_chemistry>1)then
          win(ixO^S,nw+idensitygas) = win(ixO^S,nw+idensitygas)+&
...
!---------------------------------------------------------------------
  ...
  subroutine usr_write_setting
    ...
    if(usrconfig%grackle_chemistry_on)then
      ...
      call grackle_solver%denormalize()
      call grackle_solver%write_setting(unit_config,gr_patches_name,&
      gr_patches_indices_global,gr_patches_indices_local,gr_profiles,&
      gr_epsilon_tol,gr_density_method)
      call grackle_solver%normalize()
