    ...
    cond_isotherm_0 : if(usrconfig%phys_isotherm_on)then
      ...
      cond_gamma_1 : if(dabs(phys_config%gamma-1.0_dp)<smalldouble) then
        if(usrconfig%phys_temperature_isotherm>0.0_dp) then
          usrconfig%phys_adiab = kB*phys_config%mean_mass/(phys_config%mean_mup*mp)*usrconfig%phys_temperature_isotherm
        elseif(usrconfig%phys_adiab>0.0_dp)then
          usrconfig%phys_temperature_isotherm = usrconfig%phys_adiab*phys_config%mean_mup*mp/(kB*phys_config%mean_mass)
        else
