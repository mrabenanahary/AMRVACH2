   ...
   !----------------------------------------------------
    ...
    cond_csound_set : if(self%myconfig%c_sound>0.0_dp) then
       self%myconfig%pressure = self%myconfig%c_sound**2.0_dp * self%myconfig%density /&
                                phys_config%gamma
       self%myconfig%temperature = self%myconfig%c_sound**2.0_dp*&
                                self%myconfig%mean_mup/(kB*phys_config%gamma*self%myconfig%mean_mass)
    !----------------------------------------------------
    ...
    !----------------------------------------------------
     ...
     if(self%myconfig%temperature>0.0_dp) then
      self%myconfig%pressure = self%myconfig%number_density/self%myconfig%mean_mup*&
                            kB* self%myconfig%temperature
      self%myconfig%pressure = self%myconfig%pressure * self%myconfig%mean_mass
      ...
      self%myconfig%temperature = self%myconfig%temperature/self%myconfig%mean_mass
     end if
        ...
        if(dabs( self%myconfig%profile_shift_pressure(idim,iside))<smalldouble.and. &
          ...
          if(dabs( self%myconfig%profile_shift_pressure(idim,iside))<smalldouble)then
           self%myconfig%profile_shift_pressure(idim,iside) = &
              ...
              /self%myconfig%mean_mup*kB* &
              self%myconfig%profile_shift_temperature(idim,iside)*self%myconfig%mean_mass
          else
            self%myconfig%profile_shift_temperature(idim,iside) =  &
              self%myconfig%mean_mup* self%myconfig%profile_shift_pressure(idim,iside)  / &
              (self%myconfig%profile_shift_number_density(idim,iside)*kB*self%myconfig%mean_mass)
          end if
        end if
