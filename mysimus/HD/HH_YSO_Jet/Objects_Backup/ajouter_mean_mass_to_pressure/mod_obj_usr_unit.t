...
case('mks')
 ...
 subroutine usr_physical_unit_complet(the_physics_type,self)
   ...
   if(self%myconfig%temperature>smalldouble)then
    self%myconfig%pressure      = self%myconfig%number_density * self%myconfig%mean_mass&
            *kB*self%myconfig%temperature / self%myconfig%mean_mup
   ...
   self%myconfig%luminosity       = self%myconfig%pressure*self%myconfig%velocity**2.0_dp &
     ...
     if(phys_config%cool_saveL)w_convert_factor(phys_ind%Lcool1_)   =    &
           1.0_dp/(self%myconfig%number_density**2.0_dp * self%myconfig%time &
                 / self%myconfig%pressure &
                           * (self%myconfig%mean_mass/self%myconfig%mean_mup)**2.0_dp)
