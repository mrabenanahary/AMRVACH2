...
implicit none
  ...
  type rel_wind_parameters
        ...
        character(len=78)    :: obj_name           !> wind obj name
        integer              :: myindice           !> wind associated indice
        ...
        real(dp)             :: time_evol_r_out    !> wind time to evolve r_out
  ...
  type rel_wind
    ...
    contains
     ...
     PROCEDURE, PASS(self)    :: get_patch       => usr_wind_patch
  end type rel_wind
contains
 ...
 !> Read the ism parameters  from a parfile
  subroutine usr_wind_read_p(self,wind_config,files)
    ...
    do i_file = 1, size(files)
       ...
       read(unitpar, usr_pulsar_wind_list, iostat=i_error_read)
...
!> subroutine default setting for cloud
 subroutine usr_wind_set_default(self)
  ...
  self%myconfig%obj_name               = 'relativistic wind'
  ...
  self%myconfig%time_evol_r_out        = smalldouble
 ...
 subroutine usr_wind_set_complet(self,star_speed)
   ...
   if (dabs(self%myconfig%mass_flux)>smalldouble)then
    self%myconfig%density_init        = self%myconfig%mass_flux/(4.0_dp*dpi*     &
                               ...
                               dabs(self%myconfig%velocity(r_))*self%myconfig%lfac**2.0_dp)
...
!--------------------------------------------------------------------
 subroutine usr_wind_normalize(self,physunit_inuse)
  ...
  self%myconfig%time_evol_r_out      = self%myconfig%time_evol_r_out  /physunit_inuse%myconfig%time
...
!--------------------------------------------------------------------
 ...
 subroutine usr_wind_patch(ixI^L,ixO^L,qt,x,self,force_refine,dx_loc,r_out_limit)
  implicit none
  ...
  real(kind=dp),optional     :: r_out_limit
  ! .. local ..
  integer                    :: idims
  ...
  if(qt<=self%myconfig%time_start)then
   ...
   r_in =self%myconfig%r_in
   r_out=0.5_dp*((1.0_dp+tanh((qt-self%myconfig%time_start)&
         /self%myconfig%time_evol_r_out))*self%myconfig%r_out_impos&
         +(1.0_dp-tanh((qt-self%myconfig%time_start)&
         /self%myconfig%time_evol_r_out))*self%myconfig%r_out_init)
  end if
  if(present(r_out_limit))then
   r_out_limit = r_out
  end if
